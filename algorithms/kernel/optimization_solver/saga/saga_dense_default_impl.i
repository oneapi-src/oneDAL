...
services::Status SagaKernel<algorithmFPType, method, cpu>::compute(HostAppIface* pHost,
    NumericTable *inputArgument, NumericTable *minimum, NumericTable *nIterations,
    NumericTable *gradientsTableInput, NumericTable *gradientsTableResult,
    Parameter *parameter, engines::BatchBase &engine)
{
    ...
    for(size_t iter = 0; iter < maxIterations; iter++)
    {
        ...
        {
            if(iter%1024 == 0)
                rngTask.getWithReplacement(pValues);
        ...
        if(learningRateLength != 0)
            stepLength = learningRateArray[iter % learningRateLength];
        ...
        result |= daal::services::internal::daal_memcpy_s(previous, sizeArgument * sizeof(algorithmFPType),
                                          workValue, sizeArgument * sizeof(algorithmFPType));
        ...
        for(size_t k = 0; k < sizeArgument; k++)
        {
            ...
            workValue[k] = workValue[k] - stepLength*
                           (gradient[k] - savedGradients[displacement + k] + summGrads[k]*inverse_n);
        ...
        for(size_t k = 0; k < sizeArgument; k++)
        {
            continueCheck |= (daal::internal::Math<algorithmFPType,cpu>::sFabs(previous[k] - workValue[k]) >=
        ...
        result |= daal::services::internal::daal_memcpy_s(savedGradients + displacement, sizeArgument * sizeof(algorithmFPType),
                                          gradient, sizeArgument * sizeof(algorithmFPType));
