...
services::Status SGDKernelOneAPI<algorithmFPType, miniBatch, cpu>::makeStep(
...
template <typename algorithmFPType, CpuType cpu>
services::Status SGDKernelOneAPI<algorithmFPType, miniBatch, cpu>::vectorNorm(
...
template <typename algorithmFPType, CpuType cpu>
void SGDKernelOneAPI<algorithmFPType, miniBatch, cpu>::buildProgram(
...
template <typename algorithmFPType, CpuType cpu>
services::Status SGDKernelOneAPI<algorithmFPType, miniBatch, cpu>::compute(
    HostAppIface* pHost, NumericTable *inputArgument,
        ...
        if (epoch % L == 0)
        {
            if (nIter > 1 && accuracyThreshold > 0)
            {
                ...
                const double gradientThreshold =
                    accuracyThreshold * daal::internal::Math<algorithmFPType, cpu>::sMax(1.0, pointNorm);
