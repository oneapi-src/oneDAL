...
services::Status I1SGDKernel<algorithmFPType, momentum, cpu>::compute(HostAppIface* pHost,
    NumericTable *inputArgument, NumericTable *minimum, NumericTable *nProceededIterationsNT,
    interface1::Parameter<momentum> *parameter, NumericTable *learningRateSequence,
    NumericTable *batchIndices, OptionalArgument *optionalArgument, OptionalArgument *optionalResult, engines::BatchBase &engine)
{
    ...
    I1SGDmomentumTask<algorithmFPType, cpu> task(batchSize,
            nTerms,
            minimum,
            batchIndices,
...
template<typename algorithmFPType, CpuType cpu>
services::Status I1SGDmomentumTask<algorithmFPType, cpu>::makeStep(
...
Status I1SGDmomentumTask<algorithmFPType, cpu>::setStartValue(NumericTable *inputArgument, NumericTable *minimum)
{
    ...
    processByBlocks<cpu>(minimum->getNumberOfRows(), [=, &safeStat](size_t startOffset, size_t nRowsInBlock)
    {
        ...
        {
            int result = daal::services::internal::daal_memcpy_s(workArray, nRowsInBlock * sizeof(algorithmFPType),
...
template<typename algorithmFPType, CpuType cpu>
Status I1SGDmomentumTask<algorithmFPType, cpu>::init(NumericTable *batchIndicesTable,
...
template<typename algorithmFPType, CpuType cpu>
Status I1SGDmomentumTask<algorithmFPType, cpu>::init(NumericTable *batchIndicesTable,
    NumericTable *resultTable, interface1::Parameter<momentum> *parameter,
    NumericTable *pastUpdateInput,
    NumericTable *lastIterationInput)
{
    indicesStatus = (batchIndicesTable ? user : (batchSize < nTerms ? random : all));
