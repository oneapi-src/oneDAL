...
services::Status I1SGDKernel<algorithmFPType, miniBatch, cpu>::compute(HostAppIface* pHost,
    ...
    NumericTable *batchIndices, OptionalArgument *optionalArgument, OptionalArgument *optionalResult, engines::BatchBase &engine)
{
    ...
    for(size_t epoch = task.startIteration; s.ok() && (epoch < (task.startIteration + nIter)); epoch++)
    {
        ...
        if(epoch % L == 0)
        {
            ...
            result |= daal_memcpy_s(task.prevWorkValue.get(), argumentSize * sizeof(algorithmFPType), workValue, argumentSize * sizeof(algorithmFPType));
        }
...
template<typename algorithmFPType, CpuType cpu>
void I1SGDMiniBatchTask<algorithmFPType, cpu>::makeStep(
...
template<typename algorithmFPType, CpuType cpu>
Status I1SGDMiniBatchTask<algorithmFPType, cpu>::init(NumericTable *startValueTable,
    NumericTable *learningRateSequenceTable,
    NumericTable *conservativeSequenceTable,
    NumericTable *nIterationsTable,
    NumericTable *batchIndicesTable,
    OptionalArgument *optionalArgument)
{
    ...
    if(pastWorkValueInput)
    {
        ...
        result = daal_memcpy_s(prevWorkValue.get(), argumentSize * sizeof(algorithmFPType),
