/* file: sgd_dense_default_v1_impl.i */
/*******************************************************************************
...
services::Status I1SGDKernel<algorithmFPType, defaultDense, cpu>::compute(HostAppIface * pHost, NumericTable * inputArgument, NumericTable * minimum,
                                                                          NumericTable * nIterations, interface1::Parameter<defaultDense> * parameter,
    ...
    for (epoch = startIteration; s.ok() && (epoch < (startIteration + nIter)); epoch++)
    {
        ...
        processByBlocks<cpu>(
            ...
            [=, &safeStat](size_t startOffset, size_t nRowsInBlock) {
                ...
                for (int j = 0; j < nRowsInBlock; j++) { workLocal[j] = workLocal[j] - learningRate * gradientLocal[j]; }
            },
            256);
