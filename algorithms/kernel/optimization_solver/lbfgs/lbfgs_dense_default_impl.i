 ...
 */
template<typename algorithmFPType, CpuType cpu>
services::Status LBFGSKernel<algorithmFPType, defaultDense, cpu>::compute1(HostAppIface* pHost,
    NumericTable* correctionPairsInput, NumericTable* correctionIndicesInput,
    NumericTable *inputArgument, NumericTable* averageArgLIterInput, OptionalArgument *optionalArgumentInput, NumericTable *correctionPairsResult,
    NumericTable *correctionIndicesResult, NumericTable *minimum, NumericTable *nIterationsNT, NumericTable *averageArgLIterResult,
    OptionalArgument *optionalArgumentResult, interface1::Parameter *parameter, engines::BatchBase &engine)
{
    ...
    sum_of_functions::interface1::BatchPtr gradientFunction = parameter->function;
    const size_t nTerms = gradientFunction->sumOfFunctionsParameter->numberOfTerms;
    ...
    for(; t < nLIterations;)
    {
        for(; epoch < (t + 1) * L; ++epoch, ++curIteration)
        {
            ...
            bool bContinue = true;
            DAAL_CHECK_STATUS(s, task.updateArgument1(curIteration, t, epoch, m, correctionIndex, nTerms, batchSize,
                accuracyThreshold, gradientFunction, ntGradient, ntValue, argument, bContinue, engineImpl, useWolfeConditions));
            if(!bContinue)
            {
                s |= task.setToResult(correctionIndicesResult, nIterationsNT, optionalArgumentResult, curIteration, epoch, correctionIndex);
    ...
    {
        bool bContinue = true;
        s = task.updateArgument1(curIteration, t, epoch, m, correctionIndex, nTerms, batchSize,
            accuracyThreshold, gradientFunction, ntGradient, ntValue, argument, bContinue, engineImpl, useWolfeConditions);
        if(!s || !bContinue)
        {
            s |= task.setToResult(correctionIndicesResult, nIterationsNT, optionalArgumentResult, curIteration, epoch, correctionIndex);
 ...
 */
template<typename algorithmFPType, CpuType cpu>
algorithmFPType LBFGSTask<algorithmFPType, cpu>::lineSearch1(algorithmFPType* x, NumericTablePtr &ntValue, NumericTablePtr &ntGradient, algorithmFPType* dx, sum_of_functions::interface1::BatchPtr &gradientFunction, bool& continueSearch)
{
    continueSearch = false;
    algorithmFPType stepLength = 1.0;
    const algorithmFPType stepLengthZero = 0.5;
            ...
            daal::algorithms::engines::internal::BatchBaseImpl* engine, bool useWolfeConditions)
{
    ...
    {
        result |= daal::services::internal::daal_memcpy_s(gradientPrev, this->argumentSize * sizeof(algorithmFPType),
    ...
    if(useWolfeConditions)
    {
        result |= daal::services::internal::daal_memcpy_s(gradientCurr, this->argumentSize * sizeof(algorithmFPType),
...
template<typename algorithmFPType, CpuType cpu>
Status LBFGSTask<algorithmFPType, cpu>::updateArgument1(size_t iIteration,
            size_t t, size_t epoch, size_t m, size_t correctionIndex, size_t nTerms,
            size_t batchSize, algorithmFPType accuracyThreshold,
            sum_of_functions::interface1::BatchPtr &gradientFunction,
            NumericTablePtr &ntGradient, NumericTablePtr &ntValue, algorithmFPType *argument, bool& bContinue,
            daal::algorithms::engines::internal::BatchBaseImpl* engine, bool useWolfeConditions)
{
    const algorithmFPType one = 1.0;
    ...
    if(useWolfeConditions)
    {
        ...
        {
            bool continueSearch = true;
            alpha = lineSearch1(argument,ntValue, ntGradient, gradient, gradientFunction, continueSearch);
            this->continueLineSearch = continueSearch;
        }
        stepLengthVal = -alpha;
    }
...
Status LBFGSTaskBase<algorithmFPType, cpu>::setStartArgument(NumericTable *startValueTable)
{
    ...
    int result = daal::services::internal::daal_memcpy_s(argument, argumentSize * sizeof(algorithmFPType),
...
*/
template<typename algorithmFPType, CpuType cpu>
LBFGSTask<algorithmFPType, cpu>::LBFGSTask(const interface1::Parameter *parameter, NumericTable *minimum) :
    super(minimum),
    mtBatchIndices(parameter->batchIndices.get()), mtCorrectionPairBatchIndices(parameter->correctionPairBatchIndices.get()),
    mtStepLength(parameter->stepLengthSequence.get(), 0, 1), correctionPairs(nullptr),
    correctionS(nullptr), correctionY(nullptr),
...
template<typename algorithmFPType, CpuType cpu>
Status LBFGSTask<algorithmFPType, cpu>::init(NumericTable *inputArgument, NumericTable* correctionPairsInput, NumericTable* averageArgLIterInput,
    OptionalArgument *optionalArgumentInput, const interface1::Parameter *parameter, NumericTable *minimum, NumericTable *averageArgLIterResult,
    NumericTable *correctionPairsResult, size_t nTerms, size_t batchSize, size_t correctionPairBatchSize)
{
    Status s;
    this->continueLineSearch = true;
