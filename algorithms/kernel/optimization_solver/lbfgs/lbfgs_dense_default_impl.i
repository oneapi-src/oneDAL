 ...
 */
template <typename algorithmFPType, CpuType cpu>
services::Status LBFGSKernel<algorithmFPType, defaultDense, cpu>::compute1(
    HostAppIface * pHost, NumericTable * correctionPairsInput, NumericTable * correctionIndicesInput, NumericTable * inputArgument,
    NumericTable * averageArgLIterInput, OptionalArgument * optionalArgumentInput, NumericTable * correctionPairsResult,
    NumericTable * correctionIndicesResult, NumericTable * minimum, NumericTable * nIterationsNT, NumericTable * averageArgLIterResult,
    OptionalArgument * optionalArgumentResult, interface1::Parameter * parameter, engines::BatchBase & engine)
{
    ...
    {
        bool bContinue = true;
        s = task.updateArgument1(curIteration, t, epoch, m, correctionIndex, nTerms, batchSize, accuracyThreshold, gradientFunction, ntGradient,
                                 ntValue, argument, bContinue, engineImpl, useWolfeConditions);
        if (!s || !bContinue)
        {
            s |= task.setToResult(correctionIndicesResult, nIterationsNT, optionalArgumentResult, curIteration, epoch, correctionIndex);
 ...
 */
template <typename algorithmFPType, CpuType cpu>
algorithmFPType LBFGSTask<algorithmFPType, cpu>::lineSearch1(algorithmFPType * x, NumericTablePtr & ntValue, NumericTablePtr & ntGradient,
                                                             algorithmFPType * dx, sum_of_functions::interface1::BatchPtr & gradientFunction,
                                                             bool & continueSearch)
{
    continueSearch                        = false;
    algorithmFPType stepLength            = 1.0;
...
template <typename algorithmFPType, CpuType cpu>
Status LBFGSTask<algorithmFPType, cpu>::updateArgument1(size_t iIteration, size_t t, size_t epoch, size_t m, size_t correctionIndex, size_t nTerms,
                                                        size_t batchSize, algorithmFPType accuracyThreshold,
                                                        sum_of_functions::interface1::BatchPtr & gradientFunction, NumericTablePtr & ntGradient,
                                                        NumericTablePtr & ntValue, algorithmFPType * argument, bool & bContinue,
                                                        daal::algorithms::engines::internal::BatchBaseImpl * engine, bool useWolfeConditions)
{
    const algorithmFPType one = 1.0;
...
template <typename algorithmFPType, CpuType cpu>
Status LBFGSTask<algorithmFPType, cpu>::init(NumericTable * inputArgument, NumericTable * correctionPairsInput, NumericTable * averageArgLIterInput,
                                             OptionalArgument * optionalArgumentInput, const interface1::Parameter * parameter,
                                             NumericTable * minimum, NumericTable * averageArgLIterResult, NumericTable * correctionPairsResult,
                                             size_t nTerms, size_t batchSize, size_t correctionPairBatchSize)
{
    Status s;
    this->continueLineSearch = true;
    /* Initialize work value with a start value provided by user */
...
template <typename algorithmFPType, CpuType cpu>
Status LBFGSTask<algorithmFPType, cpu>::initCorrectionPairs(NumericTable * correctionPairsInput, const interface1::Parameter * parameter,
                                                            NumericTable * correctionPairsResult)
{
    Status s;
    DAAL_OVERFLOW_CHECK_BY_MULTIPLICATION(size_t, parameter->m, this->argumentSize);
