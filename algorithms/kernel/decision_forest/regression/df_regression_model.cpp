...
static bool traverseNodeDF(size_t level, const ModelImpl::TreeType::NodeType::Base& n, decision_forest::regression::NodeVisitor& visitor)
{
    if(n.isSplit())
    {
    ...
    return visitor.onLeafNode(level, l->response);
}

void ModelImpl::traverseDF(size_t iTree, decision_forest::regression::NodeVisitor& visitor) const
{
    if(iTree >= size())
        return;
...
static bool traverseNodesBF(size_t level, NodePtrArray& aCur,
    NodePtrArray& aNext, decision_forest::regression::NodeVisitor& visitor)
{
    for(size_t i = 0; i < aCur.size(); ++i)
    {
        ...
        {
            const ModelImpl::TreeType::NodeType::Leaf* l = ModelImpl::TreeType::NodeType::castLeaf(&n);
            if(!visitor.onLeafNode(level, l->response))
                return false; //do not continue traversing
        }
    }
    aCur.clear();
