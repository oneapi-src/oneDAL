...
void BatchContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    if(valueFlag)
    {
        value = result->get(objective_function::resultCollection, objective_function::valueIdx).get();
    }

    bool hessianFlag = ((parameter->resultsToCompute & objective_function::hessian) != 0) ? true : false;
    if(hessianFlag)
    {
        hessian = result->get(objective_function::resultCollection, objective_function::hessianIdx).get();
    }

    bool gradientFlag = ((parameter->resultsToCompute & objective_function::gradient) != 0) ? true : false;
    if (gradientFlag)
    {
        gradient = result->get(objective_function::resultCollection, objective_function::gradientIdx).get();
    }
