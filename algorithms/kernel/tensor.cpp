...
{
    TensorOffsetLayout _Layout = createDefaultSubtensorLayout();

    if( !_Layout.isLayout(layout) )
    {
        _Layout.shuffleDimensions(layout.getIndices());
    }

    block.saveOffsetLayout(_Layout);

    size_t  nDim = _Layout.getDimensions().size();
    const size_t *dimSizes    = &((_Layout.getDimensions())[0]);
