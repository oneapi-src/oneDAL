    ...
    const NumericTablePtr& xTable)
{
    ...
    for(size_t i = 1; i < nClasses; i++)
    {
        for(size_t j = 0; j < i; j++)
        {
            ...
            _simplePrediction->inputBase->set(classifier::prediction::data, xTable);
            _simplePrediction->inputBase->set(classifier::prediction::model, model->getTwoClassClassifierModel(imodel));
