...
protected:
    ...
    Status getRowBlockImpl(size_t rowIndex, size_t startColIndex, size_t blockSize, size_t cacheOffset, const algorithmFPType*& block)
    {
        ...
        _kernel->parameterBase->rowIndexY = _doShrinking ? _shrinkingRowIndices[rowIndex] : rowIndex;
        Status s;
        for(size_t i = 0; i < blockSize; i++)
        {
            _kernel->parameterBase->rowIndexX = _doShrinking ? _shrinkingRowIndices[startColIndex + i] : startColIndex + i;
            _kernel->parameterBase->rowIndexResult = i;
