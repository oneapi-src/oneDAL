...
DistributedInput::DistributedInput() : InputIface(lastStep2MasterInputId + 1)
{
 ...
 */
services::Status DistributedInput::check(const daal::algorithms::Parameter *parameter, int method) const
{
    data_management::DataCollectionPtr spModels = get(partialModels);
    data_management::DataCollection *models = spModels.get();
    DAAL_CHECK(models, services::ErrorNullModel);

    size_t size = models->size();
    DAAL_CHECK(size, services::ErrorIncorrectNumberOfElementsInInputCollection);
