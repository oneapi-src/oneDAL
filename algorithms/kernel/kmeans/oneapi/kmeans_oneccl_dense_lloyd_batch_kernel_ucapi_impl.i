/* file: kmeans_oneccl_dense_lloyd_batch_kernel_ucapi_impl.i */
/*******************************************************************************
                                                                     ...
                                                                     const algorithms::Parameter * par)
{
    ...
    daal::services::String cachekey("__daal_algorithms_kmeans_lloyd_dense_batch_");
    ...
    const uint32_t nValuesInBlock    = 1024 * 1024 * 1024 / sizeof(algorithmFPType);
    const uint32_t nMinRows          = 1;
    uint32_t gemmBlockSize           = nValuesInBlock;
    ...
    uint32_t datasetBlockSize = nValuesInBlock;
    ...
    uint32_t blockSize = datasetBlockSize > gemmBlockSize ? gemmBlockSize : datasetBlockSize;
    if (blockSize > nRows)
    {
        blockSize = nRows;
    }
    ...
    auto finalize_clusters          = kernel_factory.getKernel("finalize_clusters", &st);
    ...
    size_t iter    = 0;
    size_t nBlocks = rankSize / blockSize + int(rankSize % blockSize != 0);
    ...
    for (size_t block = 0; block < nBlocks; block++)
    {
        ...
        BlockDescriptor<int> assignmentsRows;
        ntAssignments->getBlockOfRows(first, curBlockSize, readOnly, assignmentsRows);
        auto assignments = assignmentsRows.getBuffer();
...
template <typename algorithmFPType>
uint32_t KMeansOneCclDefaultBatchKernelUCAPI<algorithmFPType>::getCandidatePartNum(uint32_t nClusters)
{
    return _maxLocalBuffer / nClusters / sizeof(algorithmFPType);
}
template <typename algorithmFPType>
const char * KMeansOneCclDefaultBatchKernelUCAPI<algorithmFPType>::getBuildOptions(uint32_t nClusters)
{
...
template <typename algorithmFPType>
uint32_t KMeansOneCclDefaultBatchKernelUCAPI<algorithmFPType>::getWorkgroupsCount(uint32_t rows)
{
...
template <typename algorithmFPType>
uint32_t KMeansOneCclDefaultBatchKernelUCAPI<algorithmFPType>::getComputeSquaresWorkgroupsCount(uint32_t nFeatures)
{
...
template <typename algorithmFPType>
const char * KMeansOneCclDefaultBatchKernelUCAPI<algorithmFPType>::getComputeSquaresKernelName(uint32_t nFeatures)
{
    if (nFeatures <= 32)
    {
...
template <typename algorithmFPType>
void KMeansOneCclDefaultBatchKernelUCAPI<algorithmFPType>::computePartialCandidates(
...
template <typename algorithmFPType>
void KMeansOneCclDefaultBatchKernelUCAPI<algorithmFPType>::mergePartialCandidates(
...
template <typename algorithmFPType>
void KMeansOneCclDefaultBatchKernelUCAPI<algorithmFPType>::partialReduceCentroids(
                                                                             ...
                                                                             uint32_t nFeatures, UniversalBuffer & finalCentroids, Status * st)
{
    ...
    args.set(0, finalCounters, AccessModeIds::read);
    args.set(1, finalCentroids, AccessModeIds::write);
    ...
    {
        DAAL_ITTNOTIFY_SCOPED_TASK(compute.updateObjectiveFunction.run);
        context.run(range, kernel_finalize_clusters, args, st);
    }
}
