/* file: kmeans_cl_kernels_distr.cl */
/*******************************************************************************
...
DECLARE_SOURCE(
    ...
    __kernel void compute_assignments(__global const algorithmFPType * distances, __global int * assignments, int N, int K) {
        const int global_id = get_global_id(0);
    ...
    __kernel void merge_reduce_centroids(__global algorithmFPType * partialCentroids, __global int * partialCentroidsCounters,
        ...
        if (local_id == 0 && partialCentroidsCounters[cl_id] != 0)
        {
            for (int j = 0; j < P; j++)
            {
                centroids[cl_id * P + j] = partialCentroids[cl_id * P + j];
            }
        }
    }
