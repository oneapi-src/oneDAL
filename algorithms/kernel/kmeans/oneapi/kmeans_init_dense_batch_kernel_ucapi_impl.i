...
Status KMeansInitDenseBatchKernelUCAPI<method, algorithmFPType>::init( size_t p, size_t n, size_t nRowsTotal, size_t nClusters, NumericTable *ntClusters,
    NumericTable *ntData, unsigned int seed, engines::BatchBase &engine, size_t& clustersFound)
{
    ...
    if(method == deterministicDense)
    {
        ...
        context.copy(clusters, 0, data, 0, nClusters * p, &st); DAAL_CHECK_STATUS_VAR(st);
    ...
    if(method == randomDense)
    {
        ...
        gatherRandom(context, gather_random, data, clusters, indices, nRowsTotal, clustersFound, p, &st); DAAL_CHECK_STATUS_VAR(st);
...
template <Method method, typename algorithmFPType>
services::Status KMeansInitDenseBatchKernelUCAPI<method, algorithmFPType>::compute( size_t na, const NumericTable *const *a,
                                                                          size_t nr, const NumericTable *const *r,
                                                                          const Parameter *par,
...
template <Method method, typename algorithmFPType>
void KMeansInitDenseBatchKernelUCAPI<method, algorithmFPType>::gatherRandom
        (ExecutionContextIface& context,
         const KernelPtr& kernel_gather_random,
         const Buffer<algorithmFPType>& data,
         const Buffer<algorithmFPType>& clusters,
