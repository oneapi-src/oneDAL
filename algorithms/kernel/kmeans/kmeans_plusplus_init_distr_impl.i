    ...
    NumericTable** aLocalData, NumericTable* pOutput)
{
    ...
    for(size_t i = 0; i < outputSize; ++i)
    {
        ...
        result |= daal::services::internal::daal_memcpy_s(outputBD.get(), sizeof(algorithmFPType)*nFeatures, dataBD.get(), sizeof(algorithmFPType)*nFeatures);
        if((i + 1) == outputSize)
            break;
    ...
    const NumericTable* ntCand, const NumericTable* ntWeights, const MemoryBlock* pRngState, NumericTable* pCentroids, engines::BatchBase &engine)
{
    ...
    TaskPlusPlusBatch<algorithmFPType, cpu, DataHelperDense<algorithmFPType, cpu> > task(const_cast<NumericTable*>(ntCand),
        weightsBD.get(), pCentroids, par->nClusters, nLocalTrials, engine);
    return task.run();
}
