...
public:
    ...
    algorithmFPType getRowSumSq(size_t iRow)
    {
        const algorithmFPType* pData = _ntDataBD.get() + iRow*_dim;
        double norm2 = 0;
        for(size_t i = 0; i < _dim; ++i)
            norm2 += pData[i] * pData[i];
        return norm2;
    }
...
public:
    ...
    algorithmFPType getRowSumSq(size_t iRow)
    {
        ...
        for(size_t i = 0; i < nValues; ++i, ++pData)
            res += (*pData) * (*pData);
        return res;
    }
...
public:
    ...
    double updateMinDistInBlock(const algorithmFPType* aWeights, size_t iStartRow, size_t nRowsToProcess,
        const algorithmFPType* pLastAddedCenter, algorithmFPType lastAddedCenterSumSq,
        algorithmFPType* aMinDist) const
    {
        ...
        {
            algorithmFPType dist2(lastAddedCenterSumSq); //dist from iRow to the last added center
            for(size_t i = 0; i < dim; ++i)
                dist2 += pData[iRow*dim + i] * (pData[iRow*dim + i] - algorithmFPType(2.)*pLastAddedCenter[i]);
            if(aWeights)
    ...
    double copyOneRowCalcSumSq(size_t iRow, algorithmFPType* pDst) const
    {
        ...
        algorithmFPType* aMinDist)
    {
        ...
        for(size_t iRow = 0; iRow < nRowsToProcess; ++iRow)
        {
            ...
            for(size_t i = 0; i < nValues; ++i, ++csrCursor)
                dist2 += pData[csrCursor] * (pData[csrCursor] - algorithmFPType(2.)*pLastAddedCenter[colIdx[csrCursor] - 1]);
            if(aWeights)
                dist2 *= aWeights[iStartRow + iRow];
            if(pDistSq[iRow] > dist2)
                pDistSq[iRow] = dist2;
...
protected:
    ...
    BaseRNGs<cpu>& _brng;
    size_t _nBlocks;
    TSmartPtr<algorithmFPType, cpu> _lastAddedCenter; //center last added to the clusters
    TSmartPtr<algorithmFPType, cpu> _aMinDist; //distance to the nearest cluster for every point
    TSmartPtr<algorithmFPType, cpu> _aMinDistAcc; //accumulated aMinDist per every block
    ...
    TSmartPtr<algorithmFPType, cpu> _aProbability; //array of probabilities for all candidates
    static const size_t _nRowsInBlock = 512;
};
...
public:
        ...
        _L(par->oversamplingFactor*par->nClusters), _R(par->nRounds)
    {
        ...
        _lastAddedCenterNorm2.reset(this->_data.dim*_L);//reserve memory for L points
...
private:
    ...
    bool findBestCandidate(typename DataHelper::BlockHelperType& blockHelper, size_t iRow, algorithmFPType* pDistSq, size_t nRowsToProcess,
        size_t nNewCandidates, size_t& iBestCandidate, const algorithmFPType* gemmResult);
...
private:
    ...
    TSmartPtr<size_t, cpu> _aCandidateIdx; //array[maxNumberOfCandidates], contains row indices of the candidates added so far
    TSmartPtr<size_t, cpu> _aCandidateRating; //array[maxNumberOfCandidates], number of points closest to each candidate found so far
    TSmartPtr<size_t, cpu> _aNearestCandidateIdx; //index of the nearest candidate in _aCandidateIdx per each point
    TSmartPtr<algorithmFPType, cpu> _lastAddedCenterNorm2; //array[L] contains 0.5*(center, center) for each last added center
};
...
size_t TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::samplePoints(size_t nPt, size_t* aPt, size_t iRound)
{
    ...
    for(size_t iPt = 0; iPt < nPt; ++iPt)
    {
        ...
        if(aMinDist[iRow] > 0)
        {
            ...
            const size_t iPrevCandidate = pNearestCandIndex[iRow];
...
template <typename algorithmFPType, CpuType cpu, typename DataHelper>
bool TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::findBestCandidate(typename DataHelper::BlockHelperType& blockHelper, size_t iRow,
    algorithmFPType* pDistSq, size_t nRowsToProcess, size_t nNewCandidates, size_t& iBestCandidate, const algorithmFPType* gemmResult)
{
    const algorithmFPType rowNorm2 = blockHelper.getRowSumSq(iRow);
    algorithmFPType val = algorithmFPType(0.5)*(pDistSq[iRow] - rowNorm2);
    bool bFound = false;
    const algorithmFPType* pLastAddedCenter = this->_lastAddedCenter.get();
    for(size_t iCandidate = 0; iCandidate < nNewCandidates; ++iCandidate)
    {
        algorithmFPType valCand = this->_lastAddedCenterNorm2.get()[iCandidate] - blockHelper.getGemmResult(iRow, iCandidate, nRowsToProcess, nNewCandidates, gemmResult);
        if(val > valCand)
        {
            pDistSq[iRow] = rowNorm2 + valCand + valCand;
            val = valCand;
...
template <typename algorithmFPType, CpuType cpu, typename DataHelper>
void TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::processBlock(size_t iBlock, TlsPPData_t* tlsLocal,
    size_t iFirstOfNewCandidates, size_t nNewCandidates)
{
    int* aCandidateRating = tlsLocal->aCandidateRating;
    size_t nRowsToProcess = this->_nRowsInBlock;
    if(iBlock == this->_nBlocks - 1)
        nRowsToProcess = this->_data.nRows - iBlock * this->_nRowsInBlock;
    const size_t iStartRow = iBlock*this->_nRowsInBlock;

    typename DataHelper::BlockHelperType blockHelper(this->_data.ntIface(), this->_data.dim, iStartRow, nRowsToProcess);
    blockHelper.callGemm(this->_lastAddedCenter.get(), nRowsToProcess, nNewCandidates, tlsLocal->gemmResult);

    algorithmFPType* pDistSq = this->_aMinDist.get() + iStartRow;
    size_t* pNearestCandIndex = _aNearestCandidateIdx.get() + iStartRow;
    algorithmFPType sumOfDist2 = 0;
    for(size_t iRow = 0; iRow < nRowsToProcess; ++iRow)
    {
    ...
    this->_aMinDistAcc.get()[iBlock] = sumOfDist2;
...
bool TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::updateMinDist(size_t iFirstOfNewCandidates, size_t nNewCandidates)
{
    ...
    const size_t gemmDataSize = this->_nRowsInBlock*nCandidates;
    daal::tls<TlsPPData_t *> tlsData([=]()-> TlsPPData_t*
    {
        ...
        //allocate memory for MKL result
        if(pData)
        {
    ...
    tlsData.reduce([=, &newOverallError](TlsPPData_t* ptr)-> void
    {
        ...
        for(size_t j = 0; j < nCandidates; ++j)
            _aCandidateRating.get()[j] += ptr->aCandidateRating[j];
...
bool TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::getCandidates(HomogenNumericTableCPUPtr& pCandidates)
{
    ...
    pCandidates = HomogenNumericTableCPUPtr(new HomogenNumericTableCPU<algorithmFPType, cpu>(this->_data.dim, maxNumberOfCandidates));
    {
        ...
        //no ratings yet
        daal::services::internal::service_memset<size_t, cpu>(_aCandidateRating.get(), 0, maxNumberOfCandidates);
        ...
        daal::services::internal::service_memset<size_t, cpu>(_aNearestCandidateIdx.get(), 0, this->_data.nRows);
...
bool TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::run()
{
    ...
    TSmartPtr<algorithmFPType, cpu> aWeight(nCandidates);
