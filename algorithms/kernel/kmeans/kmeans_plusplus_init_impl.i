...
public:
    ...
    Status updateMinDistInBlock(algorithmFPType* const minDistAccTrials, size_t nBlock, size_t iBlock,
        size_t nTrials, size_t iBestTrial, const algorithmFPType* aWeights, const algorithmFPType* const pLastAddedCenter,
        algorithmFPType* const aMinDist)
    {
        ...
        for (size_t iTrials = 0u; iTrials < nTrials; iTrials++)
        {
            ...
            minDistAccTrials[iTrials*nBlock + iBlock] = updateMinDistForITrials(pDistSq, iTrials, nRowsToProcess, pData,
                pAddedCenter, weights, pDistSqBest);

        }
        minDistAccTrials[iBestTrial*nBlock + iBlock] = updateMinDistForITrials(pDistSqBest, iBestTrial, nRowsToProcess, pData,
                pLastAddedCenter, weights, pDistSqBest);
        ...
        const algorithmFPType* const aWeights, const algorithmFPType* const pDistSqBest)
    {
        ...
        for(size_t iRow = 0u; iRow < nRowsToProcess; iRow++)
        {
            ...
            {
                dist2 += (pData[iRow*dim + i] - pLastAddedCenter[i])*
                         (pData[iRow*dim + i] - pLastAddedCenter[i]);
            }
            if(aWeights)
            {
                dist2 *= aWeights[iRow];
...
public:
        ...
        algorithmFPType* const aMinDist)
    {
        ...
        for (size_t iTrials = 0u; iTrials < nTrials; iTrials++)
        {
            ...
            minDistAccTrials[iTrials*nBlock + iBlock] = updateMinDistForITrials(pDistSq, iTrials, nRowsToProcess, pData,
                colIdx, rowIdx, pAddedCenter, aWeights, pDistSqBest);

        }
        minDistAccTrials[iBestTrial*nBlock + iBlock] = updateMinDistForITrials(pDistSqBest, iBestTrial, nRowsToProcess, pData,
                colIdx, rowIdx, pLastAddedCenter, aWeights, pDistSqBest);

        return Status();
    }

    algorithmFPType updateMinDistForITrials(algorithmFPType* const pDistSq, size_t iTrials,
        size_t nRowsToProcess, const algorithmFPType* const pData, const size_t* const colIdx,
        const size_t* const rowIdx, const algorithmFPType* const pLastAddedCenter,
        const algorithmFPType* const aWeights, const algorithmFPType* const pDistSqBest)
    {
        algorithmFPType sumOfDist2 = algorithmFPType(0);
        size_t csrCursor = 0u;
        for(size_t iRow = 0u; iRow < nRowsToProcess; iRow++)
        {
            ...
            {
                dist2 += (pData[csrCursor] - pLastAddedCenter[colIdx[csrCursor] - 1])*
                         (pData[csrCursor] - pLastAddedCenter[colIdx[csrCursor] - 1]);
            }
            if(aWeights)
            {
                dist2 *= aWeights[iRow];
...
{
public:
    TaskPlusPlusBatchBase(NumericTable *ntData, NumericTable *ntClusters, size_t numClusters,
        size_t nTrials, engines::BatchBase &engine) :
        _data(ntData), _ntClusters(ntClusters),
        _nClusters(numClusters), _nTrials(nTrials), _engine(engine), _trialBest(0u)
...
protected:
    ...
    algorithmFPType overallError() const
    {
        return _overallError[_trialBest];
    }
...
public:
    ...
    TaskPlusPlusBatch(NumericTable *ntData, const algorithmFPType* aWeight,
        NumericTable *ntClusters, size_t numClusters, size_t nTrials, engines::BatchBase &engine) :
...
public:
    typedef TaskPlusPlusBatchBase<algorithmFPType, cpu, DataHelper> super;
    TaskParallelPlusBatch(NumericTable *ntData, NumericTable *ntClusters,
...
Status TaskPlusPlusBatch<algorithmFPType, cpu, DataHelper>::run()
{
    ...
    daal::services::internal::service_memset<algorithmFPType, cpu>(this->_aMinDist.get(),
        daal::services::internal::MaxVal<algorithmFPType>::get(), this->_data.nRows*this->_nTrials);
...
Status TaskPlusPlusBatchBase<algorithmFPType, cpu, DataHelper>::updateMinDist(const algorithmFPType* aWeights, size_t nTrials)
{
    ...
    {
        safeStat |= _data.updateMinDistInBlock(
            _aMinDistAcc.get(),
            _nBlocks,
            iBlock,
            nTrials,
...
void TaskPlusPlusBatch<algorithmFPType, cpu, DataHelper>::calcCenter(size_t iCluster)
{
    ...
    for (size_t iTrials = 0u; iTrials < this->_nTrials; iTrials++)
    {
        ...
        const size_t iRow = this->_nTrials == 1 ? this->samplePoint(iCluster) :
            this->findSample(this->overallError()*probability);
...
Status TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::updateMinDist(size_t iFirstOfNewCandidates, size_t nNewCandidates)
{
    TaskParallelPlusUpdateDist<algorithmFPType, cpu, DataHelper> impl(this->_nBlocks,
        ...
        this->_overallError[this->_trialBest],
        this->_data,
...
Status TaskParallelPlusBatch<algorithmFPType, cpu, DataHelper>::run()
{
    ...
    TaskPlusPlusBatch<algorithmFPType, cpu, DataHelperDense<algorithmFPType, cpu> > task(
        pCandidates.get(), aWeight.get(), this->_ntClusters, this->_nClusters, 1, this->_engine);
    return task.run();
}
