...
template<> services::Status kmeansInitTask<DAAL_FPTYPE, avx512_mic>(int dim, int clNum, DAAL_FPTYPE * centroids, void *& res)
{
    res = nullptr;
    typedef struct task_t<DAAL_FPTYPE, avx512_mic> TTask;
...
template<> services::Status addNTToTaskThreadedDense<DAAL_FPTYPE, avx512_mic, 0>
(void * task_id, const NumericTable * ntData, DAAL_FPTYPE *catCoef, NumericTable * ntAssign )
{
    ...
    daal::threader_for(nBlocks, nBlocks, [=, &safeStat](int k)
    {
        ...
        ReadRows<DAAL_FPTYPE, avx512_mic> mtData(*const_cast<NumericTable *>(ntData), k*blockSizeDeafult, blockSize);
        DAAL_CHECK_BLOCK_STATUS_THR(mtData);
        const DAAL_FPTYPE *data = mtData.get();
...
template<> services::Status getNTAssignmentsThreaded <lloydDense, DAAL_FPTYPE, avx512_mic>
(void * task_id, const NumericTable * ntData, const NumericTable * ntAssign, DAAL_FPTYPE *catCoef )
{
    ...
    daal::threader_for(nBlocks, nBlocks, [=, &safeStat](int k)
    {
        ...
        ReadRows<DAAL_FPTYPE, avx512_mic> mtData(*const_cast<NumericTable*>(ntData), k*blockSizeDeafult, blockSize);
        DAAL_CHECK_BLOCK_STATUS_THR(mtData);
        WriteOnlyRows<int, avx512_mic> mtAssign(*const_cast<NumericTable*>(ntAssign), k*blockSizeDeafult, blockSize);
