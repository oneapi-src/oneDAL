        ...
        void update(const SplitDataType& split, int iFeature, int* bestSplitIdx, const int* aIdx, size_t n)
        {
            if(_mt)
            {
                ...
                if(updateImpl(split, -1, iFeature))
                    services::internal::tmemcpy<int, cpu>(bestSplitIdx, aIdx, n);
                _mt->unlock();
            }
            else
            {
...
protected:
    ...
    NodeType::Base* buildRoot(size_t iTree, GlobalStorages<algorithmFPType, BinIndexType, cpu>& GH_SUMS_BUF)
    {
        ...
        SharedDataForTree<algorithmFPType, RowIndexType, BinIndexType, cpu> data(_ctx, bestSplitIdxBuf(), const_cast<int*>(_aSample.get()), _memHelper, this->_iTree, _tree, _mtAlloc);
...
services::Status TreeBuilder<algorithmFPType, RowIndexType, BinIndexType, cpu>::run(gbt::internal::GbtDecisionTree*& pRes,
    HomogenNumericTable<double>*& pTblImp, HomogenNumericTable<int>*& pTblSmplCnt, size_t iTree, GlobalStorages<algorithmFPType, BinIndexType,  cpu>& GH_SUMS_BUF)
{
    _tree.destroy();
    ...
    services::Status status = gbt::internal::ModelImpl::treeToTable(_tree, &pRes, &pTblImp, &pTblSmplCnt);
