...
{
    static void run(algorithmFPType* aGHSumFP, const IndexType* indexedFeature, const IndexType* aIdx, algorithmFPType* pgh, size_t nFeatures, size_t iStart, size_t iEnd, size_t nRows)
    {
        ...
        for(; i < iEndWithPrefetch; ++i)
        {
            ...
            const IndexType* ptr = indexedFeature + aIdx[i+prefetchOffset]*nFeatures;
            ...
            for(IndexType j = 0; j < nFeatures; j++)
            {
                ...
                aGHSumFP[featIdx[j] + 2] += algorithmFPType(1);
            }
        }
...
struct MergeGHSums
{
    ...
    {
        static void run(float* aGHSumFP, const IndexType* indexedFeature, const IndexType* aIdx, float* pgh, size_t nFeatures, size_t iStart, size_t iEnd, size_t nRows)
        {
            const size_t cacheLineSize = 64;  // bytes
            const size_t prefetchOffset = 10; // heuristic, prefetch on 10 rows ahead
            const size_t elementsInCacheLine = cacheLineSize / sizeof(IndexType);
    ...
    {
        static void run(double* aGHSumFP, const IndexType* indexedFeature, const IndexType* aIdx, double* pgh, size_t nFeatures, size_t iStart, size_t iEnd, size_t nRows)
        {
            const size_t cacheLineSize = 64;  // bytes
            const size_t prefetchOffset = 10; // heuristic, prefetch on 10 rows ahead
            const size_t elementsInCacheLine = cacheLineSize / sizeof(IndexType);
