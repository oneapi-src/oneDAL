...
template<typename algorithmFPType, typename RowIndexType, typename BinIndexType, typename ImpurityType, typename GHSumType, typename SplitType, typename ResultType, CpuType cpu>
...
public:
    static void find(
            ...
            bool featureUnordered, SharedDataForTree<algorithmFPType, RowIndexType, BinIndexType, cpu>& data, size_t iFeature)
    {
        if(featureUnordered)
    ...
    static void findOrdered(
            ...
            SplitType& split,
            const ResultType& res,
            DAAL_INT& idxFeatureBestSplit, SharedDataForTree<algorithmFPType, RowIndexType, BinIndexType, cpu>& data, size_t iFeature)
    {
...
public:
    static void compute(
            const size_t iStart,
            const size_t n,
            const RowIndexType* const indexedFeature,
            const RowIndexType* aIdx,
            const RowIndexType* aSampleToSourceRow,
...
{
    static void run(algorithmFPType* aGHSumFP, const BinIndexType* indexedFeature, const RowIndexType* aIdx, algorithmFPType* pgh, size_t nFeatures, size_t iStart, size_t iEnd, size_t nRows, size_t* UniquesArr)
    {
        ...
        for(; i < iEndWithPrefetch; ++i)
        {
            ...
            for(RowIndexType j = 0; j < nCacheLinesToPrefetchOneRow; j++)
                DAAL_PREFETCH_READ_T0(ptr + elementsInCacheLine*j);
...
struct MergeGHSums
{
    ...
    {
        static void run(float* aGHSumFP, const BinIndexType* indexedFeature, const RowIndexType* aIdx, float* pgh, size_t nFeatures, size_t iStart, size_t iEnd, size_t nRows, size_t* UniquesArr)
        {
            const size_t cacheLineSize = 64;  // bytes
            const size_t prefetchOffset = 10; // heuristic, prefetch on 10 rows ahead
            ...
            for(; i < iEndWithPrefetch; ++i)
            {
                DAAL_PREFETCH_READ_T0(pgh + 2*aIdx[i+prefetchOffset]);
                const BinIndexType* ptr = indexedFeature + aIdx[i+prefetchOffset]*nFeatures;
                for(IndexType j = 0; j < nCacheLinesToPrefetchOneRow; j++)
                    DAAL_PREFETCH_READ_T0(ptr + elementsInCacheLine*j);
    ...
    {
        static void run(double* aGHSumFP, const BinIndexType* indexedFeature, const RowIndexType* aIdx, double* pgh, size_t nFeatures, size_t iStart, size_t iEnd, size_t nRows, size_t* UniquesArr)
        {
            const size_t cacheLineSize = 64;  // bytes
            const size_t prefetchOffset = 10; // heuristic, prefetch on 10 rows ahead
            const size_t elementsInCacheLine = cacheLineSize / sizeof(IndexType);
