...
Status TrainingKernelBatch<algorithmFPType, method, cpu>::initialize(Tensor* data, Model* nnModel, const KeyValueDataCollectionPtr &groundTruthCollectionPtr,
                                                const neural_networks::training::Parameter *parameter)
{
    Status s;
    DAAL_NN_CHECK_STATUS(s, initializeBase(data, nnModel, parameter, groundTruthCollectionPtr))
    ...
    DAAL_NN_CHECK_MALLOC(learnableLayerIndices.get() && learnableLayerIndices->isValid())
    ...
    DAAL_NN_CHECK_MALLOC(solvers.get())

    for (size_t i = 0; i < nSolvers; i++)
    {
        DAAL_NN_CHECK_STATUS(s, solvers[i].init(parameter->optimizationSolver))
    }
    return s;
}

template<typename algorithmFPType, Method method, CpuType cpu>
Status TrainingKernelBatch<algorithmFPType, method, cpu>::compute(Tensor* data, Model* nnModel, const KeyValueDataCollectionPtr &groundTruthCollectionPtr)
{
    Status s;
    const size_t nSolvers = solvers.size();
    for(size_t i = 0; i < nSolvers; i++)
    {
        DAAL_NN_CHECK_STATUS(s, solvers[i].setSolverOptionalResult(nnModel->getSolverOptionalArgument(i)))
    }

    DAAL_NN_CHECK_STATUS(s, computeBase(data, nnModel, groundTruthCollectionPtr))
...
Status TrainingKernelBatch<algorithmFPType, method, cpu>::updateWeights(Model& nnModel)
{
    ...
    if (oneTableForAllWeights)
    {
        ...
        DAAL_NN_CHECK_STATUS(s, solver.updateWeightsAndBiases(
...
{
    Status s;
    DAAL_CHECK_STATUS(s, super::reset())
    solvers.reset(0);
 ...
 */
template<typename algorithmFPType, Method method, CpuType cpu>
Status TrainingKernelDistributed<algorithmFPType, method, cpu>::initialize(Tensor* data, Model* nnModel, const KeyValueDataCollectionPtr &groundTruthCollectionPtr,
                                                                         const neural_networks::training::Parameter *parameter)
{
    return initializeBase(data, nnModel, parameter, groundTruthCollectionPtr);
}

template<typename algorithmFPType, Method method, CpuType cpu>
Status TrainingKernelDistributed<algorithmFPType, method, cpu>::compute(Tensor* data, Model* nnModel, const KeyValueDataCollectionPtr &groundTruthCollectionPtr,
                                                                      PartialResult *partialResult, const neural_networks::training::Parameter *parameter)
{
    ...
    DAAL_NN_CHECK_BLOCK_STATUS(batchSizeBlock)
...
Status TrainingKernelBase<algorithmFPType, cpu>::initializeBase(Tensor *data, Model *nnModel, const neural_networks::training::Parameter *parameter,
                                                              const KeyValueDataCollectionPtr &groundTruthCollectionPtr)
{
    ...
    DAAL_NN_CHECK_MALLOC(lastLayersIndices.get() && lastLayersIndices->isValid())
    ...
    DAAL_NN_CHECK_MALLOC(sample.get())
    ...
    DAAL_NN_CHECK_MALLOC(groundTruthTensors.get())
    ...
    DAAL_NN_CHECK_MALLOC(sampleGroundTruthCollection.get())
    ...
    for (size_t i = 0; i < nLastLayers; i++)
    {
        ...
        DAAL_NN_CHECK_MALLOC(sampleGroundTruth.get())
...
template<typename algorithmFPType, CpuType cpu>
Status TrainingKernelBase<algorithmFPType, cpu>::computeBase(Tensor *data, Model *nnModel, const KeyValueDataCollectionPtr &groundTruthCollectionPtr)
{
    ForwardLayersPtr forwardLayers = nnModel->getForwardLayers();
    ...
    for(size_t i = 0; i < maxIterations * batchSizeParam; i += batchSizeParam)
    {
        ...
        DAAL_NN_CHECK_BLOCK_STATUS(dataSubtensor)
        ...
        for (size_t j = 0; j < nLastLayers; j++)
        {
            ...
            DAAL_NN_CHECK_BLOCK_STATUS(groundTruthTensors[j])
        ...
        for(size_t layerId = 0; layerId < nLayers; layerId++)
        {
            ...
            DAAL_NN_CHECK_STATUS(s, processLayerErrors(layerId, forwardLayer->computeNoThrow()))
        }
        ...
        for(int layerId = nLayers - 1; layerId >= 0; layerId--)
        {
            ...
            DAAL_NN_CHECK_STATUS(s, processLayerErrors(layerId, backwardLayer->computeNoThrow()))
        }

        /* Update weights and biases of the network */
        DAAL_NN_CHECK_STATUS(s, updateWeights(*nnModel))
    }
    return s;
}

template<typename algorithmFPType, CpuType cpu>
Status TrainingKernelBase<algorithmFPType, cpu>::reset()
{
