...
{
public:
    virtual NumericTablePtr copyToTable() const DAAL_C11_OVERRIDE { return _wbTable; }
    ...
    virtual NumericTablePtr copyToTable(size_t idx) const DAAL_C11_OVERRIDE
    {
        ...
        SharedPtr<HomogenNumericTable<modelFPType> > resultTable = allocateWeightsAndBiasesTable<modelFPType>(nRows);
        if (!resultTable.get())
        {
            return NumericTablePtr();
        }
    ...
    {
        if (table.get() == _wbTable.get()) return Status();
    ...
    {
        if (table.get() == _wbTable.get() || idx > _nLayers || ((_wSize[idx] + _bSize[idx]) == 0))
        {
            return Status();
        }
...
public:
    virtual NumericTablePtr copyToTable() const DAAL_C11_OVERRIDE { return tensorsToTable(_wbTensors, 0, 2 * _nLayers); }

    virtual NumericTablePtr copyToTable(size_t idx) const DAAL_C11_OVERRIDE { return tensorsToTable(_wbTensors, 2 * idx, 2); }

    virtual Status copyFromTable(const NumericTablePtr & table) DAAL_C11_OVERRIDE { return tableToTensors(table, _wbTensors, 0, 2 * _nLayers); }
...
protected:
    HomogenNumericTablePtr tensorsToTable(const Collection<TensorPtr> & tensors, size_t startTensor, size_t nTensors) const
    {
        size_t tableSize = 0;
        ...
        for (size_t i = startTensor; i < startTensor + nTensors; i++)
        {
            ...
            if (!subtensor.status()) return HomogenNumericTablePtr();
            const modelFPType * tensorArray = subtensor.get();

            daal::services::internal::daal_memcpy_s(tableArray + tableOffset, tensorSize * sizeof(modelFPType), tensorArray,
                                                    tensorSize * sizeof(modelFPType));
            tableOffset += tensorSize;
        }
        return table;
    }

    Status tableToTensors(const NumericTablePtr & table, Collection<TensorPtr> & tensors, size_t startTensor, size_t nTensors)
    {
        ReadRows<modelFPType, sse2> tableRows(table.get(), 0, table->getNumberOfRows());
        DAAL_CHECK_BLOCK_STATUS(tableRows)
        ...
        for (size_t i = startTensor; i < startTensor + nTensors; i++)
        {
            ...
            daal::services::internal::daal_memcpy_s(tensorArray, tensorSize * sizeof(modelFPType), tableArray + tableOffset,
                                                    tensorSize * sizeof(modelFPType));
            tableOffset += tensorSize;
        }
        return Status();
...
public:
    static ThisTypePtr create(neural_networks::ForwardLayersPtr & forwardLayers, Status * pSt = nullptr)
    {
        Status defaultSt;
...
public:
    static ThisTypePtr create(neural_networks::BackwardLayersPtr & backwardLayers, Status * pSt = nullptr)
    {
        Status defaultSt;
        Status & s = (pSt ? *pSt : defaultSt);
...
public:
    ...
    static ThisTypePtr create(neural_networks::ForwardLayersPtr & forwardLayers, SharedPtr<HomogenNumericTable<modelFPType> > & table,
                              Status * pSt = nullptr)
    {
        Status defaultSt;
        Status & s = (pSt ? *pSt : defaultSt);
...
protected:
    NumericTableWeightsAndBiases(neural_networks::ForwardLayersPtr & forwardLayers, Status & s)
    {
    ...
    NumericTableWeightsAndBiases(neural_networks::ForwardLayersPtr & forwardLayers, SharedPtr<HomogenNumericTable<modelFPType> > & table, Status & s)
    {
        computeWeightsAndBiasesSizes(forwardLayers);
        _wbTable = table;
        s        = setWeightsAndBiasesFromTable(forwardLayers);
...
public:
    static ThisTypePtr create(neural_networks::ForwardLayersPtr & forwardLayers, neural_networks::BackwardLayersPtr & backwardLayers,
                              Status * pSt = nullptr)
    {
        Status defaultSt;
        Status & s = (pSt ? *pSt : defaultSt);
...
protected:
    NumericTableWeightsAndBiasesDerivatives(neural_networks::ForwardLayersPtr & forwardLayers, neural_networks::BackwardLayersPtr & backwardLayers,
                                            Status & s)
    {
        computeWeightsAndBiasesSizes(forwardLayers);
        _wbTable = allocateWeightsAndBiasesTable<modelFPType>(_wbSize);
