...
services::Status Input::check(const daal::algorithms::Parameter * parameter, int method) const
{
    ...
    unsigned int one     = 1;
    DAAL_CHECK_EX(0 < index0 && index0 < nDim && 0 < index1 && index1 < nDim && index0 != index1, services::ErrorIncorrectParameter,
...
services::Status Result::check(const daal::algorithms::Input * input, const daal::algorithms::Parameter * parameter, int method) const
{
    ...
    DAAL_CHECK_EX(get(layers::forward::value)->getDimensions().size() == 2, services::ErrorIncorrectNumberOfDimensionsInTensor,
                  services::ArgumentName, valueStr());
    ...
    if (!param->predictionStage)
    {
        ...
        if (layerData->size() != 1)
            if (!layerData)
            {
                return services::Status(services::ErrorIncorrectSizeOfLayerData);
            }
    }
    return s;
}
