...
services::Status BasePoolingKernel<algorithmFPType, cpu>::compute(const Tensor & dataTensor, Tensor & valueTensor, Tensor * selectedPosTensor,
                                                                  const spatial_pooling2d::Parameter & spatialParameter)
{
    ...
    for (size_t slice = 0; slice < dims[0]; slice++)
    {
        ...
        SharedPtr<HomogenTensor<algorithmFPType> > dataSliceTensor =
            HomogenTensor<algorithmFPType>::create(dataSliceDims, const_cast<algorithmFPType *>(dataSlice), &s);
...
Status PoolingKernel<algorithmFPType, method, cpu>::computePooling(const pooling2d::Parameter & poolingPar,
                                                                   const spatial_pooling2d::Parameter & spatialParameter, const Tensor & dataTensor,
                                                                   Tensor & poolingValueTensor, Tensor * poolingSelectedPosTensor)
{
    return Status();
}
...
Status PoolingKernel<algorithmFPType, spatial_pooling2d::internal::maximum, cpu>::computePooling(
    const pooling2d::Parameter & poolingPar, const spatial_pooling2d::Parameter & spatialParameter, const Tensor & dataTensor,
    Tensor & poolingValueTensor, Tensor * poolingSelectedPosTensor)
{
