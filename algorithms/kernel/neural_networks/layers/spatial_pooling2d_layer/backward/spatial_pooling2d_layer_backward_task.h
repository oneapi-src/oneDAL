...
{
public:
    BasePoolingTask(const Tensor & _inputGradientTensor, Tensor & _gradientTensor, const spatial_pooling2d::Parameter & _spatialParameter)
    ...
    virtual Status getSlice(size_t slice) { return getInputGradientSlice(slice); }
    ...
    Status prepareCommonPoolingTensors(size_t level, size_t offset)
    {
        ...
        poolingInputGradientTensor =
            HomogenTensor<algorithmFPType>::create(inputGradientSliceDims, const_cast<algorithmFPType *>(inputGradientData), &s);
        DAAL_CHECK_STATUS_VAR(s);
        poolingGradientTensor = HomogenTensor<algorithmFPType>::create(gradientSliceDims, poolingGradientArray.get(), &s);
        return s;
    }

    virtual Status preparePoolingTensors(size_t level, size_t offset) { return prepareCommonPoolingTensors(level, offset); }
...
{
public:
    PoolingTask(const Tensor & _inputGradientTensor, Tensor & _gradientTensor, const Tensor & _selectedPosTensor,
                const spatial_pooling2d::Parameter & parameter)
        : BasePoolingTask<algorithmFPType, cpu>(_inputGradientTensor, _gradientTensor, parameter), selectedPosTensor(_selectedPosTensor)
    {}
    ...
    Status preparePoolingTensors(size_t level, size_t offset) DAAL_C11_OVERRIDE
    {
        ...
        poolingSelectedPosTensor =
            HomogenTensor<algorithmFPType>::create(this->inputGradientSliceDims, const_cast<algorithmFPType *>(selectedPosData), &s);
        return s;
    }
...
{
public:
    PoolingTask(const Tensor & _inputGradientTensor, Tensor & _gradientTensor, const spatial_pooling2d::Parameter & parameter)
