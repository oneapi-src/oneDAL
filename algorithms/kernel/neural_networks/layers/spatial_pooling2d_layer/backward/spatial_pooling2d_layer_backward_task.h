...
public:
    ...
    void prepareCommonPoolingTensors(size_t level, size_t offset)
    {
        ...
        poolingInputGradientTensor = TensorPtr(new HomogenTensor<algorithmFPType>(inputGradientSliceDims, const_cast<algorithmFPType *>(inputGradientData)));

        onePoolingGradientTensor = TensorPtr(new HomogenTensor<algorithmFPType>(oneGradientDims, onePoolingGradientArray.get()));
    ...
    TensorPtr onePoolingGradientTensor;
...
public:
    ...
    void preparePoolingTensors(size_t level, size_t offset) DAAL_C11_OVERRIDE
    {
        ...
        poolingSelectedPosTensor = TensorPtr(new HomogenTensor<algorithmFPType>(inputGradientSliceDims, selectedPosData));
    }
