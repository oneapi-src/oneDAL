...
Status DAAL_EXPORT PoolingKernel<algorithmFPType, spatial_pooling2d::internal::maximum, cpu>::compute(
    const Tensor & inputGradientTensor, Tensor & gradientTensor, const Tensor & selectedPosTensor,
...
{
    PoolingTask<algorithmFPType, spatial_pooling2d::internal::stochastic, cpu> task(inputGradientTensor, gradientTensor, selectedPosTensor,
                                                                                    parameter);
    return task.proceed();
}
...
services::Status DAAL_EXPORT BasePoolingTask<algorithmFPType, cpu>::proceed()
{
    ...
    for (size_t slice = 0; slice < nSlices; slice++)
    {
        ...
        for (size_t level = 0, pow2 = 1; level < L; pow2 *= 2, level++)
        {
            ...
            accumulatedFlattenOffset +=
                gradientTensor.getDimensionSize(6 - spatialParameter.indices.size[0] - spatialParameter.indices.size[1]) * pow2 * pow2;
        }
    }
    return s;
}
...
Status DAAL_EXPORT PoolingTask<algorithmFPType, method, cpu>::computePooling()
{
    ...
    using MaxPoolingBackwardKernel =
        layers::maximum_pooling2d::backward::internal::PoolingKernel<algorithmFPType, layers::maximum_pooling2d::defaultDense, cpu>;
    ...
    status |= maxPoolKernel.compute(*(this->poolingInputGradientTensor), *(this->poolingSelectedPosTensor), *(this->poolingGradientTensor),
...
Status DAAL_EXPORT PoolingTask<algorithmFPType, spatial_pooling2d::internal::average, cpu>::computePooling()
{
    ...
    using AvePoolingBackwardKernel =
        layers::average_pooling2d::backward::internal::PoolingKernel<algorithmFPType, layers::average_pooling2d::defaultDense, cpu>;
    ...
    Tensor * dataTensor = nullptr;
    status |= avePoolKernel.compute(*(this->poolingInputGradientTensor), this->poolingParameter, *(this->poolingGradientTensor), dataTensor);
    return status;
}
