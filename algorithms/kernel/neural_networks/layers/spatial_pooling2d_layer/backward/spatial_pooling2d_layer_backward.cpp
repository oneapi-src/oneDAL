...
services::Status Result::check(const daal::algorithms::Input * input, const daal::algorithms::Parameter * parameter, int method) const
{
    ...
    services::Collection<size_t> valueDims =
        spatial_pooling2d::forward::Result::computeValueDimensions(get(layers::backward::gradient)->getDimensions(), param);
    ...
    DAAL_CHECK_EX(index0 > 0 && index0 < gradientNDims && index1 > 0 && index1 < gradientNDims && index0 != index1, services::ErrorIncorrectParameter,
                  services::ParameterName, indicesStr());
    if (gradientDims[index0] > gradientDims[index1])
    {
        DAAL_CHECK_EX(one << (pyramidHeight - 1) <= gradientDims[index0], services::ErrorIncorrectParameter, services::ParameterName,
                      pyramidHeightStr());
    }
    if (gradientDims[index0] <= gradientDims[index1])
    {
        DAAL_CHECK_EX(one << (pyramidHeight - 1) <= gradientDims[index1], services::ErrorIncorrectParameter, services::ParameterName,
                      pyramidHeightStr());
    }
    return s;
}
