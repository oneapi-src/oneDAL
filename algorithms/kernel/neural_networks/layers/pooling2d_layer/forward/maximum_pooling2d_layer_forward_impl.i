    ...
    inline void compute_max(int n, int offset, const float * parg, float * pmax, int * pmaxidx)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                mmaxidx       = _mm_castps_si128(__mm_blendv_ps(_mm_castsi128_ps(mmaxidx), _mm_castsi128_ps(mnewidx), _mm_castsi128_ps(mmask)));
    ...
    inline void compute_max(int n, int offset, const double * parg, double * pmax, int * pmaxidx)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                mmaxidx       = _mm_castpd_si128(__mm_blendv_pd(_mm_castsi128_pd(mmaxidx), _mm_castsi128_pd(mnewidx), _mm_castsi128_pd(mmask)));
                                                                      ...
                                                                      const Parameter & parameter)
{
    ...
    if (dataMklTensor != NULL && (selectedPosMklTensorDouble != NULL || selectedPosMklTensorFloat != NULL))
    {
        ...
        if (maxPoolPrim == NULL)
        {
            ...
            err = dnn::xPoolingCreateForward(&maxPoolPrim, dnnAlgorithmPoolingMax, inputLayout, parameter.kernelSizes.size, parameter.strides.size,
        ...
        {
            err = dnn::xLayoutCreateFromPrimitive(&resultLayout, maxPoolPrim, dnnResourceDst);
            ON_ERR(err);
        ...
        else
        {
            ...
            LayoutConvertor<algorithmFPType, cpu> cvFromInnerOutput(&maxPoolRes[dnnResourceDst], resultLayout, false, &valueArray, ltUserOutput.get(),
                                                                    true);
...
void PoolingKernel<algorithmFPType, method, cpu>::defaultInnerLoop(const pooling2d::internal::Parameter & par, DAAL_INT i, DAAL_INT f, DAAL_INT k,
                                                                   DAAL_INT s, DAAL_INT j, const algorithmFPType * data, algorithmFPType * valuePtr,
                                                                   int * selectedPosPtr)
{
        ...
        for (size_t fo = 0, f = 0; fo < par.firstOutSize; fo++, ifs += par.firstStride, f += par.firstStride)
        {
            ...
            for (int fk = 0; fk < fUpper; fk++)
            {
                ...
                for (size_t ss_index = 0, so = 0; so < par.secondOutSize; so++, ss_index += par.secondStride)
                {
                    ...
                    opt.compute_max(sUpper, posOffset, &dataBase[ss_index], &valueBase[so], &posBase[so]);
