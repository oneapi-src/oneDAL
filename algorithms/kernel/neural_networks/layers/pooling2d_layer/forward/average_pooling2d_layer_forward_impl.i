...
template <typename algorithmFPType, Method method, CpuType cpu>
services::Status PoolingKernel<algorithmFPType, method, cpu>::compute(const Tensor & dataTensor, const average_pooling2d::Parameter & parameter,
                                                                      Tensor & valueTensor)
{
    ...
    if (dataMklTensor != NULL)
    {
        ...
        if (avePoolPrim == NULL)
        {
            ...
            err = dnn::xPoolingCreateForward(&avePoolPrim, dnnAlgorithmPoolingAvg, inputLayout, parameter.kernelSizes.size, parameter.strides.size,
        ...
        {
            err = dnn::xLayoutCreateFromPrimitive(&resultLayout, avePoolPrim, dnnResourceDst);
            ON_ERR(err);
        ...
        else
        {
            ...
            LayoutConvertor<algorithmFPType, cpu> cvFromInnerOutput(&avePoolRes[dnnResourceDst], resultLayout, false, &valueArray, ltUserOutput.get(),
                                                                    true);
