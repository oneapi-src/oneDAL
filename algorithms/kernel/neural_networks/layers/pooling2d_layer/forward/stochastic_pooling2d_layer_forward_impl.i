...
services::Status PoolingKernel<algorithmFPType, method, cpu>::compute(const Tensor & dataTensor, Tensor & valueTensor, Tensor * selectedPosTensor,
                                                                      const Parameter & parameter, engines::BatchBase & engine)
{
    ...
    ReadSubtensor<algorithmFPType, cpu, Tensor> dataSubtensor(const_cast<Tensor &>(dataTensor), 0, 0, 0, targetInLayout.getDimensions()[0],
                                                              targetInLayout);
        ...
        for (DAAL_INT f = -par.firstPadding, fo = 0; fo < par.firstOutSize; f += par.firstStride, fo++)
        {
            ...
            */
            for (DAAL_INT s = -par.secondPadding, so = 0; so < par.secondOutSize; s += par.secondStride, so++)
            {
                ...
                if (trainingStage)
                {
                    getMultivariateRandomDataValue(dataSlice, f, s, kernelWeights, nFlatten, par, valueSlice[valueIndex],
                                                   selectedPosSlice[valueIndex]);
                }
                else
                {
...
template <typename algorithmFPType, Method method, CpuType cpu>
void PoolingKernel<algorithmFPType, method, cpu>::computeWeightedAverage(const algorithmFPType * dataSlice, DAAL_INT f, DAAL_INT s,
                                                                         algorithmFPType * kernelWeights, pooling2d::internal::Parameter & par,
...
template <typename algorithmFPType, Method method, CpuType cpu>
void PoolingKernel<algorithmFPType, method, cpu>::getMultivariateRandomDataValue(const algorithmFPType * dataSlice, DAAL_INT f, DAAL_INT s,
                                                                                 algorithmFPType * weights, size_t nWeights,
...
{
    return distributions::uniform::internal::UniformKernelDefault<int, cpu>::compute(0, services::internal::MaxVal<int>::get(), engine, nUniformRand,
                                                                                     uniformRand);
}
