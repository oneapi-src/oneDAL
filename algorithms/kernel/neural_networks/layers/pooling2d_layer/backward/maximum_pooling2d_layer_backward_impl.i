                                                                         ...
                                                                         const services::Collection<size_t> & outDims)
{
    ...
    ltUserInput = xDnnLayout(dimension, inputSize, inputStrides);
    ON_ERR(ltUserInput.err);
    ltUserOutput = xDnnLayout(dimension, outputSize, outputStrides);
    ON_ERR(ltUserOutput.err);
    return Status();
}
                                                                      ...
                                                                      const pooling2d::Parameter & parameter)
{
    ...
    MklTensor<double> * selectedPosMklTensorDouble =
        dynamic_cast<MklTensor<double> *>(const_cast<Tensor *>(const_cast<Tensor *>(&selectedPosTensor)));
    ...
    if (dataMklTensor != NULL && (selectedPosMklTensorDouble != NULL || selectedPosMklTensorFloat != NULL))
    {
        ...
        if (maxPoolPrim == NULL)
        {
            ...
            err = dnn::xPoolingCreateBackward(&maxPoolPrim, dnnAlgorithmPoolingMax, inputLayout, parameter.kernelSizes.size, parameter.strides.size,
        ...
        {
            err = dnn::xLayoutCreateFromPrimitive(&inputGradLayout, maxPoolPrim, dnnResourceDiffDst);
            ON_ERR(err);
        ...
        else
        {
            ...
            cvToInnerInputGrad.set(&inputGradArray, ltUserInput.get(), true, &maxPoolRes[dnnResourceDiffDst], inputGradLayout, false);
            ON_ERR(cvToInnerInputGrad.err);
            cvToInnerInputGrad.convert();
            ON_ERR(cvToInnerInputGrad.err);

            dnn::xLayoutDelete(inputGradLayout);
        }

        err = dnn::xLayoutCreateFromPrimitive(&workspaceLayout, maxPoolPrim, dnnResourceWorkspace);
        ON_ERR(err);
        ...
        {
            err = dnn::xLayoutCreateFromPrimitive(&resultLayout, maxPoolPrim, dnnResourceDiffSrc);
            ON_ERR(err);
            gradMklTensor->setDnnLayout(resultLayout);
            maxPoolRes[dnnResourceDiffSrc] = gradMklTensor->getDnnArray();

            err = dnn::xExecute(maxPoolPrim, (void **)maxPoolRes);
            ON_ERR(err);
        }
        else
        {
            ...
            LayoutConvertor<algorithmFPType, cpu> cvFromInnerOutput(&maxPoolRes[dnnResourceDiffSrc], resultLayout, false, &gradArray,
                                                                    ltUserOutput.get(), true);
    ...
    else
    {
        ...
        pooling2d::internal::Parameter par(parameter.indices.size, parameter.paddings.size, parameter.strides.size, parameter.kernelSizes.size,
...
template <typename algorithmFPType, Method method, CpuType cpu>
void PoolingKernel<algorithmFPType, method, cpu>::defaultInnerLoop(const pooling2d::internal::Parameter & par, DAAL_INT i, DAAL_INT f, DAAL_INT k,
                                                                   DAAL_INT s, const algorithmFPType * inputGradPtr, const int * selectedPosPtr,
...
template <typename algorithmFPType, Method method, CpuType cpu>
void PoolingKernel<algorithmFPType, method, cpu>::indicesLastZeroPaddingsCompute(const pooling2d::internal::Parameter & par,
...
template <typename algorithmFPType, Method method, CpuType cpu>
void PoolingKernel<algorithmFPType, method, cpu>::indicesFirstZeroPaddingsCompute(const pooling2d::internal::Parameter & par,
                                                                                  const algorithmFPType * inputGrad, const int * selectedPos,
                                                                                  algorithmFPType * grad)
{
    /*
