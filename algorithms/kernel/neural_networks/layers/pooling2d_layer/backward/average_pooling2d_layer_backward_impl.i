                                                                      ...
                                                                      Tensor & gradTensor, const Tensor * dataTensor)
{
    ...
    if (dataMklTensor != 0)
    {
        ...
        if (avePoolPrim == NULL)
        {
            ...
            err = dnn::xPoolingCreateBackward(&avePoolPrim, dnnAlgorithmPoolingAvg, inputLayout, parameter.kernelSizes.size, parameter.strides.size,
        ...
        {
            err = dnn::xLayoutCreateFromPrimitive(&inputGradLayout, avePoolPrim, dnnResourceDiffDst);
            ON_ERR(err);
        ...
        else
        {
            ...
            cvToInnerInputGrad.set(&inputGradArray, ltUserInput.get(), true, &avePoolRes[dnnResourceDiffDst], inputGradLayout, false);
        ...
        {
            err = dnn::xLayoutCreateFromPrimitive(&resultLayout, avePoolPrim, dnnResourceDiffSrc);
            ON_ERR(err);
            gradMklTensor->setDnnLayout(resultLayout);
            avePoolRes[dnnResourceDiffSrc] = gradMklTensor->getDnnArray();

            err = dnn::xExecute(avePoolPrim, (void **)avePoolRes);
            ON_ERR(err);
        }
        else
        {
            ...
            LayoutConvertor<algorithmFPType, cpu> cvFromInnerOutput(&avePoolRes[dnnResourceDiffSrc], resultLayout, false, &gradArray,
