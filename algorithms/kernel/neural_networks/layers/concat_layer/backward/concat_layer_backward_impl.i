...
Status ConcatKernel<algorithmFPType, method, cpu>::compute(Tensor * inputTensor, const NumericTable * forwardOutputTable,
                                                           const concat::Parameter * parameter, Tensor * resultTensors[])
{
...

    {
        ...
        if (canUseMklTensor)
        {
            ...
            if (splitPrim == NULL)
            {
                ...
                err = dnn::xSplitCreate(&splitPrim, nOutputs, inputLayout, dstChannelSize.get());
                ON_ERR(err);
            }
            ...
            for (size_t i = 0; i < nOutputs; i++)
            {
                ...
                err = dnn::xLayoutCreateFromPrimitive(&resultLayout, splitPrim, (dnnResourceType_t)(dnnResourceMultipleDst + i));
                ON_ERR(err);
            ...
            err = dnn::xExecute(splitPrim, (void **)splitRes);
            ON_ERR(err);
        }
        else
        {
