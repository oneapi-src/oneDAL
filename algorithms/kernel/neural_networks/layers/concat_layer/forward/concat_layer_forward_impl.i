                                                           ...
                                                           Tensor * resultTensor)
{
    ...
    if (canUseMklTensor)
    {
        ...
        if (concatPrim == NULL)
        {
            ...
            err = dnn::xConcatCreate(&concatPrim, nInputs, inputLayouts);
            ON_ERR(err);
        }
        ...
        err = dnn::xLayoutCreateFromPrimitive(&resultLayout, concatPrim, dnnResourceDst);
        ON_ERR(err);
        resultMklTensor->setDnnLayout(resultLayout);

        concatRes[dnnResourceDst] = resultMklTensor->getDnnArray();

        err = dnn::xExecute(concatPrim, (void **)concatRes);
        ON_ERR(err);
    }
    else
    {
