...
services::Status Input::check(const daal::algorithms::Parameter * parameter, int method) const
{
    ...
    DAAL_CHECK_EX(xDims[param->groupDimension] % param->nGroups == 0 || param->nKernels % param->nGroups == 0, ErrorIncorrectParameter, ParameterName,
                  nGroupsStr());
    ...
    for (size_t i = 0; i < xDims.size(); i++)
    {
        ...
        {
            gradDims.push_back(l3);
        }
        else if (i == param->indices.dims[1])
        {
            gradDims.push_back(l4);
        }
