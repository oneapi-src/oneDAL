...
services::Status LocallyConnected2dKernel<algorithmFPType, method, cpu>::compute(Tensor *inputTensor, Tensor *weightsTensor, Tensor *biasesTensor,
                                Tensor *valueTensor, const locallyconnected2d::Parameter *parameter)
{
    if(inputTensor == 0 || weightsTensor == 0 || biasesTensor == 0 || valueTensor == 0) return services::Status(services::ErrorNullTensor);
