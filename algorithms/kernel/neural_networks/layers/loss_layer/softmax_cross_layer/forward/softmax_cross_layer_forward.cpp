...
services::Status Result::check(const daal::algorithms::Input * input, const daal::algorithms::Parameter * par, int method) const
{
    ...
    DAAL_CHECK_STATUS(
        s, data_management::checkTensor(get(auxProbabilities).get(), auxProbabilitiesStr(), &(in->get(layers::forward::data)->getDimensions())));
    ...
    {
        DAAL_CHECK_STATUS(
            s, data_management::checkTensor(get(auxGroundTruth).get(), auxGroundTruthStr(), &(in->get(loss::forward::groundTruth)->getDimensions())));
    }
    return s;
}
