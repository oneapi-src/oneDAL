...
services::Status Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    DAAL_CHECK_STATUS(s, data_management::checkTensor(get(layers::forward::weights).get(), weightsStr(), &weightDims));
    DAAL_CHECK_STATUS(s, data_management::checkTensor(get(layers::forward::biases).get(),  biasesStr(), &weightDims));
    DAAL_CHECK_STATUS(s, data_management::checkTensor(get(populationMean).get(),           populationMeanStr(), &weightDims));
    DAAL_CHECK_STATUS(s, data_management::checkTensor(get(populationVariance).get(),       populationVarianceStr(), &weightDims));
    return s;
}
...
services::Status Result::check(const daal::algorithms::Input *input, const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    DAAL_CHECK_STATUS(s, data_management::checkTensor(valueTensor.get(), valueStr(), &dataDims));
