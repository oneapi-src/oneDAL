...
public:
    Status initialize(const BatchNormalizationTaskDescriptor & desc)
    {
        ...
        if (_predictionStage)
        {
            _predictionScaleBuffer.reset(_dimensionSize);
            DAAL_CHECK_MALLOC(_predictionScaleBuffer.get());
            _predictionBiasBuffer.reset(_dimensionSize);
            DAAL_CHECK_MALLOC(_predictionBiasBuffer.get());
        ...
        {
            _invstdwBuffer.reset(_dimensionSize);
            DAAL_CHECK_MALLOC(_invstdwBuffer.get());
            _varianceBuffer.reset(_dimensionSize);
            DAAL_CHECK_MALLOC(_varianceBuffer.get());
...
private:
    ...
    {
        internalThreaderFor([&](size_t kbeg, size_t kend) { computeMeanAndStd(kbeg, kend); },
    ...
    void evaluateNumberOfBlocks()
    {
        ...
        if (_doThreading)
        {
            splitBlocks(threadnum, _dimensionSize, _offsetAfter, _blocknumK, _blocksizeK, _blocksizeLastK);
        }
        else
        {
            _blocknumK      = 1;
    ...
    void splitBlocks(int threadnum, size_t k_size, size_t j_size, size_t & blocknumK, size_t & blocksizeK, size_t & blocksizeLastK)
    {
