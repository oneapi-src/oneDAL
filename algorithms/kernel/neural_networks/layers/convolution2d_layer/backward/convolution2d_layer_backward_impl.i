...
Status Convolution2dKernel<algorithmFPType, method, cpu>::compute(Tensor * inGradTensor, Tensor * xTensor, Tensor * wTensor,
                                                                  const convolution2d::Parameter & parameter, Tensor * wDerTensor,
                                                                  Tensor * bDerTensor, Tensor * resultTensor)
{
    Status s;
    ...
    xDnnLayout ltUserGrad(dimension, gradSize, gradStrides);
    ON_ERR(ltUserGrad.err);
    ...
    if (_resultFlag && parameter.propagateGradient)
    {
        if (!ltUserX.get())
        {
            ...
            ltUserX = xDnnLayout(dimension, xSize, xStrides);
            ON_ERR(ltUserX.err);
        }
        ...
        if (!ltUserFilt.get())
        {
            ...
            ltUserFilt = xDnnLayout(filtDimension, filterSize, filterStrides);
            ON_ERR(ltUserFilt.err);
        }
        if (convGrad == NULL)
        {
            err = dnn::xConvolutionCreateBackwardData(&convGrad, dnnAlgorithmConvolutionDirect, nGroups, dimension, xSize, gradSize, filterSize,
                                                      convolutionStride, xOffset, dnnBorderZeros);
            ON_ERR(err);
        }
    ...
    if (_wDerFlag)
    {
        ...
        if (convFilt == NULL)
        {
            err = dnn::xConvolutionCreateBackwardFilter(&convFilt, dnnAlgorithmConvolutionDirect, nGroups, dimension, xSize, gradSize, filterSize,
                                                        convolutionStride, xOffset, dnnBorderZeros);
            ON_ERR(err);
        }
    ...
    if (_bDerFlag)
    {
        if (!ltUserBias.get())
        {
            ltUserBias = xDnnLayout(1, biasSize, biasStrides);
            ON_ERR(ltUserBias.err);
        }

        if (convBias == NULL)
        {
            err = dnn::xConvolutionCreateBackwardBias(&convBias, dnnAlgorithmConvolutionDirect, nGroups, dimension, gradSize);
            ON_ERR(err);
        }
    ...
    err = dnn::xLayoutCreateFromPrimitive(&inGradLayout, auxPrim, dnnResourceDiffDst);
    ON_ERR(err);
    ...
    else
    {
        ...
        cvToInnerGrad.set(&inGradArray, ltUserGrad.get(), true, &convRes[dnnResourceDiffDst], inGradLayout, false);
        ON_ERR(cvToInnerGrad.err);
        cvToInnerGrad.convert();
        ON_ERR(cvToInnerGrad.err);
    ...
    if (convGrad)
    {
        ...
        xDnnLayout ltInnerFilt(convGrad, dnnResourceFilter);
        ON_ERR(ltInnerFilt.err);

        LayoutConvertor<algorithmFPType, cpu> cvToInnerFilt(&wArray, ltUserFilt.get(), true, &convRes[dnnResourceFilter], ltInnerFilt.get(), false);
        ON_ERR(cvToInnerFilt.err);
        cvToInnerFilt.convert();
        ON_ERR(cvToInnerFilt.err);

        dnnLayout_t resultLayout;
        err = dnn::xLayoutCreateFromPrimitive(&resultLayout, convGrad, dnnResourceDiffSrc);
        ON_ERR(err);
        ...
        if (resultMklTensor != 0)
        {
            ...
            err = dnn::xExecute(convGrad, (void **)convRes);
            ON_ERR(err);
        }
        else
        {
            ...
            cvFromInnerBack.set(&convRes[dnnResourceDiffSrc], resultLayout, false, &resultArray, ltUserX.get(), true);
            ON_ERR(cvFromInnerBack.err);

            err = dnn::xExecute(convGrad, (void **)convRes);
            ON_ERR(err);

            cvFromInnerBack.convert();
            ON_ERR(cvFromInnerBack.err);
    ...
    if (convFilt)
    {
        ...
        err = dnn::xLayoutCreateFromPrimitive(&inputLayout, convFilt, dnnResourceSrc);
        ON_ERR(err);
        ...
        else
        {
            ...
            cvToInnerInput.set(&xArray, ltUserX.get(), true, &convRes[dnnResourceSrc], inputLayout, false);
            ON_ERR(cvToInnerInput.err);
            cvToInnerInput.convert();
            ON_ERR(cvToInnerInput.err);
        ...
        xDnnLayout ltInnerDerFilt(convFilt, dnnResourceDiffFilter);
        ON_ERR(ltInnerDerFilt.err);
        xDnnBuffer dnnResourceDiffFilterBuffer(ltInnerDerFilt.get());
        ON_ERR(dnnResourceDiffFilterBuffer.err);
        convRes[dnnResourceDiffFilter] = dnnResourceDiffFilterBuffer.get();

        LayoutConvertor<algorithmFPType, cpu> cvFromInnerDerFilt(&convRes[dnnResourceDiffFilter], ltInnerDerFilt.get(), true, &derFilt,
                                                                 ltUserFilt.get(), false);
        ON_ERR(cvFromInnerDerFilt.err);
        err = dnn::xExecute(convFilt, (void **)convRes);
        ON_ERR(err);
        cvFromInnerDerFilt.convert();
        ON_ERR(cvFromInnerDerFilt.err);
    ...
    if (convBias)
    {
        ...
        xDnnLayout ltInnerDerBias(convBias, dnnResourceDiffBias);
        ON_ERR(ltInnerDerBias.err);
        xDnnBuffer dnnResourceDiffBiasBuffer(ltInnerDerBias.get());
        ON_ERR(dnnResourceDiffBiasBuffer.err);
        convRes[dnnResourceDiffBias] = dnnResourceDiffBiasBuffer.get();

        LayoutConvertor<algorithmFPType, cpu> cvFromInnerDerBias(&convRes[dnnResourceDiffBias], ltInnerDerBias.get(), true, &derBias,
                                                                 ltUserBias.get(), false);
        ON_ERR(cvFromInnerDerBias.err);
        err = dnn::xExecute(convBias, (void **)convRes);
        ON_ERR(err);
        cvFromInnerDerBias.convert();
        ON_ERR(cvFromInnerDerBias.err);
