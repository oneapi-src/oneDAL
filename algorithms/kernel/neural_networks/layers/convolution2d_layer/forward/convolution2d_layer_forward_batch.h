...
DAAL_EXPORT services::Status Input::allocate(const daal::algorithms::Parameter *parameter, const int method)
{
    ...
    {
        TensorPtr tensor(new MklTensor<algorithmFPType>(getWeightsSizes(param), Tensor::doAllocate));
        set(layers::forward::weights, tensor);
    }

    if( !get(layers::forward::biases) )
    {
        TensorPtr tensor(new MklTensor<algorithmFPType>(getBiasesSizes(param), Tensor::doAllocate));
