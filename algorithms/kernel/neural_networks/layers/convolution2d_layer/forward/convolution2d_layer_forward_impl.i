...
Status Convolution2dKernel<algorithmFPType, method, cpu>::initialize(const services::Collection<size_t> & inDimsFull,
                                                                     const services::Collection<size_t> & wDims,
                                                                     const convolution2d::Parameter & parameter,
                                                                     const services::Collection<size_t> & outDimsFull)
{
    ...
    ltUserFilt = xDnnLayout(filterDimension, filterSize, filterStrides);
    ...
    err = dnn::xConvolutionCreateForwardBias(&convPrim, dnnAlgorithmConvolutionDirect, nGroups, dimension, inputSize, outputSize, filterSize,
                                             convolutionStride, inputOffset, dnnBorderZeros);
    ON_ERR(err);
    return Status();
}
                                                                  ...
                                                                  const convolution2d::Parameter & parameter, Tensor * resultTensor)
{
    ...
    err = dnn::xLayoutCreateFromPrimitive(&inputLayout, convPrim, dnnResourceSrc);
    ON_ERR(err);
    ...
    else
    {
        ...
        cvToInnerInput.set(&inputArray, ltUserInput.get(), true, &convRes[dnnResourceSrc], inputLayout, false);
    ...
    err = dnn::xLayoutCreateFromPrimitive(&filtLayout, convPrim, dnnResourceFilter);
    ON_ERR(err);
    ...
    else
    {
        ...
        cvToInnerFilt.set(&wArray, ltUserFilt.get(), true, &convRes[dnnResourceFilter], filtLayout, false);
    ...
    err = dnn::xLayoutCreateFromPrimitive(&biasLayout, convPrim, dnnResourceBias);
    ON_ERR(err);
    ...
    else
    {
        ...
        cvToInnerBias.set(&bArray, ltUserBias.get(), true, &convRes[dnnResourceBias], biasLayout, false);
        ON_ERR(cvToInnerBias.err);
        cvToInnerBias.convert();
        ON_ERR(cvToInnerBias.err);
    ...
    err = dnn::xLayoutCreateFromPrimitive(&resultLayout, convPrim, dnnResourceDst);
    ON_ERR(err);
    ...
    if (resultMklTensor != NULL)
    {
        ...
        err = dnn::xExecute(convPrim, (void **)convRes);
        ON_ERR(err);
    }
    else
    {
        ...
        LayoutConvertor<algorithmFPType, cpu> cvFromInnerOutput(&convRes[dnnResourceDst], resultLayout, false, &resultArray, ltUserOutput.get(),
                                                                true);
