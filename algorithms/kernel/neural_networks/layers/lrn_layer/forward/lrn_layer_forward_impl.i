...
services::Status LRNKernel<algorithmFPType, method, cpu>::compute(const Tensor & inputTensor, const lrn::Parameter & parameter,
                                                                  Tensor & sMinusBetaTensor, Tensor & resultTensor)
{
    ...
    if (inputMklTensor != 0 && sMinusBetaMklTensor != 0 && resultMklTensor != 0)
    {
        ...
        if (lrnPrim == NULL)
        {
            err = dnn::xLRNCreateForward(&lrnPrim, inputLayout, nAdjust, alpha * nAdjust, beta, kappa);
            ON_ERR(err);
        }
        ...
        err = dnn::xLayoutCreateFromPrimitive(&resultLayout, lrnPrim, dnnResourceDst);
        ON_ERR(err);
    ...
    else
    {
        if (0.0 == beta)
        {
            ...
            return computeImpl<cpu>(inputTensor,
                                    [=, &inputTensor, &sMinusBetaTensor, &resultTensor](size_t nFixedDims, size_t * fixedDims, size_t nRowsToProcess,
                                                                                        const TensorOffsetLayout & layout) -> Status {
                                        ReadSubtensor<algorithmFPType, cpu, Tensor> inputBlock(const_cast<Tensor &>(inputTensor), nFixedDims,
                                                                                               fixedDims, 0, nRowsToProcess, layout);
                                        DAAL_CHECK_BLOCK_STATUS(inputBlock);
                                        const algorithmFPType * inputArray = inputBlock.get();

                                        WriteOnlySubtensor<algorithmFPType, cpu, Tensor> resultBlock(resultTensor, nFixedDims, fixedDims, 0,
                                                                                                     nRowsToProcess, layout);
                                        DAAL_CHECK_BLOCK_STATUS(resultBlock);
                                        algorithmFPType * resultArray = resultBlock.get();

                                        WriteOnlySubtensor<algorithmFPType, cpu, Tensor> sMinusBetaBlock(sMinusBetaTensor, nFixedDims, fixedDims, 0,
                                                                                                         nRowsToProcess, layout);
                                        ...
                                        daal::services::internal::daal_memcpy_s(resultArray, nElements * sizeof(algorithmFPType), inputArray,
                                                                                nElements * sizeof(algorithmFPType));
        ...
        if (targetDimPosition >= firstFreeDim)
        {
            ...
            return computeImpl<cpu>(
                ...
                [=, &inputTensor, &sMinusBetaTensor, &resultTensor](size_t nFixedDims, size_t * fixedDims, size_t nRowsToProcess,
                                                                    const TensorOffsetLayout & layout) -> Status {
                    ReadSubtensor<algorithmFPType, cpu, Tensor> inputBlock(const_cast<Tensor &>(inputTensor), nFixedDims, fixedDims, 0,
                                                                           nRowsToProcess, layout);
                    DAAL_CHECK_BLOCK_STATUS(inputBlock);
                    const algorithmFPType * inputArray = inputBlock.get();
                    ...
                    WriteOnlySubtensor<algorithmFPType, cpu, Tensor> sMinusBetaBlock(sMinusBetaTensor, nFixedDims, fixedDims, 0, nRowsToProcess,
                                                                                     layout);
                    DAAL_CHECK_BLOCK_STATUS(sMinusBetaBlock);
                    algorithmFPType * sMinusBetaArray = sMinusBetaBlock.get();
