...
int shiftFixedDims(const size_t * fixedDims, const int shiftValue, const size_t pos, const size_t dimSize, size_t * dimsShifted, const size_t nDims)
{
    ...
    if (innerDimPosition + shiftValue < 0)
    {
        return 1;
    }
    if (innerDimPosition + shiftValue >= dimSize)
    {
        return 2;
    }
...
services::Status LRNKernel<algorithmFPType, method, cpu>::compute(const Tensor & auxDataTensor, const Tensor & sMinusBetaTensor,
                                                                  const Tensor & inputGradientTensor, Tensor & gradientTensor,
                                                                  const lrn::Parameter & parameter)
{
    ...
    if (sMinusBetaMklTensor != 0 && !sMinusBetaMklTensor->isPlainLayout())
    {
        ...
        else
        {
            ...
            err = dnn::xLayoutCreate(&inputLayout, nDims, inputSize, inputStrides);
            ON_ERR(err);
        ...
        else
        {
            ...
            err = dnn::xLayoutCreate(&inputGradLayout, nDims, inputGradSize, inputGradStrides);
            ON_ERR(err);
        ...
        if (lrnPrim == NULL)
        {
            err = dnn::xLRNCreateBackward(&lrnPrim, inputGradLayout, inputLayout, nAdjust, alpha * nAdjust, beta, kappa);
            ON_ERR(err);
        }

        dnnLayout_t workspaceLayout;
        err = dnn::xLayoutCreateFromPrimitive(&workspaceLayout, lrnPrim, dnnResourceWorkspace);
        ON_ERR(err);
        ...
        err = dnn::xLayoutCreateFromPrimitive(&gradLayout, lrnPrim, dnnResourceDiffSrc);
        ON_ERR(err);
        ...
        else
        {
            ...
            err = dnn::xLayoutCreate(&gradLayout, nDims, gradSize, gradStrides);
            ON_ERR(err);
            ...
            LayoutConvertor<algorithmFPType, cpu> cvFromInnerOutput(&lrnRes[dnnResourceDiffSrc], gradLayout, false, &gradArray, gradLayout, true);
            ON_ERR(cvFromInnerOutput.err);

            err = dnn::xExecute(lrnPrim, (void **)lrnRes);

            cvFromInnerOutput.convert();
            ON_ERR(cvFromInnerOutput.err);
        }
    }
    else
    {
        if (0.0 == beta)
        {
            ...
            return computeImpl<cpu>(inputGradientTensor,
                                    [=, &inputGradientTensor, &gradientTensor](size_t nFixedDims, size_t * fixedDims, size_t nRowsToProcess,
                                                                               const TensorOffsetLayout & layout) -> Status {
                                        ReadSubtensor<algorithmFPType, cpu, Tensor> inputGradientBlock(
                                            const_cast<Tensor &>(inputGradientTensor), nFixedDims, fixedDims, 0, nRowsToProcess, layout);
                                        DAAL_CHECK_BLOCK_STATUS(inputGradientBlock);
                                        const algorithmFPType * inputGradientArray = inputGradientBlock.get();

                                        WriteOnlySubtensor<algorithmFPType, cpu, Tensor> gradientBlock(gradientTensor, nFixedDims, fixedDims, 0,
                                                                                                       nRowsToProcess, layout);
        ...
        if (targetDimPosition >= firstFreeDim)
        {
            ...
            return computeImpl<cpu>(
                ...
                [=, &inputGradientTensor, &auxDataTensor, &sMinusBetaTensor, &gradientTensor](
                    ...
                    ReadSubtensor<algorithmFPType, cpu, Tensor> inputGradientBlock(const_cast<Tensor &>(inputGradientTensor), nFixedDims, fixedDims,
                                                                                   0, nRowsToProcess, layout);
                    DAAL_CHECK_BLOCK_STATUS(inputGradientBlock);
                    const algorithmFPType * inputGradientArray = inputGradientBlock.get();

                    ReadSubtensor<algorithmFPType, cpu, Tensor> auxDataBlock(const_cast<Tensor &>(auxDataTensor), nFixedDims, fixedDims, 0,
                                                                             nRowsToProcess, layout);
                    DAAL_CHECK_BLOCK_STATUS(auxDataBlock);
                    const algorithmFPType * auxDataArray = auxDataBlock.get();

                    ReadSubtensor<algorithmFPType, cpu, Tensor> sMinusBetaBlock(const_cast<Tensor &>(sMinusBetaTensor), nFixedDims, fixedDims, 0,
                                                                                nRowsToProcess, layout);
                    ...
                    Math<algorithmFPType, cpu>::vPowxAsLnExp(nElements, const_cast<algorithmFPType *>(sMinusBetaArray), toMinusBetaPower,
                                                             sMinusBetaMinusOneArray);
            ...
            return computeImpl<cpu>(
                ...
                [=, &inputGradientTensor, &auxDataTensor, &sMinusBetaTensor, &gradientTensor](
                    ...
                    for (int shiftValue = l; shiftValue < r; shiftValue++)
                    {
                        ...
                        if (shiftValue != 0)
                        {
                            int retCode = shiftFixedDims<cpu, algorithmFPType>(fixedDims, shiftValue, targetDimPosition, targetDimSize,
                                                                               fixedDimsShifted, nFixedDims);
                            if (retCode != 0)
                            {
                                continue;
                            }
                            inputGradientBlockLocal.set(const_cast<Tensor &>(inputGradientTensor), nFixedDims, fixedDimsShifted, 0, nRowsToProcess,
                                                        layout);
                        ...
                        Math<algorithmFPType, cpu>::vPowxAsLnExp(nElements, const_cast<algorithmFPType *>(sMinusBetaArrayShifted), toMinusBetaPower,
                                                                 sMinusBetaMinusOneArray);
