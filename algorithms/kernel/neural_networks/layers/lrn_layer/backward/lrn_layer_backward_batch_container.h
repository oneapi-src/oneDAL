...
void BatchContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    Tensor *dataTable          = input->get(layers::lrn::auxData).get();
    Tensor *sMinusBetaTable    = input->get(lrn::auxSmBeta).get();
    Tensor *inputGradientTable = input->get(layers::backward::inputGradient).get();
    Tensor *gradientTable      = result->get(layers::backward::gradient).get();

    __DAAL_CALL_KERNEL(env, internal::LRNKernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method), compute, dataTable, sMinusBetaTable, inputGradientTable,
                                                                         gradientTable, parameter);
}
} // namespace interface1
} // namespace backward
