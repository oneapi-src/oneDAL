...
struct common_fullyconnected_data_t
{
    ...
    common_fullyconnected_data_t(const Tensor & _inputTensor, const Tensor & _wTensor, const Tensor & _bTensor, Tensor & _resultTensor,
          ...
          resultTensor(const_cast<Tensor &>(_resultTensor))
    {
        ...
        status |= inputTensor.getSubtensor(0, 0, 0, inDims[0], readOnly, inputBlock);
        if (!status) return;
        status |= wTensor.getSubtensor(0, 0, 0, wDims[0], readOnly, wBlock);
        if (!status) return;
        status |= bTensor.getSubtensor(0, 0, 0, outs_num, readOnly, bBlock);
        if (!status) return;
        status |= resultTensor.getSubtensor(0, 0, 0, inDims[0], writeOnly, resultBlock);
        if (!status) return;
        ...
        if (((batch_size == 1) && (outs_num >= 200)) || ((batch_size * outs_num) > (100 * data_size))
#if __CPUID__(DAAL_CPU) >= __avx512_mic__
            || ((batch_size >= 256) && (data_size <= 7000) && (outs_num <= 1000))
#else
            || (data_size <= 7000) || ((data_size <= 200000) && (outs_num > 256))
#endif
        )
        {
            do_parallel = 0;
...
struct tls_fullyconnected_data_t
{
    ...
    ~tls_fullyconnected_data_t()
    {
        if (result)
        {
            daal::services::internal::service_scalable_free<algorithmFPType, cpu>(result);
            result = 0;
        }
    }
};
...
Status FullyconnectedKernel<algorithmFPType, method, cpu>::compute(const Tensor & inputTensor, const Tensor & wTensor, const Tensor & bTensor,
    ...
    if (_cd.batch_size == 1)
    {
        ...
        if (_cd.do_parallel)
        {
            ...
            daal::threader_for(_cd.blocknum, _cd.blocknum, [&](size_t b) {
                ...
                Blas<algorithmFPType, cpu>::xxgemv(&trans, &cursize_local, &_n, &alpha, _cd.wArray + b * _cd.blocksize, &lda,
                                                   _cd.iArray + b * _cd.blocksize, &incx, &beta, _td->result, &incy);
            }); /* daal::threader_for */
        ...
        {
            Blas<algorithmFPType, cpu>::xgemv(&trans, &_m, &_n, &alpha, _cd.wArray, &lda, _cd.iArray, &incx, &beta, _cd.rArray, &incy);
        } /* if( _cd.do_parallel ) */
    }
    ...
    else
    {
        ...
        if (_cd.do_parallel)
        {
            ...
            daal::threader_for(_cd.blocknum, _cd.blocknum, [&](size_t b) {
                ...
                Blas<algorithmFPType, cpu>::xxgemm(&transa, &transb, &_m, &_n, &cursize_local, &alpha, _cd.wArray + b * _cd.blocksize, &lda,
                                                   _cd.iArray + b * _cd.blocksize, &ldb, &beta, _td->result, &ldc);
            }); /* daal::threader_for */
        ...
        {
            Blas<algorithmFPType, cpu>::xgemm(&transa, &transb, &_m, &_n, &_k, &alpha, _cd.wArray, &lda, _cd.iArray, &ldb, &beta, _cd.rArray, &ldc);
        } /* if( _cd.do_parallel ) */
    }     /* if( _cd.batch_size == 1 ) */
    return Status();
