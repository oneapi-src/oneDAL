...
services::Status TransposedConv2dKernel<algorithmFPType, method, cpu>::compute(const Tensor & inputTensor, const Tensor & wTensor,
                                                                               const Tensor & bTensor, const transposed_conv2d::Parameter & parameter,
                                                                               Tensor & resultTensor)
{
    Status s;
...

    {
        ...
        DAAL_CHECK_STATUS(s, dconvKernel.compute(const_cast<Tensor *>(&inputTensor), NULL, const_cast<Tensor *>(&wTensor), bwdConvParameter, NULL,
                                                 NULL, &resultTensor));
