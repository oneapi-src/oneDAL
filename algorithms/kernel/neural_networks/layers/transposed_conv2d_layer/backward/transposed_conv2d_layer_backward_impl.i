...
services::Status TransposedConv2dKernel<algorithmFPType, method, cpu>::compute(const Tensor & inGradTensor, const Tensor & xTensor,
                                                                               const Tensor & wTensor, const transposed_conv2d::Parameter & parameter,
                                                                               Tensor & wDerTensor, Tensor & bDerTensor, Tensor & resultTensor)
{
...

    {
        ...
        convolution2d::backward::internal::Convolution2dKernel<algorithmFPType, neural_networks::layers::convolution2d::defaultDense, cpu>
            dconvKernel;
        DAAL_CHECK_STATUS(s, dconvKernel.initialize(false, true, false));
        DAAL_CHECK_STATUS(s, dconvKernel.compute(const_cast<Tensor *>(&xTensor), const_cast<Tensor *>(&inGradTensor), const_cast<Tensor *>(&wTensor),
                                                 bwdConvParameter, &wDerTensor, NULL, NULL));
...

    {
        ...
        ReadSubtensor<algorithmFPType, cpu> inGradBlock(const_cast<Tensor &>(inGradTensor), 0, 0, 0, inGradTensor.getDimensionSize(0),
                                                        gTargetInLayout);
    ...
    if (parameter.propagateGradient) // compute gradient w.r.t. data
    {
...

        {
            ...
            convolution2d::forward::internal::Convolution2dKernel<algorithmFPType, neural_networks::layers::convolution2d::defaultDense, cpu>
                convKernel;
            DAAL_CHECK_STATUS(s, convKernel.initialize(inDimsFull, wDims, fwdConvParameter, outDimsFull));
            DAAL_CHECK_STATUS(s, convKernel.compute(const_cast<Tensor *>(&inGradTensor), const_cast<Tensor *>(&wTensor), &bTensor, fwdConvParameter,
                                                    &resultTensor));
