...
Status PReLUKernel<algorithmFPType, method, cpu>::compute(const Tensor & inputTensor, const Tensor & wTensor, Tensor & resultTensor,
                                                          const prelu::Parameter & parameter)
{
    ...
    else
    {
        ...
        daal::threader_for(nBlocks, nBlocks, [=, &safeStat, &inputTensor, &resultTensor](size_t i) {
            ...
            if (!fDims)
            {
                safeStat |= Status(ErrorMemoryAllocationFailed);
                return;
            }
...
template <typename algorithmFPType, Method method, CpuType cpu>
Status PReLUKernel<algorithmFPType, method, cpu>::processBlock(const Tensor & inputTensor, Tensor & resultTensor, const algorithmFPType * wArray,
                                                               size_t fDimN, size_t * fDims, const TensorOffsetLayout & layout, size_t wSize,
...
template <typename algorithmFPType, Method method, CpuType cpu>
void PReLUKernel<algorithmFPType, method, cpu>::getNumberOfFixedDimensions(TensorOffsetLayout & inputLayout, const Collection<size_t> & dims,
                                                                           size_t wEndDim, size_t & fDimN, size_t & wOffset,
                                                                           size_t minElementsNumInBlock)
{
