...
services::Status ReLUKernel<algorithmFPType, method, cpu>::compute(const Tensor & inputGradientTensor, const Tensor & forwardDataTensor,
                                                                   Tensor & resultTensor)
{
    ...
    if (inputGradientMklTensor != 0 && forwardDataMklTensor != 0 && resultMklTensor != 0)
    {
        ...
        if (reluPrim == NULL)
        {
            err = dnn::xReLUCreateBackward(&reluPrim, inputGradLayout, inputLayout, (algorithmFPType)0.0);
            ON_ERR(err);
        }

        err = dnn::xLayoutCreateFromPrimitive(&resultLayout, reluPrim, dnnResourceDiffSrc);
        ON_ERR(err);
        ...
        err = dnn::xExecute(reluPrim, (void **)reluRes);
        ON_ERR(err);
    }
    else
    {
        ...
        s = computeImpl<cpu>(inputGradientTensor,
                             [=, &inputGradientTensor, &forwardDataTensor, &resultTensor](size_t fDimN, size_t * fDims, size_t nRowsToProcess,
                                                                                          const TensorOffsetLayout & layout) -> Status {
                                 ReadSubtensor<algorithmFPType, cpu, Tensor> inputGradientBlock(const_cast<Tensor &>(inputGradientTensor), fDimN,
                                                                                                fDims, 0, nRowsToProcess, layout);
                                 DAAL_CHECK_BLOCK_STATUS(inputGradientBlock);
                                 const algorithmFPType * inputGradientArray = inputGradientBlock.get();

                                 ReadSubtensor<algorithmFPType, cpu, Tensor> forwardBlock(const_cast<Tensor &>(forwardDataTensor), fDimN, fDims, 0,
