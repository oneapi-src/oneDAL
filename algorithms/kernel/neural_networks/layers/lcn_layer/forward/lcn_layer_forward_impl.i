...
Status LCNKernel<algorithmFPType, method, cpu>::initialize(const Tensor & inputTensor, Tensor & cTensor, Tensor & invMaxTensor,
                                                           const lcn::Parameter & parameter, const Tensor & kernelTensor)
{
...
template <typename algorithmFPType, Method method, CpuType cpu>
Status LCNKernel<algorithmFPType, method, cpu>::compute(const Tensor & inputTensor, Tensor & sigmaTensor, Tensor & cTensor, Tensor & resultTensor,
                                                        Tensor & centeredDataTensor, Tensor & invMaxTensor, const lcn::Parameter & parameter,
                                                        const Tensor & kernelTensor)
{
    ...
    {
        sigmaArray =
            invMaxArray; /* when we on prediction stage, we need not compute invMaxArray, and we don't use then simultaneously, so they refer on the same memory */
    }
...
template <typename algorithmFPType, Method method, CpuType cpu>
void LCNKernel<algorithmFPType, method, cpu>::getConvolutionWeightsFromInputKernel(algorithmFPType * weightsArray, const lcn::Parameter & parameter)
{
    if (parameter.sumDimension)
    {
...
Status LCNKernel<algorithmFPType, method, cpu>::calculateCTensorAndGetMaxArray(Tensor & cTensor)
{
    ...
    for (size_t i = 0; i < dataOffsetBeforeDim; i++)
    {
        ...
        {
            /* step_7:  1/x_13 = 1 / max(step_5, step_6) */
            invMaxArray[j + i * dataOffsetAfterDim] =
                one / daal::internal::Math<algorithmFPType, cpu>::sMax(sigmaArray[j + i * dataOffsetAfterDim], cArray[i]);
        }
    }
    return Status();
}
