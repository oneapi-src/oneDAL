...
struct Tls_data
{
    ...
    Tls_data(size_t dataOffsetAfterDim, size_t firstKernelDim, size_t secondKrnelDim, size_t firstDim, size_t secondDim)
    {
    ...
    ~Tls_data() { dconvKernel.reset(); }
};

template <typename algorithmFPType, Method method, CpuType cpu>
Status LCNKernel<algorithmFPType, method, cpu>::initialize(const Tensor & auxCenteredDataTensor, const Tensor & auxSigmaTensor,
                                                           const Tensor & auxCTensor, const Tensor & kernelTensor, const lcn::Parameter & parameter)
{
    ...
    batchDimension = 6 - initialSumDimension - initialFirstDim
                     - initialSecondDim; /* Calculate 4th dimension index. 6 here is a sum of all indexes: 0 + 1 + 2 + 3 */
    dataDims << initialDataDims[batchDimension] << initialDataDims[initialSumDimension] << initialDataDims[initialFirstDim]
             << initialDataDims[initialSecondDim];
...
*/
template <typename algorithmFPType, Method method, CpuType cpu>
Status LCNKernel<algorithmFPType, method, cpu>::compute(const Tensor & auxCenteredDataTensor, const Tensor & auxSigmaTensor,
                                                        const Tensor & auxCTensor, const Tensor & auxInvMaxTensor, const Tensor & kernelTensor,
                                                        const Tensor & inGradTensor, Tensor & gradientTensor, const lcn::Parameter & parameter)
{
    Status s;
    ...
    daal::threader_for(dataOffsetBeforeDim, dataOffsetBeforeDim, [&](int i) {
        ...
        s = tls_data_local->dconvKernel.compute(tls_data_local->convInGradTensor.get(), tls_data_local->convResultTensor.get(), weightsTensor.get(),
                                                convParameter, tls_data_local->wDerTensor.get(), tls_data_local->bDerTensor.get(),
                                                tls_data_local->convResultTensor.get());
        safeStat |= s;
        if (!s) return;
    ...
    tls_data.reduce([&](Tls_data<algorithmFPType, method, cpu> * tls_data_local) { delete tls_data_local; });
