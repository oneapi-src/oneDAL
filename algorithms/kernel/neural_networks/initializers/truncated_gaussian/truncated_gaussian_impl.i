...
inline algorithmFPType sqrt_2(void)
{
    return algorithmFPType(0.0);
}
template <>
inline float sqrt_2<float>(void)
{
    uint32_t iv = 0x3FB504F3;
    float v     = *(float *)&iv;
    return v;
}
template <>
inline double sqrt_2<double>(void)
{
    uint64_t iv = 0x3FF6A09E667F3BCD;
    double v    = *(double *)&iv;
    return v;
}
...
Status TruncatedGaussianKernel<algorithmFPType, method, cpu>::compute(const TruncatedGaussianInitializerTaskDescriptor<algorithmFPType> & desc)
{
    ...
    Status s;
    DAAL_CHECK_STATUS(s,
                      (UniformKernelDefault<algorithmFPType, cpu>::compute((algorithmFPType)0.0, (algorithmFPType)1.0, *engine, size, resultArray)));
