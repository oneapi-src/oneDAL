...
#define DAAL_NN_CHECK(cond, error) if(!(cond)) { reset(); return services::Status(error); }
#define DAAL_NN_CHECK_MALLOC(cond) DAAL_NN_CHECK(cond, services::ErrorMemoryAllocationFailed)
#define DAAL_NN_CHECK_STATUS(destVar, srcVal) { destVar |= (srcVal); if(!destVar) { reset(); return destVar; } }
#define DAAL_NN_CHECK_BLOCK_STATUS(block) { if(!block.status()) { reset(); return block.status(); } }
