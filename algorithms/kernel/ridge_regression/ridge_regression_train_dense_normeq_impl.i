                               ...
                               services::KernelErrorCollection *errors)
{
    ...
    if (isEqualRidge)
    {
        ...
        if (info < 0) { daal::services::daal_free(tempXTX); errors->add(services::ErrorRidgeRegressionInternal); return; }
        if (info > 0) { daal::services::daal_free(tempXTX); errors->add(services::ErrorRidgeRegressionNormEqSystemSolutionFailed); return; }
    ...
    else
    {
        ...
        for (size_t j = 0; j < static_cast<size_t>(*ny); ++j)
        {
            ...
            /* Perform L*L' decomposition of X'*X */
            Lapack<algorithmFpType, cpu>::xpotrf(&uplo, p, tempXTX, ldxtx, &info);
            if (info < 0) { daal::services::daal_free(tempXTX); errors->add(services::ErrorRidgeRegressionInternal); break; }
            if (info > 0) { daal::services::daal_free(tempXTX); errors->add(services::ErrorRidgeRegressionNormEqSystemSolutionFailed); break; }
...
template <typename algorithmFpType, CpuType cpu>
services::Status RidgeRegressionTrainBatchKernel<algorithmFpType, training::normEqDense, cpu>::compute(
...
template <typename algorithmfptype, CpuType cpu>
services::Status RidgeRegressionTrainOnlineKernel<algorithmfptype, training::normEqDense, cpu>::compute(
    NumericTable *x, NumericTable *y, ridge_regression::Model *r, const daal::algorithms::Parameter * par)
{
...
template <typename algorithmfptype, CpuType cpu>
services::Status RidgeRegressionTrainOnlineKernel<algorithmfptype, training::normEqDense, cpu>::finalizeCompute(
