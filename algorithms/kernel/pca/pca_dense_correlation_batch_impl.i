    ...
    data_management::NumericTable & means, data_management::NumericTable & variances)
{
    ...
    if (isCorrelation)
    {
        ...
        if (resultsToCompute & mean) { DAAL_CHECK_STATUS(status, this->fillTable(means, (algorithmFPType)0)); }

        if (resultsToCompute & variance) { DAAL_CHECK_STATUS(status, this->fillTable(variances, (algorithmFPType)1)); }
