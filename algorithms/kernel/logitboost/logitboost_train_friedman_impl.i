...
template<typename algorithmFPType, CpuType cpu>
services::Status LogitBoostTrainKernel<friedman, algorithmFPType, cpu>::compute(const size_t na, NumericTablePtr a[], logitboost::interface1::Model *r,
...
public:
    LogitBoostLsNew(const size_t n): _nRows(n), _isInit(false) {}
    ~LogitBoostLsNew() {}
    DAAL_NEW_DELETE();
...
services::Status UpdateFPNew( size_t nc, size_t n, algorithmFPType *F, algorithmFPType *P, const algorithmFPType *pred,
    daal::ls<LogitBoostLsNew<algorithmFPType, cpu> *>& lsData)
{
    const size_t minBlockSize = 768;
    const size_t nThreads = threader_get_threads_number();
    size_t nBlocks = n / minBlockSize;
    ...
    daal::threader_for(nBlocks, nBlocks, [&] (size_t iBlock)
    {
        ...
        struct LogitBoostLsNew<algorithmFPType, cpu> *lsLocal = lsData.local();
        if (!lsLocal)
        {
            safeStat.add(services::ErrorMemoryAllocationFailed);
            return;
...
template<typename algorithmFPType, CpuType cpu>
services::Status LogitBoostTrainKernelNew<friedman, algorithmFPType, cpu>::compute(const size_t na, NumericTablePtr a[], Model *r, const Parameter *par)
{
    const algorithmFPType zero(0.0);
    const algorithmFPType fp_one(1.0);
    ...
    daal::ls<LogitBoostLsNew<algorithmFPType, cpu> *> lsData([&]()
    {
        auto ptr = new LogitBoostLsNew<algorithmFPType, cpu>(n);
        if(!ptr)
        {
            safeStat.add(services::ErrorMemoryAllocationFailed);
        }
    ...
    lsData.reduce([ = ](LogitBoostLsNew<algorithmFPType, cpu> *logitBoostLs)
    {
        delete logitBoostLs;
    });
    return s;
