...
template <typename algorithmFPType, CpuType cpu>
void PredictKernel<algorithmFPType, defaultDense, cpu>::computeBlockOfResponsesSOA(DAAL_INT * numFeatures, DAAL_INT * numRows,
                                                                                   const algorithmFPType * dataBlock, DAAL_INT * numBetas,
                                                                                   const algorithmFPType * beta, DAAL_INT * numResponses,
                                                                                   algorithmFPType * responseBlock, bool findBeta0)
{
    ...
    Blas<algorithmFPType, cpu>::xxgemm(&trans, &trans, numResponses, numRows, numFeatures, &one, beta + 1, numBetas, dataBlock, numRows, &zero,
                                       responseBlock, numResponses);
...
services::Status PredictKernel<algorithmFPType, defaultDense, cpu>::compute(const NumericTable * a, const linear_model::Model * m, NumericTable * r)
{
    ...
    SOANumericTable * soaPtr = dynamic_cast<SOANumericTable *>(dataTable);
    ...
    if (soaPtr)
    {
        const size_t numRowsInBlock = _numRowsInBlock;
        const size_t numBlocks      = numVectors / numRowsInBlock + !!(numVectors % numRowsInBlock);
        ...
        daal::threader_for(numBlocks, numBlocks, [=, &safeStat](int iBlock) {
            ...
            TArray<algorithmFPType, cpu> dataArray(numRows * numFeatures);
            DAAL_CHECK_MALLOC_THR(dataArray.get());
            algorithmFPType * const data = dataArray.get();

            for (size_t j = 0; j < numFeatures; ++j)
            {
                daal::internal::ReadColumns<algorithmFPType, cpu> dataColumn(soaPtr, j, startRow, numRows);
                DAAL_CHECK_BLOCK_STATUS_THR(dataColumn);
                const algorithmFPType * dataBlock = dataColumn.get();

                daal::services::internal::daal_memcpy_s(data + j * numRows, sizeof(algorithmFPType) * numRows,
                                                        dataBlock, sizeof(algorithmFPType) * numRows);
            }

            computeBlockOfResponsesSOA(&numFeatures, &numRows, data, &nAllBetas, beta, pnumResponses, responseBlock, model->getInterceptFlag());
        });
    }
    else
    {
