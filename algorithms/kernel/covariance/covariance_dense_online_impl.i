                    ...
                    const Parameter *parameter)
{
    ...
    else
    {
        ...
        if( method == defaultDense )
        {
            ...
            DAAL_KERNEL_CHECK_EX( userSums, services::ErrorMemoryAllocationFailed );
        }
        else // method == sumDense
        {
            NumericTable *userSumsTable = dataTable->basicStatistics.get(NumericTable::sum).get();
            DAAL_KERNEL_CHECK_EX(userSumsTable, services::ErrorPrecomputedSumNotAvailable);
        ...
        DAAL_KERNEL_CHECK_EX(partialCrossProduct, services::ErrorMemoryAllocationFailed);
