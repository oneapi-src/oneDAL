...
private:
    services::Status grow()
    {
        ...
        T * const newData = static_cast<T *>(daal::services::internal::service_calloc<T, cpu>(_capacity * sizeof(T)));
...
private:
    ...
    {
        _capacity = (_capacity == 0 ? defaultSize : _capacity * 2);
        void * ptr = daal::services::daal_calloc(_capacity * sizeof(size_t));
