...
#include "dbscan_batch.h"
#include "dbscan_distributed.h"
...
services::Status BatchContainer<algorithmFPType, method, cpu>::compute()
{
    ...
    {
        __DAAL_CALL_KERNEL(env, internal::DBSCANBatchKernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method), computeNoMemSave,
                           ntData.get(), ntWeights.get(), ntAssignments.get(), ntNClusters.get(), ntCoreIndices.get(), ntCoreObservations.get(), par);
    }
    else
    {
        __DAAL_CALL_KERNEL(env, internal::DBSCANBatchKernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method), computeMemSave,
                           ntData.get(), ntWeights.get(), ntAssignments.get(), ntNClusters.get(), ntCoreIndices.get(), ntCoreObservations.get(), par);
    }
}
...
services::Status DistributedContainer<step1Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep1Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, ntData.get(), ntPartialOrder.get(), par);
}
...
services::Status DistributedContainer<step2Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep2Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, dcPartialData.get(), ntBoundingBox.get(), par);
}
...
services::Status DistributedContainer<step3Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep3Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, dcPartialData.get(), dcPartialBoundingBoxes.get(), ntSplit.get(), par);
}
...
services::Status DistributedContainer<step4Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep4Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, dcPartialData.get(), dcPartialSplits.get(), dcPartialOrders.get(),
                       dcPartitionedData.get(), dcPartitionedPartialOrders.get(), par);
}
...
services::Status DistributedContainer<step5Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep5Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, dcPartialData.get(), dcPartialBoundingBoxes.get(), dcPartitionedHaloData.get(),
                       dcPartitionedHaloDataIndices.get(), par);
}
...
services::Status DistributedContainer<step6Local, algorithmFPType, method, cpu>::compute()
{
    ...
    if (par->memorySavingMode == false)
    {
        __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep6Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                           computeNoMemSave, dcPartialData.get(), dcHaloData.get(),
                           dcHaloDataIndices.get(), dcHaloBlocks.get(),
                           ntClusterStructure.get(), ntFinishedFlag.get(),
                           ntNClusters.get(), dcQueries.get(), par);
    }
    else
    {
        __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep6Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                           computeMemSave, dcPartialData.get(), dcHaloData.get(),
                           dcHaloDataIndices.get(), dcHaloBlocks.get(),
                           ntClusterStructure.get(), ntFinishedFlag.get(),
                           ntNClusters.get(), dcQueries.get(), par);
    }
...
services::Status DistributedContainer<step7Master, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep7Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, dcPartialFinishedFlags.get(), ntFinishedFlag.get());
}
...
services::Status DistributedContainer<step8Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep8Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, ntInputClusterStructure.get(), ntInputNClusters.get(), dcPartialQueries.get(),
                       ntClusterStructure.get(), ntFinishedFlag.get(), ntNClusters.get(), dcQueries.get(), par);
}
...
services::Status DistributedContainer<step9Master, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep9Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, dcPartialNClusters.get(), dcClusterOffsets.get());
}
...
services::Status DistributedContainer<step9Master, algorithmFPType, method, cpu>::finalizeCompute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep9Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       finalizeCompute, dcClusterOffsets.get(), ntNClusters.get());
}
...
services::Status DistributedContainer<step10Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep10Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, ntInputClusterStructure.get(), ntClusterOffset.get(),
                       ntClusterStructure.get(), ntFinishedFlag.get(), dcQueries.get(), par);
}
...
services::Status DistributedContainer<step11Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep11Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, ntInputClusterStructure.get(), dcPartialQueries.get(),
                       ntClusterStructure.get(), ntFinishedFlag.get(), dcQueries.get(), par);
}
...
services::Status DistributedContainer<step12Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep12Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, ntInputClusterStructure.get(), dcPartialOrders.get(), dcAssignmentQueries.get(), par);
}
...
services::Status DistributedContainer<step13Local, algorithmFPType, method, cpu>::compute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep13Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       compute, dcPartialAssignmentQueries.get(), ntAssignmentQueries.get(), par);
    return services::Status();
}
...
services::Status DistributedContainer<step13Local, algorithmFPType, method, cpu>::finalizeCompute()
{
    ...
    __DAAL_CALL_KERNEL(env, internal::DBSCANDistrStep13Kernel, __DAAL_KERNEL_ARGUMENTS(algorithmFPType, method),
                       finalizeCompute, ntAssignmentQueries.get(), ntAssignments.get(), par);
}
