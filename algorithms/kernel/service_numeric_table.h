...
{
public:
    GetRowsCSR(CSRNumericTableIface& data, size_t iStartFrom, size_t nRows) : m_data(&data)
    {
        m_data->getSparseBlock(iStartFrom, nRows, mode, m_block);
    }
    GetRowsCSR(CSRNumericTableIface* data, size_t iStartFrom, size_t nRows) : m_data(data)
    {
        if(m_data)
    ...
    GetRowsCSR() : m_data(nullptr){}
    ~GetRowsCSR() { release(); }
    algorithmFPAccessType* values() const { return m_data ? m_block.getBlockValuesPtr() : nullptr; }
    const size_t* cols() const { return m_data ? m_block.getBlockColumnIndicesPtr() : nullptr; }
    const size_t* rows() const { return m_data ? m_block.getBlockRowIndicesPtr() : nullptr; }
    void next(size_t iStartFrom, size_t nRows)
    {
        if(m_data)
        {
            m_data->releaseSparseBlock(m_block);
            m_data->getSparseBlock(iStartFrom, nRows, mode, m_block);
        }
    }
...
{
public:
    TSmartPtr(size_t n = 0) : m_data(nullptr) { m_data = (T*)(n ? daal::services::daal_malloc(n*sizeof(T)) : nullptr); }
...
{
public:
    TNewSmartPtr(size_t n = 0) : m_data(nullptr) { m_data = (T*)(n ? new T[n] : nullptr); }
    ~TNewSmartPtr() { if(m_data) delete [] (m_data); }
    T* get() { return m_data; }
    const T* get() const { return m_data; }
    void reset(size_t n)
    {
