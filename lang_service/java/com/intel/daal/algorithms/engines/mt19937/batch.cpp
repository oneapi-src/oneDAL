...
(JNIEnv *env, jobject thisObj, jint prec, jint method, jint seed)
{
    ...
    if(prec == 0)
    {
        ...
        {
            addr = (jlong)(engines::mt19937::Batch<double, engines::mt19937::defaultDense>::create(seed));
        }
    }
    else
    {
        if(method == defaultDenseMethod)
        {
            addr = (jlong)(engines::mt19937::Batch<float, engines::mt19937::defaultDense>::create(seed));
        }
    }
