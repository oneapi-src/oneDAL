...
public:
     ...
     */
    JavaTensor(Collection<size_t> & dims, JavaVM * _jvm, jobject _JavaTensor) : Tensor(&_layout), jvm(_jvm), _layout(dims), jJavaTensor(NULL)
    {
    ...
    {
        return getTSubtensor<double>(fixedDims, fixedDimNums, rangeDimIdx, rangeDimNum, rwflag, subtensor, layout, "getDoubleSubtensor",
    ...
    {
        return getTSubtensor<float>(fixedDims, fixedDimNums, rangeDimIdx, rangeDimNum, rwflag, subtensor, layout, "getFloatSubtensor",
    ...
    {
        return getTSubtensor<int>(fixedDims, fixedDimNums, rangeDimIdx, rangeDimNum, rwflag, subtensor, layout, "getIntSubtensor",
                                  "([JJJLjava/nio/ByteBuffer;)Ljava/nio/IntBuffer;");
    }
    ...
    services::Status getTSubtensor(size_t fixedDims, const size_t * fixedDimNums, size_t rangeDimIdx, size_t rangeDimNum, ReadWriteMode rwFlag,
                                   SubtensorDescriptor<T> & subtensor, const TensorOffsetLayout & layout, const char * javaMethodName,
                                   const char * javaMethodSignature)
    {
        ...
        local_tls.jbuf =
            (local_tls.jenv)->CallObjectMethod(jJavaTensor, jmeth, jFixedDimNums, (jlong)rangeDimIdx, (jlong)rangeDimNum, local_tls.jbuf);
    ...
    DAAL_DEPRECATED_VIRTUAL virtual TensorPtr getSampleTensor(size_t firstDimIndex) DAAL_C11_OVERRIDE { return TensorPtr(); }

    virtual services::Status check(const char * description) const DAAL_C11_OVERRIDE { return services::Status(); }

    virtual TensorOffsetLayout createDefaultSubtensorLayout() const DAAL_C11_OVERRIDE { return TensorOffsetLayout(_layout); }
    ...
    virtual jobject getJavaObject() const DAAL_C11_OVERRIDE { return jJavaTensor; }
