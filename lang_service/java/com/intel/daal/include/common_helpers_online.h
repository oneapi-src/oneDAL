...
struct jniOnline
{
    ...
    {
        if (method == head)
            return jniOnline<_Method, _AlgClass, head>::newObj(prec, method, args...);
        else
            return jniOnline<_Method, _AlgClass, values...>::newObj(prec, method, args...);
    }

    static jlong getParameter(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniOnline<_Method, _AlgClass, head>::getParameter(prec, method, algAddr);
        else
            return jniOnline<_Method, _AlgClass, values...>::getParameter(prec, method, algAddr);
    }

    static jlong getInput(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniOnline<_Method, _AlgClass, head>::getInput(prec, method, algAddr);
        else
            return jniOnline<_Method, _AlgClass, values...>::getInput(prec, method, algAddr);
    }
    ...
    {
        if (method == head)
            jniOnline<_Method, _AlgClass, head>::template setInput<_Input>(prec, method, algAddr, inputAddr);
        else
            jniOnline<_Method, _AlgClass, values...>::template setInput<_Input>(prec, method, algAddr, inputAddr);
    }

    static jlong getResult(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniOnline<_Method, _AlgClass, head>::getResult(prec, method, algAddr);
        else
            return jniOnline<_Method, _AlgClass, values...>::getResult(prec, method, algAddr);
    }
    ...
    {
        if (method == head)
            jniOnline<_Method, _AlgClass, head>::template setResult<_Result>(prec, method, algAddr, resAddr);
        else
            jniOnline<_Method, _AlgClass, values...>::template setResult<_Result>(prec, method, algAddr, resAddr);
    }

    static jlong getPartialResult(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniOnline<_Method, _AlgClass, head>::getPartialResult(prec, method, algAddr);
        else
            return jniOnline<_Method, _AlgClass, values...>::getPartialResult(prec, method, algAddr);
    }
    ...
    {
        if (method == head)
            jniOnline<_Method, _AlgClass, head>::template setPartialResult<_PartialResult>(prec, method, algAddr, presAddr, args...);
        else
            jniOnline<_Method, _AlgClass, values...>::template setPartialResult<_PartialResult>(prec, method, algAddr, presAddr, args...);
    }
    ...
    {
        if (method == head)
            jniOnline<_Method, _AlgClass, head>::template setPartialResultImpl<_PartialResult>(prec, method, algAddr, presAddr, args...);
        else
            jniOnline<_Method, _AlgClass, values...>::template setPartialResultImpl<_PartialResult>(prec, method, algAddr, presAddr, args...);
    }

    static jlong getClone(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniOnline<_Method, _AlgClass, head>::getClone(prec, method, algAddr);
        else
            return jniOnline<_Method, _AlgClass, values...>::getClone(prec, method, algAddr);
    }
};
...
struct jniOnline<_Method, _AlgClass, head>
{
    ...
    static jlong getParameter(jint prec, int method, jlong algAddr)
    {
        if (method == head)
        {
            ...
            {
                return (jlong)
                       & (staticPointerCast<_AlgClass<double, (_Method)head>, AlgorithmIface>(*(SharedPtr<AlgorithmIface> *)algAddr))->parameter;
            }
            else
            {
                return (jlong)
                       & (staticPointerCast<_AlgClass<float, (_Method)head>, AlgorithmIface>(*(SharedPtr<AlgorithmIface> *)algAddr))->parameter;
            }
        }
        return 0;
    }
    ...
    static jlong getPartialResult(jint prec, int method, jlong algAddr)
    {
        ...
        if (method == head)
        {
            if (prec == 0)
            {
                *ptr = staticPointerCast<_AlgClass<double, (_Method)head>, AlgorithmIface>(*(SharedPtr<AlgorithmIface> *)algAddr)->getPartialResult();
            }
            else
            {
                *ptr = staticPointerCast<_AlgClass<float, (_Method)head>, AlgorithmIface>(*(SharedPtr<AlgorithmIface> *)algAddr)->getPartialResult();
            }
        }
        return (jlong)ptr;
    }
