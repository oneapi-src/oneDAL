...
struct jniDistributed
{
    ...
    {
        if( method==head ) return jniDistributed<_Step, _Method, _AlgClass, head     >::getBaseParameter( prec, method, algAddr );
        else               return jniDistributed<_Step, _Method, _AlgClass, values...>::getBaseParameter( prec, method, algAddr );
    }
