...
struct jniDistributed
{
    ...
    {
        if (method == head)
            return jniDistributed<_Step, _Method, _AlgClass, head>::newObj(prec, method, args...);
        else
            return jniDistributed<_Step, _Method, _AlgClass, values...>::newObj(prec, method, args...);
    }

    static jlong getParameter(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniDistributed<_Step, _Method, _AlgClass, head>::getParameter(prec, method, algAddr);
        else
            return jniDistributed<_Step, _Method, _AlgClass, values...>::getParameter(prec, method, algAddr);
    }

    static jlong getBaseParameter(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniDistributed<_Step, _Method, _AlgClass, head>::getBaseParameter(prec, method, algAddr);
        else
            return jniDistributed<_Step, _Method, _AlgClass, values...>::getBaseParameter(prec, method, algAddr);
    }

    static jlong getInput(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniDistributed<_Step, _Method, _AlgClass, head>::getInput(prec, method, algAddr);
        else
            return jniDistributed<_Step, _Method, _AlgClass, values...>::getInput(prec, method, algAddr);
    }
    ...
    {
        if (method == head)
            jniDistributed<_Step, _Method, _AlgClass, head>::template setInput<_Input>(prec, method, algAddr, inputAddr);
        else
            jniDistributed<_Step, _Method, _AlgClass, values...>::template setInput<_Input>(prec, method, algAddr, inputAddr);
    }

    static jlong getResult(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniDistributed<_Step, _Method, _AlgClass, head>::getResult(prec, method, algAddr);
        else
            return jniDistributed<_Step, _Method, _AlgClass, values...>::getResult(prec, method, algAddr);
    }
    ...
    {
        if (method == head)
            jniDistributed<_Step, _Method, _AlgClass, head>::template setResult<_Result>(prec, method, algAddr, resAddr);
        else
            jniDistributed<_Step, _Method, _AlgClass, values...>::template setResult<_Result>(prec, method, algAddr, resAddr);
    }

    static jlong getPartialResult(jint prec, int method, jlong algAddr)
    {
        if (method == head)
            return jniDistributed<_Step, _Method, _AlgClass, head>::getPartialResult(prec, method, algAddr);
        else
            return jniDistributed<_Step, _Method, _AlgClass, values...>::getPartialResult(prec, method, algAddr);
    }
    ...
    {
        if (method == head)
            jniDistributed<_Step, _Method, _AlgClass, head>::template setPartialResult<_PartialResult>(prec, method, algAddr, presAddr, args...);
        else
            jniDistributed<_Step, _Method, _AlgClass, values...>::template setPartialResult<_PartialResult>(prec, method, algAddr, presAddr, args...);
    }
    ...
    {
        if (method == head)
            jniDistributed<_Step, _Method, _AlgClass, head>::template setPartialResultImpl<_PartialResult>(prec, method, algAddr, presAddr, args...);
        else
            jniDistributed<_Step, _Method, _AlgClass, values...>::template setPartialResultImpl<_PartialResult>(prec, method, algAddr, presAddr,
                                                                                                                args...);
    }
    ...
    {
        if (method == head)
            return jniDistributed<_Step, _Method, _AlgClass, head>::getClone(prec, method, algAddr);
        else
            return jniDistributed<_Step, _Method, _AlgClass, values...>::getClone(prec, method, algAddr);
    }
};
