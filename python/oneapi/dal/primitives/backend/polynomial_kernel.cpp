...
ONEDAL_PARAM_DISPATCH_SECTION_START(std::string, method) {
    ONEDAL_PARAM_DISPATCH_VALUE("dense", polynomial_kernel::method::dense)
...
auto get_descriptor(const py::dict& params) {
    ...
    return descriptor<Float, Method, Task>{}
        .set_scale(params["scale"].cast<double>())
...
TYPE2STR(polynomial_kernel::compute_result<polynomial_kernel::task::compute>, "");
TYPE2STR(polynomial_kernel::compute_input<polynomial_kernel::task::compute>, "");
TYPE2STR(dal::detail::host_policy, "");
TYPE2STR(dal::detail::data_parallel_policy, "");

void init_polynomial_kernel(py::module_& m) {
    using namespace dal::detail;
    using namespace dal::polynomial_kernel;
