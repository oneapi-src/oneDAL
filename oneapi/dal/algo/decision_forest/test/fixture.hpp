...
template <typename T>
struct checker_info {
    typedef T (*checker_func)(const dal::table& infer_responses, const dal::table& ground_truth);
...
public:
    ...
    static double calculate_mse(const dal::table& infer_responses, const dal::table& ground_truth) {
        double mean = 0.0;
        const auto responses = dal::row_accessor<const Float>(infer_responses).pull();
        const auto truth_responses = dal::row_accessor<const Float>(ground_truth).pull();
        for (std::int64_t i = 0; i < responses.get_count(); i++) {
    ...
    static double calculate_mae(const dal::table& infer_responses, const dal::table& ground_truth) {
        double mae = 0.0;
        const auto responses = dal::row_accessor<const Float>(infer_responses).pull();
        const auto truth_responses = dal::row_accessor<const Float>(ground_truth).pull();
