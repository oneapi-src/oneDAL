...
def _demo_rule_impl(ctx):
    ...
    ctx.actions.write(
        output=out,
        content="""\
    export CMAKE_PREFIX_PATH="`pwd`/daal/latest":$CMAKE_PREFIX_PATH
    export CPATH="`pwd`/daal/latest/include":$CPATH
    export DALROOT="`pwd`/daal/latest"
    export LD_LIBRARY_PATH="`pwd`/daal/latest/lib/intel64":$LD_LIBRARY_PATH
    export LIBRARY_PATH="`pwd`/daal/latest/lib/intel64":$LIBRARY_PATH
    """,
    )
    return [out]
