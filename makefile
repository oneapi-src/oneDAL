...
TBBDIR.libia.win.vc1  := $(if $(OS_is_win),$(if $(wildcard $(call frompf1,$(TBBDIR.libia.prefix))/$(_IA)/vc14),$(TBBDIR.libia.prefix)/$(_IA)/vc14,$(if $(wildcard $(call frompf1,$(TBBDIR.libia.prefix))/$(_IA)/vc_mt),$(TBBDIR.libia.prefix)/$(_IA)/vc_mt)))
...
TBBDIR.libia.win:= $(if $(OS_is_win),$(if $(TBBDIR.libia.win.vc22),$(TBBDIR.libia.win.vc2),$(if $(TBBDIR.libia.win.vc1),$(TBBDIR.libia.win.vc1),$(error Can`t find TBB libs nether in $(call frompf,$(TBBDIR.libia.prefix))/$(_IA)/vc14 not in $(firstword $(filter $(TBBROOT)%,$(subst ;,$(space),$(LIB)))).))))
...
RELEASEDIR.tbb.libia := $(RELEASEDIR.tbb)/lib$(if $(OS_is_mac),,/$(_IA)$(if $(OS_is_win),/vc14,/$(TBBDIR.libia.lnx.gcc)))
RELEASEDIR.tbb.soia  := $(if $(OS_is_win),$(RELEASEDIR.tbb)/redist/$(_IA)/vc14,$(RELEASEDIR.tbb.libia))
...
daaldep.win32e.vml :=
daaldep.win32e.ipp :=
daaldep.win32e.rt.thr  := -LIBPATH:$(RELEASEDIR.tbb.libia) tbb.lib tbbmalloc.lib msvcprt.lib msvcrt.lib $(if $(CHECK_DLL_SIG),Wintrust.lib)
daaldep.win32e.rt.seq  := msvcprt.lib msvcrt.lib $(if $(CHECK_DLL_SIG),Wintrust.lib)
