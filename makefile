...
ifdef OS_is_win
  ifdef IA_is_intel64
    $(WORKDIR.lib)/$(thr_tbb_y): LOPT += -DEF:$(THR.srcdir)/export_lnx32e.def
  else
  ...
  ifdef OS_is_lnx
    ...
    ifdef OS_is_fbsd
      ifdef IA_is_intel64
        $(WORKDIR.lib)/$(thr_tbb_y): LOPT += $(addprefix -Wl$(comma)-u -Wl$(comma),$(shell grep -v -E '^(EXPORTS|;)' $(THR.srcdir)/export_lnx32e.def))
      else
...
ifdef OS_is_win
  ifdef IA_is_intel64
    $(WORKDIR.lib)/$(thr_seq_y): LOPT += -DEF:$(THR.srcdir)/export_lnx32e.def
  else
  ...
  ifdef OS_is_lnx
    ...
    ifdef OS_is_fbsd
      ifdef IA_is_intel64
        $(WORKDIR.lib)/$(thr_seq_y): LOPT += $(addprefix -Wl$(comma)-u -Wl$(comma),$(shell grep -v -E '^(EXPORTS|;)' $(THR.srcdir)/export_lnx32e.def))
      else
