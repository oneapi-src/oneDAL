...
USECPUS.files := $(subst sse2,nrh,$(subst ssse3,mrm,$(subst sse42,neh,$(subst avx,snb,$(subst avx2,hsw,$(subst avx512,skx,$(subst avx512_mic,,$(USECPUS))))))))
...
cpy = cp -fp "$<" "$@"
dlinkopt = $(subst LIB_MAJOR_VER,$(MAJORBINARY),$(subst LIB_SUFFIX,$(1),$(2)))
...
$(WORKDIR.lib)/$(core_y):                   $(daaldep.ipp) $(daaldep.vml) $(daaldep.mkl) \
                                            ...
                                            $(CORE.tmpdir_y)/$(core_y:%.$y=%_link.txt) ; $(call dlinkopt,onedal_core,$(LINK.DYNAMIC)) ; $(LINK.DYNAMIC.POST)
...
$(WORKDIR.lib)/$(oneapi_y): \
    ...
    $(ONEAPI.tmpdir_y)/$(oneapi_y:%.$y=%_link.txt) ; $(call dlinkopt,onedal,$(LINK.DYNAMIC)) ; $(LINK.DYNAMIC.POST)
...
$(WORKDIR.lib)/$(oneapi_y.dpc): \
    $(daaldep.ipp) $(daaldep.vml) $(daaldep.mkl) \
    $(ONEAPI.tmpdir_y.dpc)/$(oneapi_y.dpc:%.$y=%_link.txt) ; $(call dlinkopt,onedal_dpc,$(DPC.LINK.DYNAMIC)) ; $(LINK.DYNAMIC.POST)
...
$(WORKDIR.lib)/$(thr_tbb_y): $(THR_TBB.objs_y) $(daaldep.mkl.thr) $(daaldep.mkl) $(if $(OS_is_win),$(THR.tmpdir_y)/dll_tbb.res,) \
   $(THR.tmpdir_y)/$(thr_tbb_y:%.$y=%_link.def) ; $(call dlinkopt,onedal_thread,$(LINK.DYNAMIC)) ; $(LINK.DYNAMIC.POST)
...
$(WORKDIR.lib)/$(jni_so):                $(JNI.tmpdir)/$(jni_so:%.$y=%_link.txt); $(call dlinkopt,JavaAPI,$(LINK.DYNAMIC)) ; $(LINK.DYNAMIC.POST)
