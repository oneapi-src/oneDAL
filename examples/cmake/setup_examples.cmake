...
function (set_link_type)
    if ("${ONEDAL_TARGET_LINK}" STREQUAL "static")
...
function (set_common_compiler_options)
    ...
    if (ONEDAL_USE_NEW_IFACES STREQUAL "no")
        if (UNIX)
...
function (add_examples examples_paths)
    foreach(example_file_path ${examples_paths})
        ...
        if (UNIX AND NOT APPLE)
            ONEDAL_target_link_libraries(${example} PRIVATE -Wl,--start-group ${oneDAL_IMPORTED_TARGETS} -Wl,--end-group)
        else()
            ONEDAL_target_link_libraries(${example} PRIVATE ${oneDAL_IMPORTED_TARGETS})
        endif()
        target_compile_options(${example} PRIVATE ${ONEDAL_CUSTOM_COMPILE_OPTIONS})
        ONEDAL_target_link_options(${example} PRIVATE ${ONEDAL_CUSTOM_LINK_OPTIONS})
