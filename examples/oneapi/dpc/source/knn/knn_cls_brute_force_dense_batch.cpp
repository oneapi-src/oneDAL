...
void run(sycl::queue& queue) {
    ...
    const auto x_train_table = dal::homogen_table{ queue, row_count, column_count, x_train, dal::make_default_delete<const float>(queue) };

    auto y_train = sycl::malloc_shared<float>(row_count * 1, queue);
    queue.memcpy(y_train, y_train_host, sizeof(float) * row_count * 1).wait();
    const auto y_train_table = dal::homogen_table{ queue, row_count, 1, y_train, dal::make_default_delete<const float>(queue) };
    ...
    const auto x_test_table = dal::homogen_table{ queue, row_count, column_count, x_test, dal::make_default_delete<const float>(queue) };

    auto y_test = sycl::malloc_shared<float>(row_count * 1, queue);
    queue.memcpy(y_test, y_test_host, sizeof(float) * row_count * 1).wait();
    const auto y_test_table = dal::homogen_table{ queue, row_count, 1, y_test, dal::make_default_delete<const float>(queue) };
