...
void run(sycl::queue &queue) {
  ...
  const auto result_train =
      dal::train(queue, svm_desc, x_train_table, y_train_table);
  ...
  auto x_test =
      sycl::malloc_shared<float>(row_count_test * column_count, queue);
  queue
      .memcpy(x_test, x_test_host,
              sizeof(float) * row_count_test * column_count)
      .wait();

  const auto x_test_table =
      dal::homogen_table{queue, row_count_test, column_count, x_test};
  const auto y_true_table = dal::homogen_table{row_count_test, 1, y_true_host};

  const auto result_test =
      dal::infer(queue, svm_desc, result_train.get_model(), x_test_table);
