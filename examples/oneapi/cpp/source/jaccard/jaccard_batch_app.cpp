...
// input parameters - processed graph, size of block by row, size of block by column
// algorithm process only upper triangular matrix
template<class Graph> 
void vertex_similarity_block_processing(const Graph &g, const int32_t row_size, const int32_t column_size) 
{
  ...
  std::vector< std::vector<byte_t>> processing_blocks(tbb::this_task_arena::max_concurrency(), std::vector<byte_t> (block_result_size)); // memory for all threads
  ...
  tbb::parallel_for(
      ...
      [&](const tbb::blocked_range<int> &r) {
        for (int i = r.begin(); i != r.end(); ++i) {
          ...
          int32_t begin_column = 1 + block_begin_row; // start column ranges from diagonal
          int32_t columns_count = (vertex_count - begin_column) / column_size; // the number columns
          ...
          tbb::parallel_for(
              ...
              [&](const tbb::blocked_range<int> &inner_r) {
                for (int j = inner_r.begin(); j != inner_r.end(); ++j) {
                  ...
                  int32_t block_end_column = begin_column + (j + 1) * column_size;
                  //set block ranges
                  const auto jaccard_desc_default = jaccard::descriptor<>().set_block(
                  {block_begin_row, std::min(block_end_row, vertex_count)},
                  {block_begin_column, std::min(block_end_column, vertex_count)});
                  //compute coeffs
                  vertex_similarity(jaccard_desc_default, g,
                        static_cast<void *>((processing_blocks[tbb::this_task_arena::current_thread_index()]).data()));
