...
int main(int argc, char **argv) {
  ...
  const auto graph = load_graph::load(d, ds);

  // set the block sizes for Jaccard similarity block processing
  const int32_t block_row_count = 2;
  const int32_t block_column_count = 5;

  // set the number of threads
  const int32_t tbb_threads_number = 4;
...
void vertex_similarity_block_processing(const Graph &g,
  ...
  const int32_t vertex_count = get_vertex_count(g);
  ...
  tbb::parallel_for(
      ...
      [&](const tbb::blocked_range<int32_t> &r) {
        for (int32_t i = r.begin(); i != r.end(); ++i) {
          ...
          const int32_t row_range_begin = i * block_row_count;
          const int32_t row_range_end   = (i + 1) * block_row_count;

          // start column ranges from diagonal
          const int32_t column_begin = 1 + row_range_begin;
          ...
          tbb::parallel_for(
              ...
              [&](const tbb::blocked_range<int32_t> &inner_r) {
                for (int32_t j = inner_r.begin(); j != inner_r.end(); ++j) {
                  ...
                  const int32_t column_range_begin = column_begin + j * block_column_count;
                  const int32_t column_range_end   = column_begin + (j + 1) * block_column_count;
