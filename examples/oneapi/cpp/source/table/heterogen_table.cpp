...
dal::array<Type> get_arange(std::int64_t count, 
    ...
    return dal::array<Type>(raw_data, count, //
        [](Type* const ptr) -> void { delete[] ptr; });
}

template <typename Type = float>
dal::chunked_array<Type> get_chunked_arange(std::int64_t count, 
    ...
    for (std::int64_t i = 0l; i != chunk_count; ++i) {
        ...
        std::int64_t local_count = (i + 1  == chunk_count) 
                            ? (count - first) : min_count;
