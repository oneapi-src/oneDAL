...
template <typename type1, typename type2>
void printTensors(TensorPtr dataTable1, TensorPtr dataTable2, const char * title1 = "", const char * title2 = "", const char * message = "",
                  size_t nPrintedRows = 0, size_t interval = 15)
{
    ...
    if (nPrintedRows != 0)
    {
        nPrintedRows = std::min(nRows1, nPrintedRows);
    }
    else
    {
        nPrintedRows = nRows1;
    ...
    if (nPrintedRows != 0)
    {
        nPrintedRows = std::min(nRows2, nPrintedRows);
    }
    else
    {
        nPrintedRows = nRows2;
...
void printTensor3d(TensorPtr dataTable, const char * message = "", size_t nFirstDim = 0, size_t nSecondDim = 0, size_t interval = 10)
{
    const daal::services::Collection<size_t> & dims = dataTable->getDimensions();
    size_t nRows                                    = dims[0];
    size_t nCols                                    = dims[1];
