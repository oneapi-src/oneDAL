...
int main(int argc, char* argv[]) {
    ...
    for (const auto& deviceSelector : getListOfDevices()) {
        ...
        NumericTablePtr data =
            SyclHomogenNumericTable<>::create(nFeatures, 0, NumericTable::doNotAllocate, &s);
        checkStatus(s);
        NumericTablePtr dependentVariables =
            SyclHomogenNumericTable<>::create(1, 0, NumericTable::doNotAllocate, &s);
        checkStatus(s);
        ...
        optimization_solver::sgd::Batch<float, optimization_solver::sgd::miniBatch> sgdAlgorithm(
            logLoss);
        ...
        printNumericTable(
            sgdAlgorithm.getResult()->get(optimization_solver::iterative_solver::minimum),
            "Minimum:");
        printNumericTable(
