...
int main()
{
    ...
    for (const auto & deviceSelector : getListOfDevices())
    {
        ...
        SyclCSRNumericTablePtr dataTable =
            SyclCSRNumericTable::create<float>(cl::sycl::buffer<float>(values, nNotZeroes), cl::sycl::buffer<size_t>(colIndices, nNotZeroes),
                                               cl::sycl::buffer<size_t>(rowOffsets, nObservations + 1), nFeatures, nObservations);
