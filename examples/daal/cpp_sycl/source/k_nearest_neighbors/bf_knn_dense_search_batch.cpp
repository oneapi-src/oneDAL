...
void trainModel(bf_knn_classification::training::ResultPtr &trainingResult) {
  ...
  NumericTablePtr trainData = SyclHomogenNumericTable<>::create(
      nFeatures, 0, NumericTable::notAllocate);
...
void testModel(bf_knn_classification::training::ResultPtr &trainingResult,
  ...
  NumericTablePtr testData = SyclHomogenNumericTable<>::create(
      nFeatures, 0, NumericTable::doNotAllocate);
  NumericTablePtr testGroundTruth =
      SyclHomogenNumericTable<>::create(1, 0, NumericTable::doNotAllocate);
...
void readGroundTruth(NumericTablePtr &groundTruthIndices,
  ...
  /* Create Numeric Tables for testing data and labels */
  groundTruthIndices = SyclHomogenNumericTable<>::create(
      kNeighbors, 0, NumericTable::doNotAllocate);
  groundTruthDistances = SyclHomogenNumericTable<>::create(
      kNeighbors, 0, NumericTable::doNotAllocate);
...
void printDistancesResults(
  ...
  printNumericTables<float, float>(
      ...
      "Ground truth", "Distances results",
      "BF kNN search results (first 20 observations):", 20);
}
