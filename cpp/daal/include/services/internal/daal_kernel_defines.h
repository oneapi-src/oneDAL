...
#define DAAL_KERNEL_SSE2_CONTAINER_CASE(ContainerTemplate, ...) DAAL_KERNEL_CONTAINER_CASE(ContainerTemplate, sse2, __VA_ARGS__)
    ...
    #define DAAL_KERNEL_AVX512_MIC_CONTAINER(ContainerTemplate, ...) , DAAL_KERNEL_CONTAINER_TEMPL(ContainerTemplate, avx512, __VA_ARGS__)
    ...
    case avx512_mic:                                                                                \
        _cntr = (new DAAL_KERNEL_CONTAINER_TEMPL(ContainerTemplate, avx512, __VA_ARGS__)(daalEnv)); \
        break;
    #define DAAL_KERNEL_AVX512_MIC_CONTAINER_CASE_SYCL(ContainerTemplate, ...)                     \
    case avx512_mic:                                                                               \
    {                                                                                              \
        using contTemplType = DAAL_KERNEL_CONTAINER_TEMPL(ContainerTemplate, avx512, __VA_ARGS__); \
        static volatile daal::services::internal::GpuSupportRegistrar<contTemplType> registrar;    \
        _cntr = (new contTemplType(daalEnv));                                                      \
        break;                                                                                     \
    }
