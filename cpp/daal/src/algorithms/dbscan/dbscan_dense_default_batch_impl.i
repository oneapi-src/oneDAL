...
template <typename algorithmFPType, Method method, CpuType cpu>
Status DBSCANBatchKernel<algorithmFPType, method, cpu>::processNeighborhoodParallel(size_t clusterId, int * const assignments,
                                                                            const Neighborhood<algorithmFPType, cpu> & neigh,
                                                                            daal::tls<Queue<size_t, cpu> *>& tls,
                                                                            TArray<Neighborhood<algorithmFPType, cpu>, cpu>& neighs,
                                                                            algorithmFPType minObservations,
                                                                            int * const isCore,
                                                                            size_t nestedLevel)
{
    ...
    daal::threader_for(nBlocks, nBlocks, [&](size_t iBlock)
    {
        ...
        while (!qu.empty())
        {
            ...
            {
                DAAL_CHECK_STATUS_THR(processNeighborhoodParallel(clusterId, assignments, curNeigh, tls, neighs,
                                      minObservations, isCore, nestedLevel + 1));
            }
            else
            {
