...
BatchContainer<algorithmFPType, method, cpu>::BatchContainer(
  ...
  if (deviceInfo.isCpu) {
    __DAAL_INITIALIZE_KERNELS(internal::KMeansBatchKernel, method,
                              algorithmFPType);
  } else {
...
services::Status BatchContainer<algorithmFPType, method, cpu>::compute() {
  ...
  NumericTable *a[lastInputId + 1] = { input->get(data).get(),
                                       input->get(inputCentroids).get() };

  NumericTable *r[lastResultId + 1] = { result->get(centroids).get(),
  ...
  if (deviceInfo.isCpu || method != lloydDense) {
    __DAAL_CALL_KERNEL(env, internal::KMeansBatchKernel,
                       __DAAL_KERNEL_ARGUMENTS(method, algorithmFPType),
                       compute, a, r, par);
  } else {
    return ((internal::KMeansDenseLloydBatchKernelUCAPI<
                algorithmFPType> *)(_kernel))->compute(a, r, par);
  }
}

template <typename algorithmFPType, Method method, CpuType cpu>
DistributedContainer<
    step1Local, algorithmFPType, method,
    cpu>::DistributedContainer(daal::services::Environment::env *daalEnv) {
  auto &context =
      services::Environment::getInstance()->getDefaultExecutionContext();
  auto &deviceInfo = context.getInfoDevice();

  if (deviceInfo.isCpu) {
    __DAAL_INITIALIZE_KERNELS(internal::KMeansDistributedStep1Kernel, method,
                              algorithmFPType);
  } else {
    _kernel =
        new internal::KMeansDistributedStep1KernelUCAPI<algorithmFPType>();
  }
}

template <typename algorithmFPType, Method method, CpuType cpu>
DistributedContainer<step1Local, algorithmFPType, method,
                     cpu>::~DistributedContainer() {
  __DAAL_DEINITIALIZE_KERNELS();
}

template <typename algorithmFPType, Method method, CpuType cpu>
services::Status
DistributedContainer<step1Local, algorithmFPType, method, cpu>::compute() {
  Input *input = static_cast<Input *>(_in);
  ...
  const size_t isAssignments =
      par->resultsToEvaluate & computeAssignments || par->assignFlag;
  ...
  r[3] =
      static_cast<NumericTable *>(pres->get(partialCandidatesDistances).get());
  r[4] =
      static_cast<NumericTable *>(pres->get(partialCandidatesCentroids).get());
  if (isAssignments) {
  ...
  if (deviceInfo.isCpu) {
    ...
    return ((internal::KMeansDistributedStep1KernelUCAPI<
                algorithmFPType> *)(_kernel))->compute(na, a, nr, r, par);
  }
}
...
services::Status DistributedContainer<step1Local, algorithmFPType, method,
  ...
  if (deviceInfo.isCpu) {
    __DAAL_CALL_KERNEL(env, internal::KMeansDistributedStep1Kernel,
                       __DAAL_KERNEL_ARGUMENTS(method, algorithmFPType),
                       finalizeCompute, na, a, nr, r, par);
  } else {
    return ((internal::KMeansDistributedStep1KernelUCAPI<
                algorithmFPType> *)(_kernel))
        ->finalizeCompute(na, a, nr, r, par);
  }
}
...
template <typename algorithmFPType, Method method, CpuType cpu>
DistributedContainer<
    step2Master, algorithmFPType, method,
    cpu>::DistributedContainer(daal::services::Environment::env *daalEnv) {
  auto &context =
      services::Environment::getInstance()->getDefaultExecutionContext();
  auto &deviceInfo = context.getInfoDevice();
  ...
  if (deviceInfo.isCpu) {
    __DAAL_INITIALIZE_KERNELS(internal::KMeansDistributedStep2Kernel, method,
                              algorithmFPType);
  } else {
    _kernel =
        new internal::KMeansDistributedStep2KernelUCAPI<algorithmFPType>();
  }
}

template <typename algorithmFPType, Method method, CpuType cpu>
DistributedContainer<step2Master, algorithmFPType, method,
                     cpu>::~DistributedContainer() {
  __DAAL_DEINITIALIZE_KERNELS();
}
...
template <typename algorithmFPType, Method method, CpuType cpu>
services::Status
DistributedContainer<step2Master, algorithmFPType, method, cpu>::compute() {
  DistributedStep2MasterInput *input =
      static_cast<DistributedStep2MasterInput *>(_in);
  ...
  for (size_t i = 0; i < nPartials; i++) {
    ...
    a[i * 5 + 0] =
        static_cast<NumericTable *>(inPres->get(nObservations).get());
    a[i * 5 + 1] = static_cast<NumericTable *>(inPres->get(partialSums).get());
    a[i * 5 + 2] = static_cast<NumericTable *>(
        inPres->get(partialObjectiveFunction).get());
    a[i * 5 + 3] = static_cast<NumericTable *>(
        inPres->get(partialCandidatesDistances).get());
    a[i * 5 + 4] = static_cast<NumericTable *>(
        inPres->get(partialCandidatesCentroids).get());
  }
  ...
  if (deviceInfo.isCpu) {
    s = __DAAL_CALL_KERNEL_STATUS(
        env, internal::KMeansDistributedStep2Kernel,
        __DAAL_KERNEL_ARGUMENTS(method, algorithmFPType), compute, na, a, nr, r,
        par);
  } else {
    s = ((internal::KMeansDistributedStep2KernelUCAPI<
             algorithmFPType> *)(_kernel))->compute(na, a, nr, r, par);
  }
  dcInput->clear();
  return s;
...
template <typename algorithmFPType, Method method, CpuType cpu>
services::Status DistributedContainer<step2Master, algorithmFPType, method,
                                      cpu>::finalizeCompute() {
  PartialResult *pres = static_cast<PartialResult *>(_pres);
  Result *result = static_cast<Result *>(_res);
  ...
  if (deviceInfo.isCpu) {
    ...
    return ((internal::KMeansDistributedStep2KernelUCAPI<
                algorithmFPType> *)(_kernel))
        ->finalizeCompute(na, a, nr, r, par);
  }
}
