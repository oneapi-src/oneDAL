/* file: kernel_function_sigmoid_csr_fast_impl.i */
/*******************************************************************************
...
#ifndef __KERNEL_FUNCTION_SIGMOID_CSR_FAST_IMPL_I__
#define __KERNEL_FUNCTION_SIGMOID_CSR_FAST_IMPL_I__
    ...
    if (a1 == a2)
    {
        ...
        if (k != one || b != zero)
        {
            daal::threader_for_optional(nVectors1, nVectors1, [=](size_t i) {
                ...
                {
                    dataR[i * nVectors1 + j] *= k;
                    dataR[i * nVectors1 + j] += b;
                }
            ...
            if (!isSOARes)
            {
                ...
                if (k != one || b != zero)
                {
                    ...
                    for (size_t i = 0; i < nRowsInBlock1; ++i)
                    {
                        ...
                        {
                            dataR[i * ldc + j + startRow2] *= k;
                            dataR[i * ldc + j + startRow2] += b;
                        }
