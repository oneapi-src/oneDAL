...
{
public:
    GetView(algorithmFPAccessType* dataStart, size_t ld, size_t numRows, size_t numCols) : _dataStart(dataStart), _ld(ld), _numRows(numRows), _numCols(numCols) {}
...
public:
    GetDataBlocker(NumericTableType * data, size_t nRowBlocks, size_t nRowsInBlock, size_t nColBlocks, size_t nColsInBlock) : _data(data),
        _nRowBlocks(nRowBlocks), _nRowsInBlock(nRowsInBlock), _nColBlocks(nColBlocks), _nColsInBlock(nColsInBlock) {
        initialize();
    }
    ...
    GetView<algorithmFPType, algorithmFPAccessType, cpu, mode, NumericTableType> getView(size_t rowBlockIndex, size_t colBlockIndex) {
        ...
        if (_isRows) {
            ...
            return GetView<algorithmFPType, algorithmFPAccessType, cpu, mode, NumericTableType>(_dataStarts[rowBlockIndex] + startColumn, _ld, nRowsInBlock, nColsInBlock);
        } else {
            return GetView<algorithmFPType, algorithmFPAccessType, cpu, mode, NumericTableType>(_dataStarts[colBlockIndex] + startRow, _ld, nRowsInBlock, nColsInBlock);
        }
    }

    const services::Status& status() {
        return _status;
    }
    ...
    size_t getNumRowsInBlock() {
        return _nRowsInBlock;
    }

    size_t getNumColsInBlock() {
        return _nColsInBlock;
    }
...
private:
    ...
    void initialize() {
        ...
        algorithmFPAccessType** dataStarts = static_cast<algorithmFPAccessType**>(daal::services::internal::service_malloc<algorithmFPAccessType*, cpu>(startsSize)); // TODO allocate more memory to eliminate cache false sharings in threading version
