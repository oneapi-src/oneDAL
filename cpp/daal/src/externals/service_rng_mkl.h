...
int uniformRNG(const size_t cn, size_t * r, void * stream, const size_t a, const size_t b, const int method)
{
    ...
    if (sizeof(size_t) == sizeof(unsigned int))
    {
        ...
        int nn     = (int)n;
        int * rr   = (int *)r;
        __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, iRngUniform, ((const MKL_INT)method, stream, (const MKL_INT)nn, rr, na, nb), errcode);
    ...
    else
    {
        ...
        else
        {
            ...
            if (MAX == 0)
            {
                ...
                __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, iRngUniformBits64,
...
int uniformRNG(const size_t n, int * r, void * stream, const int a, const int b, const int method)
{
    ...
    int nn      = (int)n;
    int * rr    = r;
    __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, iRngUniform, ((const MKL_INT)method, stream, (const MKL_INT)nn, rr, a, b), errcode);
    return errcode;
}
...
int uniformRNG(const size_t n, float * r, void * stream, const float a, const float b, const int method)
{
    ...
    int nn      = (int)n;
    float * rr  = r;
    __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, sRngUniform, ((const MKL_INT)method, stream, (const MKL_INT)nn, rr, a, b), errcode);
    return errcode;
}
...
int uniformRNG(const size_t n, double * r, void * stream, const double a, const double b, const int method)
{
    ...
    int nn      = (int)n;
    double * rr = r;
    __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, dRngUniform, ((const MKL_INT)method, stream, (const MKL_INT)nn, rr, a, b), errcode);
    return errcode;
}
...
int uniformBits32RNG(const size_t n, unsigned int * r, void * stream, const int method)
{
    ...
    int nn            = (int)n;
    unsigned int * rr = r;
    __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, iRngUniformBits32, ((const MKL_INT)method, stream, (const MKL_INT)nn, rr), errcode);
    return errcode;
}
...
int gaussianRNG(const size_t n, float * r, void * stream, const float a, const float sigma, const int method)
{
    ...
    int nn      = (int)n;
    float * rr  = r;
    __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, sRngGaussian, ((const MKL_INT)method, stream, (const MKL_INT)nn, rr, a, sigma), errcode);
    return errcode;
}
...
int gaussianRNG(const size_t n, double * r, void * stream, const double a, const double sigma, const int method)
{
    ...
    int nn      = (int)n;
    double * rr = r;
    __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, dRngGaussian, ((const MKL_INT)method, stream, (const MKL_INT)nn, rr, a, sigma), errcode);
    return errcode;
}
...
int bernoulliRNG(const size_t n, int * r, void * stream, const double p, const int method)
{
    ...
    int nn      = (int)n;
    int * rr    = r;
    __DAAL_VSLFN_CALL_NR_WHILE(mkl_vsl_kernel, iRngBernoulli, ((const MKL_INT)method, stream, (const MKL_INT)nn, rr, p), errcode);
    return errcode;
}
...
public:
    BaseRNG(const unsigned int seed, const int brngId) : _stream(0), _seed(nullptr), _seedSize(0), _brngId(brngId)
    {
        ...
        if (s)
        {
            _seed[0]    = seed;
            int errcode = 0;
            __DAAL_VSLFN_CALL_NR(mkl_vsl_sub_kernel, vslNewStreamEx, (&_stream, (const MKL_INT)brngId, (const MKL_INT)1, &seed), errcode);
        }
    }
        ...
        : _stream(0), _seed(nullptr), _seedSize(0), _brngId(brngId)
    {
        ...
        if (s)
        {
            ...
            int errcode = 0;
            __DAAL_VSLFN_CALL_NR(mkl_vsl_sub_kernel, vslNewStreamEx, (&_stream, (const MKL_INT)brngId, (const MKL_INT)n, seed), errcode);
        }
    }
    ...
    BaseRNG(const BaseRNG<cpu> & other) : _stream(0), _seed(nullptr), _seedSize(other._seedSize), _brngId(other._brngId)
    {
        ...
        if (s)
        {
            ...
            int errcode = 0;
            __DAAL_VSLFN_CALL_NR(mkl_vsl_sub_kernel, vslNewStreamEx, (&_stream, (const MKL_INT)_brngId, (const MKL_INT)_seedSize, _seed), errcode);
    ...
    {
        int errcode = 0;
        __DAAL_VSLFN_CALL_NR(mkl_vsl_sub_kernel, vslLeapfrogStream, (_stream, (const MKL_INT)threadNum, (const MKL_INT)nThreads), errcode);
        return errcode;
    }
