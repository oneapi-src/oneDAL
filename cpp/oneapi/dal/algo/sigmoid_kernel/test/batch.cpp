...
public:
    ...
    void general_checks(const te::dataframe& x_data,
        ...
        const auto sigmoid_kernel_desc = get_descriptor(scale, shift);

        INFO("run compute");
        const auto compute_result = this->compute(sigmoid_kernel_desc, x, y);
    ...
    void check_compute_result(double scale,
                              ...
                              const table& x_data,
                              const table& y_data,
                              const sigmoid_kernel::compute_result<>& result) {
    ...
    la::matrix<double> compute_reference(double scale,
        ...
        la::enumerate_linear(reference, [&](std::int64_t i, double) {
            ...
            reference.set(i) = std::tanh(reference.set(i));
        });
...
TEMPLATE_LIST_TEST_M(sigmoid_kernel_batch_test,
                     "sigmoid_kernel common flow",
                     "[sigmoid_kernel][integration][batch]",
...
TEMPLATE_LIST_TEST_M(sigmoid_kernel_batch_test,
                     "sigmoid_kernel compute one element matrix",
                     "[sigmoid_kernel][integration][batch]",
                     sigmoid_kernel_types) {
    SKIP_IF(this->not_available_on_device());
    SKIP_IF(this->not_float64_friendly());
