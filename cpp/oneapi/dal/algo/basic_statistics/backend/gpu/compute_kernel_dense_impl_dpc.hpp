...
template <typename Float, bs_list List, bool DefferedFin, bool use_weights>
inline void single_pass_block_processor(const Float* data_ptr,
                                        const Float* weigths_ptr,
                                        Float* rmin_ptr,
                                        Float* rmax_ptr,
                                        Float* rsum_ptr,
                                        Float* rsum2_ptr,
    ...
    if (x < column_count) {
        ...
        for (std::int64_t row = 0; row < row_block_size; ++row) {
            ...
            if constexpr (use_weights) weight = weights_ptr[row];
            const Float el = data_ptr[y + x] * weight;
...
template <typename Float, bs_list List, bool weights>
