...
TEST(svm_bad_arg_tests, set_train_data) {
    ...
    ASSERT_THROW(dal::train(queue, svm_desc, x_train_table, y_train_table), dal::domain_error);
}
...
TEST(svm_bad_arg_tests, set_train_weight) {
    ...
    const auto svm_desc = svm::descriptor{};
    ASSERT_THROW(dal::train(queue, svm_desc, x_train_table, y_train_table, weight_train_table),
                 dal::domain_error);
}
...
TEST(svm_bad_arg_tests, data_rows_matches_labels_rows) {
    ...
    const auto svm_desc = svm::descriptor{};
    ASSERT_THROW(dal::train(queue, svm_desc, x_train_table, y_train_table), dal::invalid_argument);
}
...
TEST(svm_bad_arg_tests, set_infer_data) {
    ...
    ASSERT_THROW(dal::infer(queue, svm_desc, result_train.get_model(), x_test_table),
                 dal::domain_error);
}
