...
TEST(svm_thunder_dense_test, can_classify_linear_separable_surface) {
    ...
    const auto x_train_table = homogen_table{ x_train, row_count_train, column_count, empty_delete<const float>() };
    const auto y_train_table = homogen_table{ y_train, row_count_train, 1, empty_delete<const float>() };
...
TEST(svm_thunder_dense_test, can_classify_linear_separable_surface_with_equal_weights) {
    ...
    const auto x_train_table = homogen_table{ x_train, row_count_train, column_count, empty_delete<const float>() };
    const auto y_train_table = homogen_table{ y_train, row_count_train, 1, empty_delete<const float>() };
    const auto weights_table = homogen_table{ weights, row_count_train, 1, empty_delete<const float>() };
    ...
    const auto x_test_table      = homogen_table{ x_test, 1, column_count, empty_delete<const float>() };
