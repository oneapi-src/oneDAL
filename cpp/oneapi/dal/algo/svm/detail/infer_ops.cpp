...
struct ONEAPI_DAL_EXPORT infer_ops_dispatcher<default_policy, Float, Task, Method> {
    infer_result operator()(const default_policy& ctx,
...
#define INSTANTIATE(F, T, M) \
    template struct ONEAPI_DAL_EXPORT infer_ops_dispatcher<default_policy, F, T, M>;
