...
enum class cov_list : std::uint64_t {
    max = 1UL << 0,
    min = 1UL << 1,
    sum = 1UL << 2,
    sum2 = 1UL << 3,
...
constexpr inline cov_list bs_mode_min_max = (cov_list::min | cov_list::max);
constexpr inline cov_list bs_mode_mean_variance = (cov_list::mean | cov_list::varc);
constexpr inline cov_list bs_mode_all =
    (cov_list::min | cov_list::max | cov_list::sum | cov_list::sum2 | cov_list::sum2cent |
     cov_list::mean | cov_list::sorm | cov_list::varc | cov_list::stdev | cov_list::vart);

constexpr inline cov_list sum2cent_based_stat =
    cov_list::sum2cent | cov_list::varc | cov_list::stdev | cov_list::vart;
...
public:
    static own_t empty(sycl::queue& q, std::int64_t count, bool deffered_fin = false) {
        own_t res;
        if constexpr (check_mask_flag(cov_list::min, List)) {
            res.rmin_ = pr::ndarray<Float, 1>::empty(q, { count }, alloc::device);
        }
        if constexpr (check_mask_flag(cov_list::max, List)) {
            res.rmax_ = pr::ndarray<Float, 1>::empty(q, { count }, alloc::device);
        }
        if (check_mask_flag(cov_list::sum, List) ||
            (deffered_fin && check_mask_flag(cov_list::mean | sum2cent_based_stat, List))) {
            res.rsum_ = pr::ndarray<Float, 1>::empty(q, { count }, alloc::device);
        }
        if constexpr (check_mask_flag(cov_list::sum2 | cov_list::sorm, List)) {
            res.rsum2_ = pr::ndarray<Float, 1>::empty(q, { count }, alloc::device);
        }
        if (check_mask_flag(cov_list::sum2cent, List) ||
            (deffered_fin &&
             check_mask_flag(cov_list::varc | cov_list::stdev | cov_list::vart, List))) {
...
public:
    static own_t empty(sycl::queue& q, std::int64_t count) {
        own_t res;
        if constexpr (check_mask_flag(cov_list::mean | sum2cent_based_stat, List)) {
        ...
        if (check_mask_flag(cov_list::sum | cov_list::mean | sum2cent_based_stat, List)) {
            res.rsum_ = pr::ndarray<Float, 1>::empty(q, { count }, alloc::device);
        }
        if constexpr (check_mask_flag(cov_list::sum2 | cov_list::sorm, List)) {
            res.rsum2_ = pr::ndarray<Float, 1>::empty(q, { count }, alloc::device);
