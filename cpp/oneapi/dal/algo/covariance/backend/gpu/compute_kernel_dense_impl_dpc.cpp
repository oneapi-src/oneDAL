...
std::int64_t compute_kernel_dense_impl<Float>::get_row_block_count(std::int64_t row_count) {
...
template <typename Float>
std::int64_t compute_kernel_dense_impl<Float>::get_column_block_count(std::int64_t column_count) {
...
template <typename Float>
result_t compute_kernel_dense_impl<Float>::get_result(const descriptor_t& desc,
                                                      const local_result<Float>& ndres,
...
auto compute_covariance(sycl::queue& q,
    ...
    auto cov_event = pr::covariance(q, data, sums, means, cov, vars, tmp, cov_event);
