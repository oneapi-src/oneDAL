...
using pca_methods = testing::Types<pca::method::cov>;

template <typename Tuple>
class pca_common_overflow_gpu_tests : public ::testing::Test {};

TYPED_TEST_SUITE_P(pca_common_overflow_gpu_tests);
...
TYPED_TEST_P(pca_common_overflow_gpu_tests, train_throws_if_component_count_leads_to_overflow) {
    ...
    ASSERT_THROW(train(queue, pca_desc, data_table), range_error);
}

TYPED_TEST_P(pca_common_overflow_gpu_tests, infer_throws_if_component_count_leads_to_overflow) {
    auto selector = sycl::gpu_selector();
    auto queue = sycl::queue(selector);
    constexpr std::int64_t row_count = 8;
    ...
    ASSERT_THROW(infer(queue, pca_desc, m, data_infer_table), range_error);
}

REGISTER_TYPED_TEST_SUITE_P(pca_common_overflow_gpu_tests,
                            infer_throws_if_component_count_leads_to_overflow,
                            train_throws_if_component_count_leads_to_overflow);

INSTANTIATE_TYPED_TEST_SUITE_P(run_pca_common_overflow_gpu_tests,
                               pca_common_overflow_gpu_tests,
                               pca_methods);
