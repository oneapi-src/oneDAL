...
inline bool find_next_bin_index(std::int64_t& curr_bin_index, const BinsVector& local_bins) {
    ...
    auto total = oneapi::dal::detail::parallel_reduce_int32_int64_t(
        (std::int32_t)local_bins.size(),
        (std::int64_t)max_bin_count,
        [&](std::int32_t begin, std::int32_t end, std::int64_t thread_min_index) -> std::int64_t {
...
inline std::int64_t reduce_to_common_bin(const std::int64_t& curr_bin_index,
    ...
    dal::detail::threader_for_int64(local_bins.size(), [&](std::int64_t i) {
...
template <typename EdgeValue, typename Allocator>
struct delta_stepping_cpu {
    traverse_result<task::one_to_all> operator()(
        const detail::descriptor_base<task::one_to_all>& desc,
        const dal::preview::detail::topology<std::int32_t>& t,
        const EdgeValue* vals,
        ...
        using atomic_value_allocator_type = typename std::allocator_traits<
            allocator_type>::template rebind_alloc<std::atomic<value_type>>;

        using vertex_allocator_type =
            typename std::allocator_traits<allocator_type>::template rebind_alloc<vertex_type>;

        vertex_allocator_type vertex_allocator(alloc);
        atomic_value_allocator_type atomic_value_allocator(alloc);
        ...
        dal::detail::threader_for_int64(vertex_count, [&](std::int64_t i) {
            dist[i] = max_dist;
        });
        dist[source].store(0);
        ...
        using v1a_t = typename std::allocator_traits<allocator_type>::template rebind_alloc<
            v1v_t>; // Allocator<v1v_t>;

        using v2v_t = vector_container<v1v_t, v1a_t>;
        using v2a_t = typename std::allocator_traits<allocator_type>::template rebind_alloc<
            v2v_t>; //Allocator<v2v_t>;
        v2a_t v2a(alloc);
        ...
        while (curr_bin_index != max_bin_count && iter != max_bin_count && !empty_queue) {
            dal::detail::threader_for_int64(curr_shared_bin_tail, [&](std::int64_t i) {
            ...
            dal::detail::threader_for_int64(thread_cnt, [&](std::int64_t i) {
                int thread_id = dal::detail::threader_get_current_thread_index();
...
template <typename EdgeValue, typename Allocator>
struct delta_stepping_with_pred_cpu {
    traverse_result<task::one_to_all> operator()(
        const detail::descriptor_base<task::one_to_all>& desc,
        const dal::preview::detail::topology<std::int32_t>& t,
        const EdgeValue* vals,
        ...
        using atomic_vp_allocator_type =
            typename std::allocator_traits<allocator_type>::template rebind_alloc<atomic_vp_type>;

        using vertex_allocator_type =
            typename std::allocator_traits<allocator_type>::template rebind_alloc<vertex_type>;

        vertex_allocator_type vertex_allocator(alloc);
        atomic_vp_allocator_type atomic_vp(alloc);
    ...
    inline traverse_result<task::one_to_all> operator()(
        ...
        if (nrh_dispatcher(ctx)) {
            ...
            if (desc.get_optional_results() & optional_results::predecessors) {
                ...
                return delta_stepping<float, task::one_to_all, topology_type, value_type>{}(
                    ctx,
                    desc,
                    t,
                    vals,
            ...
            if (desc.get_optional_results() & optional_results::predecessors) {
                return delta_stepping_with_pred_cpu<edge_user_value_type<Graph>, Allocator>{}(
                    desc,
                    t,
                    vals,
                    alloc);
                ...
                return delta_stepping_cpu<edge_user_value_type<Graph>, Allocator>{}(desc,
                                                                                    t,
                                                                                    vals,
                                                                                    alloc);
            }
