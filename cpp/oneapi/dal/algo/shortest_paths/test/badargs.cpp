...
//     std::array<double, 2> edge_weights = { 3, 4 };
//     std::array<float, 3> distances = { 0, 3, 4 };
//     std::array<float, 0> predecessors = {};
// };
class example_graph_type : public graph_base_data {
public:
    example_graph_type() {
    ...
    std::array<float, 6> distances = { 0, 10, 20, 40, 20, 21 };
    std::array<float, 0> predecessors = {};
};
...
template<typename GraphType>
    void check_shortest_paths(double delta){
        ...
        std::cout<<"builder is created"<<std::endl;
        ...
        const auto shortest_paths_desc =
            descriptor<float, method::delta_stepping, task::one_to_all, std::allocator<char>>(
                ...
                alloc);
        std::cout<<"descriptor is created"<<std::endl;
...
SHORTEST_PATHS_BADARG_TEST("Check delta is > 0"){
    REQUIRE_THROWS_AS((this->check_shortest_paths<example_graph_type>(100)), invalid_argument);
}
