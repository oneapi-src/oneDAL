...
static train_result<Task> call_dal_kernel(const context_gpu& ctx,
    ...
    sycl::event train_event = opt_ptr->minimize(queue, loss_func, x_suf, { fill_event });
...
template struct train_kernel_gpu<float, method::dense_batch, task::binary_classification>;
template struct train_kernel_gpu<double, method::dense_batch, task::binary_classification>;
