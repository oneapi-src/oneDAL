...
static result_t call_daal_kernel(const context_gpu& ctx,
    ...
    std::int64_t row_count = local_data.get_row_count();
    const std::int64_t block_size = row_count;
    const std::int64_t block_start = 0;
    const std::int64_t block_end = block_size;
    std::int64_t global_row_count = block_size;
    auto global_rank_offsets = array<std::int64_t>::zeros(rank_count);
    global_rank_offsets.get_mutable_data()[rank] = rank * row_count;
    {
    ...
    for (std::int64_t i = 0; i < rank; i++) {
        ...
        offset += global_rank_offsets.get_data()[i];
    }

    std::int64_t global_block_start = block_start + offset;
    std::int64_t global_block_end = block_end + offset;
    ...
    auto global_rank_ends = array<std::int64_t>::zeros(rank_count);
    global_rank_ends.get_mutable_data()[rank] = global_rank_offsets.get_data()[rank] + row_count;
    {
        ONEDAL_PROFILER_TASK(allreduce_recv_counts, queue);
        comm.allreduce(global_rank_ends, spmd::reduce_op::sum).wait();
    }
    ...
    cluster_index =
        cluster_index < block_size ? cluster_index + global_block_start : global_row_count;
    {
    ...
    while (cluster_index < de::integral_cast<std::int32_t>(global_row_count)) {
        cluster_count++;
        bool in_range = cluster_index >= global_block_start && cluster_index < global_block_end;
        if (in_range) {
            set_arr_value(queue,
                          arr_responses,
                          cluster_index - global_block_start,
                          cluster_count - 1)
                .wait_and_throw();
    ...
    if (rank_count > 1) {
        auto total_arr_cores =
            pr::ndarray<std::int32_t, 1>::empty(queue, global_row_count, sycl::usm::alloc::device);

        auto total_arr_responses =
            pr::ndarray<std::int32_t, 1>::empty(queue, global_row_count, sycl::usm::alloc::device);
        comm.allgatherv(arr_cores.flatten(queue),
                        total_arr_cores.flatten(queue),
                        global_rank_offsets.get_data(),
                        global_rank_ends.get_data())
            .wait();
        comm.allgatherv(arr_responses.flatten(queue),
                        total_arr_responses.flatten(queue),
                        global_rank_offsets.get_data(),
                        global_rank_ends.get_data())
            .wait();
