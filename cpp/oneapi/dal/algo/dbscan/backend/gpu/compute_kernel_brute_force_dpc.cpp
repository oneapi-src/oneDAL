...
static result_t compute_kernel_dense_impl(const context_gpu& ctx,
    ...
    auto [arr_responses, responses_event] =
        pr::ndarray<std::int32_t, 1>::full(queue, row_count, -1, sycl::usm::alloc::device);
    auto [arr_connected_components_flag, connected_components_flag_event] =
    ...
    sycl::event::wait({ cores_event,
                        responses_event,
                        connected_components_flag_event,
                        adj_matrix_event,
                        neighbours_event });
    ...
    kernels_fp<Float>::get_cores(queue,
                                 ...
                                 min_observations)
        .wait_and_throw();
    //temporary just batch tests work
    ...
    while (flag) {
        connected_components(queue,
                             arr_responses,
                             adj_matrix,
                             arr_connected_components_flag,
                             arr_neighbours,
                             min_observations)
            .wait_and_throw();
        auto value = kernels_fp<Float>::get_queue_front(queue, arr_connected_components_flag);
        if (value == 0) {
            flag = false;
        }
    }
    auto arr_fake_responses_host = arr_responses.to_host(queue);
    ...
    for (int i = 0; i < row_count; i++) {
        if (fake_responses_ptr[i] != -1) {
            if (element_to_id.find(fake_responses_ptr[i]) == element_to_id.end()) {
                element_to_id[fake_responses_ptr[i]] = cluster_count++;
            }
        }
    }
    ...
    return make_results(queue, desc, data_nd, arr_responses_ordered, arr_cores, cluster_count);
}
