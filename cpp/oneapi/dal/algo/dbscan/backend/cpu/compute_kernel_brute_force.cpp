...
static compute_result<Task> call_daal_kernel(const context_cpu& ctx,
    ...
    const bool mem_save_mode = desc.get_mem_save_mode();
    ...
    array<int> arr_core_flags = array<int>::empty(row_count * 1);
    ...
    const auto daal_core_flags =
        interop::convert_to_daal_homogen_table(arr_core_flags, row_count, 1);
    ...
    const auto daal_cluster_count =
        interop::convert_to_daal_homogen_table(arr_cluster_count, 1, 1);
    ...
    return compute_result<Task>()
        .set_responses(dal::detail::homogen_table_builder{}.reset(arr_responses, row_count, 1).build())
        .set_core_flags(dal::detail::homogen_table_builder{}.reset(arr_core_flags, row_count, 1).build())
        .set_core_observation_indices(dal::detail::homogen_table_builder{}.reset(arr_core_observation_indices, row_count, 1).build())
        .set_core_observations(dal::detail::homogen_table_builder{}.reset(arr_core_observations, row_count, column_count).build())
...
struct compute_kernel_cpu<Float, method::brute_force, task::clustering> {
    compute_result<task::clustering> operator()(const context_cpu& ctx,
                                              const descriptor_t& desc,
