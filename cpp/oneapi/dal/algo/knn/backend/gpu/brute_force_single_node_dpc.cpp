...
#include "oneapi/dal/algo/knn/backend/gpu/brute_force_single_node.hpp"
...
public:
    knn_callback(sycl::queue& q,
        ...
        if (this->ropts_.test(result_options::responses)) {
    ...
    auto& set_responses(const pr::ndview<res_t, 1>& responses) {
        if (this->ropts_.test(result_options::responses)) {
            ONEDAL_ASSERT(responses.get_count() == this->query_length_);
    ...
    auto& set_indices(const pr::ndview<idx_t, 2>& indices) {
        if (this->ropts_.test(result_options::indices)) {
            ONEDAL_ASSERT(indices.get_dimension(0) == this->query_length_);
            ONEDAL_ASSERT(indices.get_dimension(1) == this->k_neighbors_);
    ...
    auto& set_distances(const pr::ndview<dst_t, 2>& distances) {
        if (this->ropts_.test(result_options::distances)) {
            ONEDAL_ASSERT(distances.get_dimension(0) == this->query_length_);
            ONEDAL_ASSERT(distances.get_dimension(1) == this->k_neighbors_);
    ...
    sycl::event operator()(std::int64_t qb_id,
                           ...
                           const bk::event_vector& deps = {}) {
        sycl::event copy_ids, copy_dts, comp_rps;

        const auto bnds = this->block_bounds(qb_id);

        if (this->ropts_.test(result_options::indices)) {
            copy_ids = this->output_indices(bnds, inp_indices, deps);
        }

        if (this->ropts_.test(result_options::distances)) {
            copy_dts = this->output_distances(bnds, inp_distances, deps);
        }

        if (this->ropts_.test(result_options::responses)) {
            using namespace bk;
            const auto ndeps = deps + copy_ids + copy_dts;
            comp_rps = this->output_responses(bnds, inp_indices, inp_distances, ndeps);
        }

        sycl::event::wait_and_throw({ copy_ids, copy_dts, comp_rps });
        return sycl::event();
    }
...
protected:
    ...
    sycl::event output_distances(const std::pair<idx_t, idx_t>& bnds,
        ...
        ONEDAL_ASSERT(this->ropts_.test(result_options::distances));
    ...
    sycl::event output_indices(const std::pair<idx_t, idx_t>& bnds,
        ...
        ONEDAL_ASSERT(this->ropts_.test(result_options::indices));
    ...
    sycl::event do_ucls(const std::pair<idx_t, idx_t>& bnds,
        ...
        ONEDAL_ASSERT(this->ropts_.test(result_options::responses));
...
sycl::event bf_kernel(sycl::queue& queue,
    ...
    const auto tcount = train.get_dimension(0);
    ...
    knn_callback<Float, Task> callback(queue, ropts, qbcount, qcount, kcount);
