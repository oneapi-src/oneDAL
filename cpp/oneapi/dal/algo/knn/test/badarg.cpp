...
public:
    ...
    table get_train_labels(std::int64_t override_row_count = train_row_count,
                           std::int64_t override_column_count = 1) const {
        ONEDAL_ASSERT(override_row_count <= train_row_count);
        return homogen_table::wrap(train_labels_.data(), override_row_count, override_column_count);
    }
...
KNN_BADARG_TEST("throws if both train data and labels are empty") {
    SKIP_IF(this->not_available_on_device());
    const auto knn_desc = this->get_descriptor();
    REQUIRE_THROWS_AS(this->train(knn_desc, homogen_table{}, homogen_table{}), domain_error);
}

KNN_BADARG_TEST("throws if train labels are empty") {
    SKIP_IF(this->not_available_on_device());
    const auto knn_desc = this->get_descriptor();
