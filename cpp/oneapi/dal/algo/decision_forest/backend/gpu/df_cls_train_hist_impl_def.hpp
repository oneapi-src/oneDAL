...
inline T atomic_global_add(T* ptr, T operand) {
    return cl::sycl::atomic_fetch_add<T, address::global_space>({cl::sycl::multi_ptr<T, address::global_space>{ptr}}, operand);
}

template <typename Float, typename Bin, typename Index>
std::uint64_t df_cls_train_hist_impl<Float, Bin, Index>::get_part_hist_required_mem_size(Index selected_ftr_count, Index max_bin_count_among_ftrs, Index class_count) const {
...
void df_cls_train_hist_impl<Float, Bin, Index>::init_params ( const descriptor_t& desc,
    ...
    selected_ftr_count_ = desc.get_features_per_node() ? desc.get_features_per_node() : std::sqrt(column_count_);
    ...
    while (preferable_local_size_for_part_hist_kernel_ > std::max(selected_ftr_count_, min_preferable_local_size_for_part_hist_kernel_)) {
        preferable_local_size_for_part_hist_kernel_ >>= 1;
    }

    auto vimp = desc.get_variable_importance_mode();
    mda_required_ = (variable_importance_mode::mda_raw == vimp || variable_importance_mode::mda_scaled == vimp);
    ...
    oob_required_ = (check_mask_flag(emm, error_metric_mode::out_of_bag_error) || check_mask_flag(emm, error_metric_mode::out_of_bag_error_per_observation) ||
                    mda_required_);
    oob_err_required_ = check_mask_flag(emm, error_metric_mode::out_of_bag_error); 
    oob_err_obs_required_ = check_mask_flag(emm, error_metric_mode::out_of_bag_error_per_observation); 

    // init ftr -> bins map and related params
    indexed_features<Float, Bin, Index> ind_ftrs(queue_, desc.get_min_bin_size(), desc.get_max_bins() );
    ...
    for (Index clmn_idx = 0; clmn_idx < column_count_; clmn_idx++) {
        ...
        max_bin_count_among_ftrs_ = (max_bin_count_among_ftrs_ < ftr_bins) ? ftr_bins : max_bin_count_among_ftrs_;
    }
   ...
   // DAAL_CHECK_STATUS_VAR(_treeLevelBuildHelper.init(buildOptions.c_str(), impl_const_t::node_prop_count_));
    ...
    const std::uint64_t device_global_mem_size = queue_.get_device().get_info<cl::sycl::info::device::global_mem_size>();
    const std::uint64_t device_max_mem_alloc_size = queue_.get_device().get_info<cl::sycl::info::device::max_mem_alloc_size>();

    const auto part_hist_size = get_part_hist_required_mem_size(selected_ftr_count_, max_bin_count_among_ftrs_, class_count_);
    const auto max_mem_alloc_size = std::min(device_max_mem_alloc_size, std::uint64_t(max_mem_alloc_size_for_algo_));

    std::uint64_t used_mem_size = sizeof(Float) * row_count_ * (column_count_ + 1); // input table size + response
    ...
    std::uint64_t available_global_mem_size = device_global_mem_size > used_mem_size ? device_global_mem_size - used_mem_size : 0;

    std::uint64_t available_mem_size_for_tree_block =
        std::min(max_mem_alloc_size, static_cast<std::uint64_t>(available_global_mem_size * global_mem_fraction_for_tree_block_));

    std::uint64_t required_mem_size_for_one_tree = oob_required_ ? tree_level_build_helper_.get_oob_rows_required_mem_size(row_count_, 1 /* for 1 tree */, desc.get_observations_per_tree_fraction()) : 0;

    required_mem_size_for_one_tree += sizeof(Index) * selected_row_count_ * 2; // main tree order and auxilliary one used for partitioning

    tree_in_block_ = de::integral_cast<Index>(available_mem_size_for_tree_block / required_mem_size_for_one_tree);
    ...
    available_global_mem_size =
        available_global_mem_size > (tree_in_block_ * required_mem_size_for_one_tree) ? available_global_mem_size - (tree_in_block_ * required_mem_size_for_one_tree) : 0;
    // size for one part hist was already reserved, add some more if there is available mem
    max_part_hist_cumulative_size_ =
        std::min(max_mem_alloc_size, static_cast<std::uint64_t>(part_hist_size + available_global_mem_size * global_mem_fraction_for_part_hist_));
...
void df_cls_train_hist_impl<Float, Bin, Index>::allocate_buffers () {
    ...
    tree_order_lev_ = pr::ndarray<Index, 1>::empty(queue_, {selected_row_count_ * tree_in_block_}, alloc::device);
    tree_order_lev_buf_ = pr::ndarray<Index, 1>::empty(queue_, {selected_row_count_ * tree_in_block_}, alloc::device);
    ...
    if (oob_required_)
    {
        ...
        auto [oob_per_obs_list, event] = pr::ndarray<Index, 1>::zeros(queue_, {row_count_ * class_count_}, alloc::device);
...
template <typename Float, typename Bin, typename Index>
pr::ndarray<Index, 1> df_cls_train_hist_impl<Float, Bin, Index>::gen_features(Index node_count, const pr::ndarray<Index, 1>& node_vs_tree_map, dal::array<engine_impl>& engine_arr) {
    //DAAL_OVERFLOW_CHECK_BY_MULTIPLICATION(size_t, (node_count + 1), nSelectedFeatures);
    auto selectedFeaturesHost = pr::ndarray<Index, 1>::empty({(node_count + 1) * selected_ftr_count_}); // first part is used features indices, +1 - part for generator
    auto selectedFeaturesCom  = pr::ndarray<Index, 1>::empty(queue_, {node_count * selected_ftr_count_}, alloc::device);

    auto selectedFeaturesHost_ptr_orig = selectedFeaturesHost.get_mutable_data(); 
    std::int32_t* selectedFeaturesHost_ptr = reinterpret_cast<std::int32_t*>(selectedFeaturesHost_ptr_orig);
    ...
    if (selected_ftr_count_ != column_count_)
    {
        ...
        for (Index node = 0; node < node_count; node++) {
            rn_gen.uniform_without_replacement(selected_ftr_count_, selectedFeaturesHost_ptr + node * selected_ftr_count_,
                                          selectedFeaturesHost_ptr + (node + 1) * selected_ftr_count_,
                                          engines[treeMap_ptr[node]].get_state(), 0, column_count_);
        }
    }
    else {
        for (Index node = 0; node < node_count; node++) {
    ...
    selectedFeaturesCom.assign(queue_, selectedFeaturesHost_ptr_orig, selectedFeaturesCom.get_count()).wait_and_throw();
...
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::compute_class_histogram(
    ...
    Index node_count, const be::event_vector& deps) {
    ...
    const Index node_imp_prop_count = impl_const_t::node_imp_prop_count_; // num of split attributes for node
    ...
    const cl::sycl::nd_range<2> nd_range = be::make_multiple_nd_range_2d({ local_size, node_count}, { local_size, 1});
    ...
    auto event = queue_.submit([&](cl::sycl::handler& cgh) {
        ...
        cgh.parallel_for<class compute_class_histogram_krn<Float, Bin, Index>>(
            ...
            constexpr Index buff_size = 16; // class_count, find a way to specialize it in kernel build time 
...
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::computeBestSplit(
    const pr::ndarray<Bin, 2>& data, const pr::ndview<Float, 1>& response, 
    const pr::ndarray<Index, 1>& treeOrder, const pr::ndarray<Index, 1>& selectedFeatures, const pr::ndarray<Index, 1>& binOffsets, 
    const pr::ndarray<Float, 1>& impList, const pr::ndarray<Index, 1>& class_hist_list, 
    pr::ndarray<Index, 1>& nodeList, pr::ndarray<Float, 1>& left_child_imp_list, pr::ndarray<Index, 1>& left_child_class_hist_list,
    pr::ndarray<Float, 1>& nodeImpDecreaseList, bool updateImpDecreaseRequired, Index nNodes, const be::event_vector& deps) {
    ...
    [[maybe_unused]] auto nodesGroups = pr::ndarray<Index, 1>::empty(queue_, {_nNodesGroups * _nodeGroupProps}, alloc::device);
    [[maybe_unused]] auto nodeIndices = pr::ndarray<Index, 1>::empty(queue_, {nNodes}, alloc::device);

    cl::sycl::event last_event;
    last_event = tree_level_build_helper_.split_node_list_on_groups_by_size(nodeList, nodesGroups, nodeIndices, nNodes, _nNodesGroups, _nodeGroupProps, deps);
    ...
    for (Index i = 0; i < _nNodesGroups; i++, processedNodes += nGroupNodes)
    {
        ...
        {
            const Index partHistSize = get_part_hist_required_mem_size(selected_ftr_count_, max_bin_count_among_ftrs_, class_count_);

            Index nPartialHistograms = maxGroupBlocksNum <= _minRowsBlocksForOneHist ? 1 : _maxLocalHistograms;
            ...
            const Index nPHBlocks = nPHBlockElems / maxPHBlockElems ? (nPHBlockElems / maxPHBlockElems + bool(nPHBlockElems % maxPHBlockElems)) : 1;

            Index nBlockNodes = nGroupNodes / nPHBlocks + bool(nGroupNodes % nPHBlocks);

            for (Index blockIndicesOffset = groupIndicesOffset; blockIndicesOffset < groupIndicesOffset + nGroupNodes;
                 blockIndicesOffset += nBlockNodes)
            {
                nBlockNodes = std::min(nBlockNodes, groupIndicesOffset + nGroupNodes - blockIndicesOffset);
                if (1 == nPartialHistograms)
                {
                    auto nodesHistograms = pr::ndarray<Index, 1>::empty(queue_, {nBlockNodes * partHistSize}, alloc::device);
                    ...
                    last_event = compute_partial_histograms(
                        data, response, 
                        treeOrder, selectedFeatures, binOffsets, 
                        nodeList, nodeIndices, blockIndicesOffset,
                        nodesHistograms, nPartialHistograms, 
                        nBlockNodes, {last_event});

                    //last_event.wait_and_throw();
                    //last_event.wait_and_throw();

                    last_event = compute_best_split_by_histogram(
                        nodesHistograms, 
                        selectedFeatures, binOffsets, 
                        impList, class_hist_list, nodeIndices, blockIndicesOffset,
                        nodeList, left_child_imp_list, left_child_class_hist_list, 
                        nodeImpDecreaseList, updateImpDecreaseRequired,
                        nBlockNodes, {last_event});
                    ////_P("ngrp_nodes = %d", nGroupNodes);
...
*/                    
                ...
                else
                {
                    ...
                    auto partialHistograms = pr::ndarray<Index, 1>::empty(queue_, {nBlockNodes * nPartialHistograms * partHistSize}, alloc::device);
                    auto nodesHistograms = pr::ndarray<Index, 1>::empty(queue_, {nBlockNodes * partHistSize}, alloc::device);

                    last_event = compute_partial_histograms(
                        data, response, treeOrder, 
                        selectedFeatures, binOffsets, 
                        nodeList, nodeIndices, blockIndicesOffset,
                        partialHistograms, nPartialHistograms, 
                        nBlockNodes, {last_event});

                    last_event = reduce_partial_histograms(partialHistograms, nodesHistograms, nPartialHistograms, 
                        nBlockNodes, reduce_local_size_part_hist_, {last_event});
        ...
        else
        {
            ...
            last_event = compute_best_split_single_pass(
                data, response, treeOrder, 
                selectedFeatures, binOffsets, 
                impList, class_hist_list, nodeIndices, groupIndicesOffset,
                nodeList, left_child_imp_list, left_child_class_hist_list, 
                nodeImpDecreaseList, updateImpDecreaseRequired,
                nGroupNodes, {last_event});
...
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::compute_partial_histograms(
    const pr::ndarray<Bin, 2>& data, const pr::ndview<Float, 1>& response, 
    const pr::ndarray<Index, 1>& treeOrder, const pr::ndarray<Index, 1>& selectedFeatures, const pr::ndarray<Index, 1>& binOffsets, 
    const pr::ndarray<Index, 1>& nodeList, const pr::ndarray<Index, 1>& nodeIndices, Index nodeIndicesOffset,
    pr::ndarray<Index, 1>& partialHistograms, Index nPartialHistograms, 
    Index node_count, const be::event_vector& deps) {
    ...
    auto event = queue_.submit([&](cl::sycl::handler& cgh) {
        ...
        cgh.parallel_for(nd_range, [=](cl::sycl::nd_item<2> item) {
            ...
            for (Index i = iStart; i < iEnd; i++)
            {
                ...
                for (Index featIdx = ftrGrpIdx; featIdx < selected_ftr_count; featIdx += ftrGrpSize)
                {
                    const Index featId = selected_ftrs_ptr[nodeId * selected_ftr_count + featIdx];

                    Index* histogram_ptr =
                        partial_histogram_ptr + ((nodeIdx * nPartHist + histIdx) * selected_ftr_count + featIdx) * max_bin_count_among_ftrs * nProp;
...
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::reduce_partial_histograms(
    const pr::ndarray<Index, 1>& partialHistograms, pr::ndarray<Index, 1>& histograms, Index nPartialHistograms, 
    Index node_count, Index reduce_local_size, const be::event_vector& deps) {
    ...
    const cl::sycl::nd_range<3> nd_range =
        be::make_multiple_nd_range_3d({ max_bin_count_among_ftrs * selected_ftr_count, reduce_local_size, node_count},
                                  { 1, reduce_local_size, 1});

    auto event = queue_.submit([&](cl::sycl::handler& cgh) {
        cgh.depends_on(deps);
        cl::sycl::accessor<Index, 1, cl::sycl::access::mode::read_write, cl::sycl::access::target::local> buf(LOCAL_BUFFER_SIZE * nProp, cgh);
        ...
        cgh.parallel_for(nd_range, [=](cl::sycl::nd_item<3> item) {
            ...
            const Index * nodePartialHistograms =
                partial_histogram_ptr + nodeIdx * nPartialHistograms * selected_ftr_count * max_bin_count_among_ftrs * nProp;
            Index * nodeHistogram = histogram_ptr + nodeIdx * selected_ftr_count * max_bin_count_among_ftrs * nProp;

            for (Index i = local_id; i < nPartialHistograms; i += local_size) {
                Index offset = i * selected_ftr_count * max_bin_count_among_ftrs * nProp + binId * nProp;
                for (Index prop = 0; prop < nProp; prop++) {
...
inline bool float_eq(Float a, Float b) { return cl::sycl::fabs(a - b) <= float_accuracy<Float>::val; }

template <typename Float>
inline bool float_gt(Float a, Float b) { return (a - b) > float_accuracy<Float>::val; }
//// By histogram///////////////////////////////////
template <typename Float, typename Bin, typename Index>
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::compute_best_split_by_histogram(
    const pr::ndarray<Index, 1>& nodesHistograms, const pr::ndarray<Index, 1>& selectedFeatures, const pr::ndarray<Index, 1>& binOffsets,
    const pr::ndarray<Float, 1>& impList, const pr::ndarray<Index, 1>& class_hist_list, const pr::ndarray<Index, 1>& nodeIndices, Index nodeIndicesOffset,
    pr::ndarray<Index, 1>& nodeList, pr::ndarray<Float, 1>& left_child_imp_list, pr::ndarray<Index, 1>& left_child_class_hist_list, 
    pr::ndarray<Float, 1>& nodeImpDecreaseList, bool updateImpDecreaseRequired,
    Index node_count, const be::event_vector& deps) {
    ...
    const cl::sycl::nd_range<2> nd_range =
        be::make_multiple_nd_range_2d({ local_size, node_count},
                                  { local_size, 1});
    ...
    auto event = queue_.submit([&](cl::sycl::handler& cgh) {
        ...
        cgh.parallel_for<class compute_best_split_by_histogram_krn<Float, Bin, Index>>(nd_range, [=](cl::sycl::nd_item<2> item) 
        {
            ...
            for(Index currFtrIdx = sub_group_local_id; currFtrIdx < selected_ftr_count; currFtrIdx += sub_group_size) {
                const Index* nodeHistogram       = node_histogram_ptr + nodeIdx * selected_ftr_count * nMaxBinsAmongFtrs * nProp;
                const Index* histogramForFeature = nodeHistogram + currFtrIdx * nMaxBinsAmongFtrs * nProp;
                ...
                for (Index tbin = 0; tbin < currFtrBins; tbin++) {
                    ...
                    Float divL = (0 < mrgLRN[0]) ? Float(1) / (Float(mrgLRN[0]) * Float(mrgLRN[0])) : Float(0);
                    Float divR = (0 < mrgLRN[1]) ? Float(1) / (Float(mrgLRN[1]) * Float(mrgLRN[1])) : Float(0);

                    for (Index prop = 0; prop < nProp; prop++) {
                        impL -= Float(mrgLRCls[prop]) * Float(mrgLRCls[prop]) * divL;
                        impR -= Float(mrgCls[prop] - mrgLRCls[prop]) * Float(mrgCls[prop] - mrgLRCls[prop]) * divR;
                    ...
                    Float impDec = imp - (Float(mrgLRN[0]) * impL + Float(mrgLRN[1]) * impR) / Float(mrgN);
                ...
                Index bestFeatureId    = reduce(sbg, impDecIsBest ? curFeatureId : valNotFound, minimum<Index>());
                Index bestFeatureValue = reduce(sbg, (bestFeatureId == curFeatureId && impDecIsBest) ? curFeatureValue : valNotFound, minimum<Index>());
                ...
                if (noneSplitFoundBySubGroup || mySplitIsBest) {
                        ...
                        node_list_ptr[nodeId * nNodeProp + 2] = curFeatureId == valNotFound ? leafMark : curFeatureId;
                        node_list_ptr[nodeId * nNodeProp + 3] = curFeatureValue == valNotFound ? leafMark : curFeatureValue;
                        node_list_ptr[nodeId * nNodeProp + 4] = bestLN;

                        if (updateImpDecreaseRequired) node_imp_decr_list_ptr[nodeId] = bestImpDec;
                }
            }
        });
    });
            ...
            for(Index currFtrIdx = sub_group_local_id; currFtrIdx < selected_ftr_count; currFtrIdx += sub_group_size) {
                ...
                for (Index i = 0; i < nRows; i++) {
                    ...
                    if(bin_not_processed) {
                        ...
                        if ((Float)0 < impDec && !float_eq(imp, (Float)0) && imp >= impurityThreshold
                            && (curFeatureValue == leafMark || float_gt(impDec, curImpDec) || 
                                (float_eq(impDec, curImpDec) && (curFeatureId < featId || (curFeatureId == featId && tbin < curFeatureValue))))
                            && mrgLRN[0] >= minObservationsInLeafNode && mrgLRN[1] >= minObservationsInLeafNode) {
                            curFeatureId    = featId;
                            curFeatureValue = tbin;
                            curImpDec       = impDec;
...
template <typename Float, typename Bin, typename Index>
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::do_node_split(const pr::ndarray<Index, 1>& node_list, 
                                                                     ...
                                                                     Index node_count, Index node_count_new, const be::event_vector& deps) {
    ...
    auto event = queue_.submit([&](cl::sycl::handler& cgh) {
        ...
        cgh.parallel_for<class do_node_split_krn<Float, Bin, Index>>(nd_range, [=](cl::sycl::nd_item<1> item) {
            ...
            for (Index i = local_id; i < node_count; i += local_size) {
                Index splitNode      = Index(node_list_ptr[i * nNodeProp + 2] != bad_val); // featId != -1
                Index newLeftNodePos = nCreatedNodes + exclusive_scan(sbg, splitNode, plus<Index>()) * 2;
                if (splitNode) {
                    ...
                    const Index* left_child_class_hist  = left_child_class_hist_list_ptr + i * class_count;
                    Index* class_hist_left  = class_hist_list_new_ptr + newLeftNodePos * class_count;
                    Index* class_hist_right = class_hist_list_new_ptr + (newLeftNodePos + 1) * class_count;
...
Float df_cls_train_hist_impl<Float, Bin, Index>::compute_oob_error(
                        const model_builder_t& model_builder, const pr::ndarray<Float, 1>& data_host, const pr::ndarray<Float, 1>& response_host,
                        const pr::ndarray<Index, 1>& oob_row_list, pr::ndarray<Index, 1>& oob_per_obs_list, Index tree_idx, Index indicesOffset, Index n, const be::event_vector& deps) {
    ...
    for (Index i = 0; i < n; i++)
    {
        ...
        Index class_idx = static_cast<Index>(model_builder.get_tree_response(tree_idx, &data_host_ptr[row_ind * column_count_]));
...
Float df_cls_train_hist_impl<Float, Bin, Index>::compute_oob_error_perm(
                        const model_builder_t& model_builder, const pr::ndarray<Float, 1>& data_host, const pr::ndarray<Float, 1>& response_host,
                        const pr::ndarray<Index, 1>& oob_row_list, const pr::ndarray<Index, 1>& permutation_host, Index tree_idx, Index indicesOffset, Index n, Index column_idx, const be::event_vector& deps) {
    ONEDAL_ASSERT(data_host.get_count() == row_count_ * column_count_);
    ONEDAL_ASSERT(response_host.get_count() == column_count_);
    //ONEDAL_ASSERT(indices.get_count() == column_count_);
    ...
    for (Index i = 0; i < n; i++)
    {
        ...
        memcpy(de::default_host_policy{}, buf_ptr, &data_host_ptr[row_ind * column_count_], column_count_ * sizeof(Float));
...
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::compute_results(
                        const model_builder_t& model_builder, const pr::ndarray<Float, 1>& data_host, const pr::ndarray<Float, 1>& response_host,
                        const pr::ndarray<Index, 1>& oob_row_list, const pr::ndarray<Index, 1>& oobRowsNumList, pr::ndarray<Index, 1>& oob_per_obs_list, 
                        pr::ndarray<Float, 1>& var_imp, pr::ndarray<Float, 1>& var_imp_variance, const dal::array<engine_impl>& engine_arr,
                        Index tree_idx_in_block, Index tree_in_block_count, Index built_tree_count, const be::event_vector& deps) {
    ...
    Index oob_row_count = nOOBRowsHost_ptr[tree_idx_in_block + 1] - nOOBRowsHost_ptr[tree_idx_in_block];

    if ( (oob_required_ || mda_required_) && oob_row_count) {
        [[maybe_unused]] const Float oob_err = compute_oob_error(model_builder, data_host, response_host, oob_row_list, oob_per_obs_list, tree_idx_in_block, oob_indices_offset, oob_row_count);
        ...
        if (mda_required_) {
            ...
            for (Index column_idx = 0; column_idx < column_count_; column_idx++) {
                rn_gen.shuffle(oob_row_count, permutation_ptr, engine_arr[built_tree_count + tree_idx_in_block].get_state()).wait_and_throw();
                const Float oob_err_perm = compute_oob_error_perm(model_builder, data_host, response_host, oob_row_list, permutation, tree_idx_in_block, oob_indices_offset, oob_row_count, column_idx);
                ...
                if (var_imp_var_host_ptr) {
                    var_imp_var_host_ptr[column_idx] += delta * (diff - var_imp_host_ptr[column_idx]);
                }
            }
...
template <typename Float, typename Bin, typename Index>
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::finalize_oob_error(const pr::ndarray<Float, 1>& response_host, pr::ndarray<Index, 1>& oob_per_obs_list,
                                                                              pr::ndarray<Float, 1>& res_oob_err, pr::ndarray<Float, 1>& res_oob_err_obs, const be::event_vector& deps) {
    ONEDAL_ASSERT(oob_per_obs_list.get_count() == row_count_ * class_count_);
    ...
    for (Index i = 0; i < row_count_; i++) {
        ...
        if (0 < max_val) {
            ...
            if (oob_err_obs_required_) res_oob_err_obs_host_ptr[i] = prediction_res;
            oob_err += prediction_res;
            predicted_count++;
        }
        else if (oob_err_obs_required_)
            res_oob_err_obs_host_ptr[i] = Float(-1); //was not in OOB set of any tree and hence not predicted
    }
...
template <typename Float, typename Bin, typename Index>
cl::sycl::event df_cls_train_hist_impl<Float, Bin, Index>::finalize_var_imp(pr::ndarray<Float, 1>& var_imp, pr::ndarray<Float, 1>& var_imp_variance,
    ...
    else if (mdi_required_)
    {
        ...
        for (Index i = 0; i < column_count_; i++) var_imp_host_ptr[i] *= div;
        var_imp = var_imp_host.to_device(queue_);
    }
...
result_t df_cls_train_hist_impl<Float, Bin, Index>::operator() ( const descriptor_t& desc,
    ...
    model_builder_interop<Float, Index, task::classification> model_builder(tree_count_, column_count_, class_count_);
    ...
    for (Index iter = 0; iter < tree_count_; iter += tree_in_block_) {
        ...
        std::vector<pr::ndarray<Index, 1>> levelNodeLists;    // lists of nodes int props(rowsOffset, rows, ftrId, ftrVal ... )
        std::vector<pr::ndarray<Float, 1>> levelNodeImpLists;    // lists of nodes int props(rowsOffset, rows, ftrId, ftrVal ... )
        ...
        auto levelNodeLists_init_host = pr::ndarray<Index, 1>::empty({nNodes * impl_const_t::node_prop_count_});
        //levelNodeLists.push_back(pr::ndarray<Index, 1>::empty(queue_, {nNodes * impl_const_t::node_prop_count_}, alloc::device));
        levelNodeImpLists.push_back(pr::ndarray<Float, 1>::empty(queue_, {nNodes * impl_const_t::node_imp_prop_count_}, alloc::device));
        level_node_class_hist_list.push_back(pr::ndarray<Index, 1>::empty(queue_, {nNodes * class_count_}, alloc::device));
        ...
        for (Index node = 0; node < nNodes; node++) {
            treeMap[node] = iter + node; // check for par.nTrees less than int32 was done at the beggining
            rootNode[node * impl_const_t::node_prop_count_ + 0] = selected_row_count_ * node; // rows offset
            rootNode[node * impl_const_t::node_prop_count_ + 1] = selected_row_count_;        // num of rows
        }
        ...
        if (bootstrap_) {
            ...
            std::int32_t* selected_rows_ptr = reinterpret_cast<std::int32_t*>(selected_rows_ptr_index);

            for (Index tree_idx = 0; tree_idx < nTrees; tree_idx++) {
                rng<std::int32_t> rn_gen;
                rn_gen.uniform(selected_row_count_, selected_rows_ptr + selected_row_count_ * tree_idx, engines[iter + tree_idx].get_state(), 0, row_count_);
            }

            cl::sycl::event event = tree_order_lev_.assign(queue_, selected_rows_ptr_index, selected_rows_host_.get_count());
            event.wait_and_throw();
        }
        else {
            cl::sycl::event event = tree_level_build_helper_.initialize_tree_order(tree_order_lev_, nTrees, selected_row_count_);
            event.wait_and_throw();
        }

        last_event = compute_class_histogram( response_nd_, tree_order_lev_, levelNodeLists[0], levelNodeImpLists[0], level_node_class_hist_list[0], nNodes, {last_event});

        if (oob_required_) {
            //_P("get oob rows");
            cl::sycl::event event = tree_level_build_helper_.get_oob_row_list(tree_order_lev_, oobRowsNumList, oobRows, selected_row_count_, nTrees); // oobRowsNumList and oobRows are the output
            event.wait_and_throw();
        }
        ...
        for (Index level = 0; nNodes > 0; level++)
        {
            ...
            auto left_child_imp_list = pr::ndarray<Float, 1>::empty(queue_, {nNodes * impl_const_t::node_imp_prop_count_}, alloc::device);
            auto left_child_class_hist_list = pr::ndarray<Index, 1>::empty(queue_, {nNodes * class_count_}, alloc::device);

            auto selectedFeaturesCom = gen_features(nNodes, nodeVsTreeMap, engine_arr);

            if (mdi_required_) {
                node_imp_decrease_list = pr::ndarray<Float, 1>::empty(queue_, {nNodes}, alloc::device);
            }

            ////_P("before split class hist list :");
            //print_nd_arr(queue_, class_hist_list, nNodes, class_count_);

            last_event = computeBestSplit(full_data_nd_, response_nd_, tree_order_lev_, selectedFeaturesCom, ftr_bin_offsets_nd_, impList, class_hist_list, 
                nodeList, left_child_imp_list, left_child_class_hist_list, node_imp_decrease_list, mdi_required_, nNodes, {last_event});
            last_event.wait_and_throw();

            tree_level_record_t level_record(queue_, nodeList, impList, class_hist_list, nNodes, class_count_);
            DFTreeRecords.push_back(level_record);

            if (mdi_required_) {
                //mdi is calculated only on split nodes and is not calculated on last level
                last_event = tree_level_build_helper_.update_mdi_var_importance(nodeList, node_imp_decrease_list, res_var_imp, column_count_, nNodes, {last_event});
            }

            Index node_count_new;
            last_event = tree_level_build_helper_.get_split_node_count(nodeList, nNodes, node_count_new, {last_event});
            last_event.wait_and_throw();
            ...
            if (node_count_new)
            {
                ...
                auto node_list_new = pr::ndarray<Index, 1>::empty(queue_, {node_count_new * impl_const_t::node_prop_count_}, alloc::device);
                auto imp_list_new = pr::ndarray<Float, 1>::empty(queue_, {node_count_new * impl_const_t::node_imp_prop_count_}, alloc::device);
                auto class_hist_list_new = pr::ndarray<Index, 1>::empty(queue_, {node_count_new * class_count_}, alloc::device);
                auto node_vs_tree_map_new = pr::ndarray<Index, 1>::empty(queue_, {node_count_new}, alloc::device);

                last_event = do_node_split(nodeList, class_hist_list, left_child_imp_list, left_child_class_hist_list, nodeVsTreeMap, 
                                           node_list_new, imp_list_new, class_hist_list_new, node_vs_tree_map_new, nNodes, node_count_new, {last_event}); 
                last_event.wait_and_throw();
                ...
                if (max_tree_depth_ > 0 && max_tree_depth_ == level) {
                    tree_level_record_t level_record(queue_, node_list_new, imp_list_new, class_hist_list_new, node_count_new, class_count_);
                    DFTreeRecords.push_back(level_record);
                    node_count_new = 0;
                }
                else {
                    ...
                    last_event = tree_level_build_helper_.do_level_partition_by_groups(full_data_nd_, nodeList, tree_order_lev_, tree_order_lev_buf_, column_count_, nNodes, {last_event});
                }
            }
        ...
        for (Index tree_idx = 0; tree_idx < nTrees; tree_idx++) {
            compute_results(model_builder, data_host_, response_host_,
                        oobRows, oobRowsNumList, oob_per_obs_list_, res_var_imp, var_imp_variance_host_, engine_arr,
                        tree_idx, nTrees, iter, {last_event}).wait_and_throw();
        }
    }
    ...
    if (oob_err_required_ || oob_err_obs_required_) {
        ...
        finalize_oob_error(response_host_, oob_per_obs_list_, res_oob_err, res_oob_err_obs).wait_and_throw();
        ...
        if(oob_err_obs_required_) {
            ...
            res.set_oob_err_per_observation(homogen_table::wrap(res_oob_err_obs_host.flatten(), row_count_, 1));
        }
    }
