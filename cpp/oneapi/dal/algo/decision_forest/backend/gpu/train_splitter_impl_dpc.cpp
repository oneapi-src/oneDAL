...
struct split_smp {
    ...
    inline void process_bin(
        ...
        if constexpr (std::is_same_v<Task, task::classification>) {
            merge_bin_hist(ts_left_count,
                            ts_left_hist,
                            ftr_hist_ptr + bin_ofs,
                            hist_prop_count);
        }
        else {
            merge_bin_hist(ts_left_hist,
                            ftr_hist_ptr + bin_ofs,
                            hist_prop_count);
        }
...
template <typename Float, typename Bin, typename Index, typename Task, bool use_private_mem>
sycl::event
train_splitter_impl<Float, Bin, Index, Task, use_private_mem>::compute_split_single_pass(
    sycl::queue& queue,
    const context_t& ctx,
