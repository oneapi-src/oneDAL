...
struct sconsistent_conditions {
    ...
    ~sconsistent_conditions() {
        if (array != nullptr) {
            allocator.deallocate<std::int64_t>(array, length);
            array = nullptr;
        }
    }
...
sorter<Cpu>::~sorter() {
    if (p_degree_probability != nullptr) {
        allocator.deallocate<float>(p_degree_probability, degree_max_size);
        p_degree_probability = nullptr;
    }

    if (p_vertex_attribute_probability != nullptr) {
        allocator.deallocate<float>(p_vertex_attribute_probability, vertex_attribute_max_size);
        p_vertex_attribute_probability = nullptr;
    }
}
