...
graph::graph(const dal::preview::detail::topology<std::int32_t>& t,
    ...
    if (use_bit_representation) { // use bit vector
        graph_data_storage.pbit_data = new graph_input_bit_data(vertex_count, _allocator);
    }
    else { // use adj list
        graph_data_storage.plist_data = new graph_input_list_data(vertex_count, _allocator);
    }
    ...
    for (std::int64_t i = 0; i < vertex_count; i++) {
        ...
        if (use_bit_representation) {
            ...
            if (degree > 0) {
                graph_data_storage.plist_data->data[i] = _allocator.allocate<std::int64_t>(degree);
            }
            else {
                ...
                if (edge_attr >= 0 || has_edges_attribute) {
                    if (graph_data_storage.pbit_data->edges_attribute[i] == nullptr) {
                        graph_data_storage.pbit_data->edges_attribute[i] =
                            _allocator.allocate<std::int64_t>(degree);
    ...
    if (p_edges_bit != nullptr) {
        for (std::int64_t i = 0; i < n; i++) {
            if (p_edges_bit[i] != nullptr) {
                _allocator.deallocate<std::uint8_t>(p_edges_bit[i], 0);
                p_edges_bit[i] = nullptr;
            }
        }
        _allocator.deallocate<std::uint8_t*>(p_edges_bit, n);
        p_edges_bit = nullptr;
    }
}
    ...
    if (p_edges_list != nullptr) {
        for (std::int64_t i = 0; i < n; i++) {
            if (p_edges_list[i] != nullptr) {
                _allocator.deallocate<std::int64_t>(p_edges_list[i], 0);
                p_edges_list[i] = nullptr;
            }
        }
        _allocator.deallocate<std::int64_t*>(p_edges_list, n);
...
bit_vector graph::get_vertex_neighbors(const std::int64_t vertex,
    ...
    switch (edge_type) {
        case both: {
            bit_vector result(bit_array_size, _allocator.get_byte_allocator());
            bit_vector::set(bit_array_size, result.get_vector_pointer(), p_edges_bit[vertex]);
            return result;
        }
        default: {
            return bit_vector(_allocator.get_byte_allocator());
        }
    }
}
...
bit_vector::bit_vector(bit_vector& bvec) : _allocator(bvec._allocator.get_byte_allocator()) {
    n = bvec.n;
...
bit_vector::bit_vector(bit_vector&& a)
        : vector(a.vector),
          _allocator(a._allocator.get_byte_allocator()) {
    n = a.n;
    a.n = 0;
    a.vector = nullptr;
}
...
bit_vector::~bit_vector() {
    if (vector != nullptr) {
        _allocator.deallocate<std::uint8_t>(vector, n);
        vector = nullptr;
        n = 0;
    }
}
...
graph_input_data::graph_input_data(inner_alloc allocator) : _allocator(allocator) {
    vertex_count = 0;
    degree = nullptr;
    attr = nullptr;
...
graph_input_data::graph_input_data(const std::int64_t vertex_size, inner_alloc allocator)
    ...
    degree = _allocator.allocate<std::int64_t>(vertex_count);
    attr = _allocator.allocate<std::int64_t>(vertex_count);

    edges_attribute = _allocator.allocate<std::int64_t*>(vertex_count);
...
graph_input_data::~graph_input_data() {
    _allocator.deallocate<std::int64_t>(degree, vertex_count);
    _allocator.deallocate<std::int64_t>(attr, vertex_count);

    for (int64_t i = 0; i < vertex_count; i++) {
        if (edges_attribute[i] != nullptr) {
            _allocator.deallocate<std::int64_t>(edges_attribute[i], 1);
            edges_attribute[i] = nullptr;
        }
    }
    _allocator.deallocate<std::int64_t*>(edges_attribute, vertex_count);
}
...
graph_input_list_data::graph_input_list_data(const std::int64_t vertex_size, inner_alloc allocator)
    ...
    data = _allocator.allocate<std::int64_t*>(vertex_count);
    for (int64_t i = 0; i < vertex_count; i++) {
        data[i] = nullptr;
    }
}
...
graph_input_list_data::~graph_input_list_data() {
    ...
    _allocator.deallocate<std::int64_t*>(data, vertex_count);
}
...
graph_input_bit_data::graph_input_bit_data(const std::int64_t vertex_size, inner_alloc allocator)
    ...
    data = _allocator.allocate<std::uint8_t*>(vertex_count);
    for (int64_t i = 0; i < vertex_count; i++) {
        data[i] = _allocator.allocate<std::uint8_t>(bit_array_size);
...
graph_input_bit_data::~graph_input_bit_data() {
    for (int64_t i = 0; i < vertex_count; i++) {
        if (data[i] != nullptr) {
            _allocator.deallocate<std::uint8_t>(data[i], 0);
            data[i] = nullptr;
        }
    }
    _allocator.deallocate<std::uint8_t*>(data, vertex_count);
}
} // namespace oneapi::dal::preview::subgraph_isomorphism::detail