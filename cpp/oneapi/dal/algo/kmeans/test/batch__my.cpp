...
TEMPLATE_LIST_TEST_M(kmeans_batch_test,
    ...
    const std::int64_t iters = 3;
    const std::string data_path = "/export/users/vsfedoto/code/onedal_kmeans_csr/__csr_data_5_50_20.csv";
    const auto x_train = dal::read<dal::table>(q, dal::csv::data_source{ data_path });
    dal::row_accessor<const float> acc(x_train);
    const homogen_table y;

    SECTION("cluster=5") {
        std::cout << " !!!! In dense !!!!" << std::endl;
        std::int64_t cluster_count = 5;
        const auto cent_rows = acc.pull({0, cluster_count});
        const auto cent_table = dal::homogen_table::wrap(cent_rows, cluster_count, x_train.get_column_count());
        this->exact_checks(x_train,
                           cent_table,
                           cent_table,
                           y,
                           cluster_count,
...
TEMPLATE_LIST_TEST_M(kmeans_batch_test,
    ...
    SECTION("cluster=5") {
        ...
        #if 0
        const csr_table & data_csr = static_cast<const csr_table&>(data);
        const auto row_count = data_csr.get_row_count();
        const auto column_count = data_csr.get_column_count();

        const auto [val, icol, prow] = csr_accessor<const float>(data_csr).pull({0, -1}, sparse_indexing::zero_based);
        std::vector<float> dense_data(row_count * column_count);

        for (auto& i : dense_data) { i = 0.0f; }

        for (auto i = 0; i < row_count; ++i) {
            const auto start_idx = prow[i];
            const auto end_idx = prow[i + 1];
            for (auto j = start_idx; j < end_idx; ++j) {
                dense_data[i * column_count + icol[j]] = val[j];
            }
        }

        std::cout << "======== CSR data as dense =======" << std::endl;
        for (auto i = 0; i < row_count; ++i) {
            for (auto j = 0; j < column_count; ++j) {
                std::cout << dense_data[i * column_count + j] << ", ";
            }
            std::cout << std::endl;
        }

        std::cout << "======== End of CSR data as dense =======" << std::endl;
        #endif
        bool init_centroids = true;
        this->test_on_sparse_data(input, 3, 0.01, init_centroids);
    }
/*
    SECTION("cluster=16") {
        bool init_centroids = true;
