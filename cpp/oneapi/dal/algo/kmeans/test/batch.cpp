...
TEMPLATE_LIST_TEST_M(kmeans_batch_test,
    ...
    SECTION("clusters=10") {
        this->test_on_dataset(higgs_path, 10, 3, 3.1997724684, 14717484.0);
    }

    SECTION("clusters=100") {
        this->test_on_dataset(higgs_path, 100, 3, 2.7450205195, 10704352.0);
    }

    SECTION("cluster=250") {
        this->test_on_dataset(higgs_path, 250, 3, 2.5923397174, 9335216.0);
    }
}
