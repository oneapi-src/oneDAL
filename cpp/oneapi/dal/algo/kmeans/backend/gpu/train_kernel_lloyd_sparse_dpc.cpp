...
sycl::event custom_spgemm(sycl::queue& q,
    ...
    const auto a_row_count = data.get_row_count();
    ...
    const auto local_size =
        std::min<std::int32_t>(bk::device_max_wg_size(q), bk::up_pow2(reduce_dim));
    ...
    return q.submit([&](sycl::handler& cgh) {
        ...
        cgh.parallel_for(nd_range, [=](auto item) {
            ...
            const Float scalar_mul =
                sycl::reduce_over_group(item.get_group(), acc, sycl::ext::oneapi::plus<Float>());
            if (local_id == 0) {
...
sycl::event assign_clusters(sycl::queue& q,
    ...
    auto dist_event = custom_spgemm(q, data, centroids, distances, Float(-2.0), Float(0), deps);
...
sycl::event update_centroids(sycl::queue& q,
    ...
    const auto column_count = data.get_column_count();
    const auto row_count = data.get_row_count();
    const auto data_ptr = values.get_data();
...
struct train_kernel_gpu<Float, method::lloyd_sparse, task::clustering> {
    train_result<task::clustering> operator()(const dal::backend::context_gpu& ctx,
        ...
        auto data_squares_event = compute_data_squares(queue, data, arr_data_squares);
