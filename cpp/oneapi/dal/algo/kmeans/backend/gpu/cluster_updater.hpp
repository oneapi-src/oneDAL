...
public:
    ...
    {
        ONEDAL_ASSERT(row_count_ == data.get_dimension(0));
        ONEDAL_ASSERT(column_count_ == data.get_dimension(1));
        ...
        ONEDAL_ASSERT(distance_block.get_dimension(1) == column_count_);
        ...
        ONEDAL_ASSERT(objective_function.get_dimension(1) == 1);
        ...
        auto assign_event = kernels_fp<Float>::template assign_clusters<pr::squared_l2_metric<Float>>(queue_,
                                                                                data_,
                                                                                initial_centroids_,
                                                                                block_size_in_rows,
                                                                                labels,
        ...
        auto count_event = count_clusters(queue_, labels, cluster_count_, counters_, { assign_event });
        auto objective_function_event = kernels_fp<Float>::compute_objective_function(queue_,
                                                                        closest_distances,
                                                                        objective_function,
                                                                        { assign_event });
        ...
        auto centroids_event = kernels_fp<Float>::partial_reduce_centroids(queue_,
                                                            ...
                                                            partial_centroids_,
                                                            { count_event });
        centroids_event = kernels_fp<Float>::merge_reduce_centroids(queue_,
                                                        counters_,
                                                        partial_centroids_,
                                                        part_count_,
                                                        centroids,
                                                        { count_event, centroids_event });
        count_empty_clusters(queue_, cluster_count_, counters_, empty_cluster_count_, { count_event });
