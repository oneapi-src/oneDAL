...
template <typename Graph>
ONEAPI_DAL_EXPORT void convert_to_csr_impl(const edge_list<vertex_type<Graph>> &edges, Graph &g) {
...
template ONEAPI_DAL_EXPORT void convert_to_csr_impl(
    const edge_list<vertex_type<graph_default>> &edges,
    graph_default &g);
