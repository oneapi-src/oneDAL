...
public:
    ...
    void test_1d_cumsum(const Type tol = 1.e-5) {
        ...
        auto [mut_input, input_event] = ndarray<Type, 1>::zeros(
            this->get_queue(), { this->n_ }, sycl::usm::alloc::device);
        
        auto mut_2d = mut_input.template reshape<2>({1, this->n_});
        auto immut_2d = immut_input.template reshape<2>({1, this->n_});
        auto copy_event = copy(this->get_queue(), mut_2d, immut_2d, { input_event });
        auto cum_event = cumulative_sum_1d(this->get_queue(), mut_input, { copy_event });
        ...
        std::cout << "SUCCESS " << this->n_ << std::endl;
        SUCCEED();
    }
