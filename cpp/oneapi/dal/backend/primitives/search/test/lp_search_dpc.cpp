...
class search_test : public te::float_algo_fixture<std::tuple_element_t<0, TestType>> {
    ...
    using metric_t = lp_metric<float_t>;
    using distance_t = lp_distance<float_t>;
...
public:
    ...
    void test_correctness(const float_t threshold = 1e-5) {
        ...
        if (m_ > k_) {
            ...
            const metric_t metric(this->p_);
            const distance_t distance(queue, metric);
            const search_t engine(queue, train, tblock, distance);
    ...
    auto naive_knn_search(const table& train_data, const table& infer_data) const {
    ...
    auto distances(const table& train_data, const table& infer_data) const {
        ...
        for (std::int64_t j = 0; j < n; ++j) {
            ...
            for (std::int64_t i = 0; i < m; ++i) {
                ...
                for (std::int64_t s = 0; s < d; ++s) {
                    const auto diff = std::abs(queue_row[s] - train_row[s]);
                    distances_ptr[j * m + i] += std::pow(diff, this->p_);
                }
            }
        }
        for (std::int64_t j = 0; j < n; ++j) {
            for (std::int64_t i = 0; i < m; ++i) {
                auto& ref = distances_ptr[j * m + i];
                ref = std::pow(ref, float_t(1.0) / this->p_);
            }
        }
