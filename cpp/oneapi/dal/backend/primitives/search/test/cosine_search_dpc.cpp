...
class search_test : public te::float_algo_fixture<std::tuple_element_t<0, TestType>> {
    ...
    using distance_t = cosine_distance<float_t>;
    ...
    static auto distances(const table& train_data, const table& infer_data) {
        ...
        for (std::int64_t j = 0; j < n; ++j) {
            ...
            for (std::int64_t i = 0; i < m; ++i) {
                ...
                float_t ip = 0.0, qn = 0.0, tn = 0.0;

                for (std::int64_t s = 0; s < d; ++s) {
                    const auto q = queue_row[s];
                    const auto t = train_row[s];
                    qn += q * q;
                    tn += t * t;
                    ip += q * t;
                }

                distances_ptr[j * m + i] = float_t(1.0) - ip / (std::sqrt(qn) * std::sqrt(tn));
            }
        }
