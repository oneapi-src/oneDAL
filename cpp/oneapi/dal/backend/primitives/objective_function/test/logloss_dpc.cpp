...
public:
    ...
    void test_gold_input() {
        ...
        const float_t data[n_ * p_] = { 0.83731708,  -0.70899924, -1.23362082, 0.23468538,
        ...
        auto p_event =
            compute_predictions(this->get_queue(), params_gpu, data_gpu, out_predictions, {});
    ...
    float_t test_predictions_and_logloss(const ndarray<float_t, 2>& data_host,
        ...
        for (int i = 0; i < n_; ++i) {
            ...
            logloss -=
                labels_host.at(i) * sycl::log(prob) + (1 - labels_host.at(i)) * sycl::log(1 - prob);
    ...
    void test_derivative_and_hessian(const ndarray<float_t, 2>& data,
        ...
        constexpr std::int64_t MAXN = 1000;
        ...
        for (int i = 0; i <= p_; ++i) {
            ...
            sycl::event logloss_up_event = compute_logloss(this->get_queue(),
                                                           params_gpu_up,
                                                           data,
                                                           labels,
                                                           out_logloss,
            ...
            sycl::event pred_up_event =
                compute_predictions(this->get_queue(), params_gpu_up, data, out_predictions, {});
            ...
            // Compute logloss with params [w0, w1, ... w_i - eps, ...., w_p]
            sycl::event logloss_down_event = compute_logloss(this->get_queue(),
                                                             params_gpu_down,
                                                             data,
                                                             labels,
                                                             out_logloss,
            ...
            sycl::event pred_down_event =
                compute_predictions(this->get_queue(), params_gpu_down, data, out_predictions, {});
            ...
            for (int j = 0; j <= p_; ++j) {
                ...
                REQUIRE(
                    abs((der_up_host.at(j) - der_down_host.at(j)) - 2 * eps * hessian.at(i, j)));
            }
            cur_param[i] += eps;
        }
    }
