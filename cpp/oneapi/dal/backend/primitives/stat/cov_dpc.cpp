...
sycl::event correlation(sycl::queue& queue,
    ...
    for_each_block(data, block_max_row_count, [&](const row_block_info& bi) {
        const auto x = ndview<Float, 2>::wrap(
            data.get_data() + data.get_dimension(1) * bi.get_start_row_index(),
            bi.get_shape());
        ONEDAL_ASSERT(x.get_data());
    ...
    // sycl::event gemm_event;
    // auto gemm_event = gemm(queue, data.t(), data, corr, Float(1), Float(0));
...
template <typename Float>
inline void validate_input(const ndview<Float, 2>& data,
                           const ndview<Float, 1>& sums,
...
sycl::event correlation(sycl::queue& q,
                        ...
                        ndview<Float, 1>& tmp,
                        const event_vector& deps) {
    validate_input(data, sums, corr, means, vars, tmp);
