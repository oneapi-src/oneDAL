...
public:
    ...
    static derived_t make(std::int64_t h, std::uint64_t l, derived_t result = {}) {
...
protected:
    ...
    auto to_floatingpoint() const {
        return (get_high_half() < 0l) ? -to_floatingpoint_pos<Float>(-(*this))
    ...
    static auto to_floatingpoint_pos(const int128_base<AnotherImpl>& x) {
        ...
        return static_cast<Float>(x.get_low_half()) +
               static_cast<Float>(x.get_high_half()) * mul.floatingpoint;
    }
...
public:
    ...
    template <typename Impl>
    int128_raw(const int128_base<Impl>& x) : high_(x.get_high_half()),
                                             low_(x.get_low_half()) {}
