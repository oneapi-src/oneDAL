...
void memcpy_usm2host(const data_parallel_policy& policy,
                     ...
                     std::int64_t size) {
    auto& queue = policy.get_queue();
    backend::memcpy_usm2host(queue, (byte_t*)dest_host, (byte_t*)src_usm, integral_cast<std::size_t>(size)).wait_and_throw();
}
...
void memcpy_host2usm(const data_parallel_policy& policy,
                     ...
                     std::int64_t size) {
    auto& queue = policy.get_queue();
    backend::memcpy_host2usm(queue, (byte_t*)dest_usm, (byte_t*)src_host, integral_cast<std::size_t>(size)).wait_and_throw();
}
