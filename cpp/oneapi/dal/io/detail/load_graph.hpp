...
struct collect_degrees_from_edge_list<Graph, /* IsInput = */ false> {
    template <typename Index, typename AtomicType>
    auto operator()(const edge_list<Index> &edges, AtomicType *degrees_cv) {
...
template <typename Graph>
struct collect_degrees_from_edge_list<Graph, /* IsInput = */ true> {
    template <typename Index, typename AtomicType>
    auto operator()(const edge_list<Index> &edges, AtomicType *degrees_cv) {
        dal::detail::threader_for_int64(edges.size(), [&](std::int64_t u) {
            ++degrees_cv[edges[u].first];
...
template <typename Graph>
struct get_edges_count<Graph, /* IsInput = */ false> {
    template <typename Index>
...
template <typename Graph>
struct get_edges_count<Graph, /* IsInput = */ true> {
    template <typename Index>
    Index operator()(Index degrees_sum) {
        return degrees_sum;
    }
...
template <typename Index, typename AtomicType>
void collect_outward_degrees_from_edge_list(const edge_list<Index> &edges, AtomicType *degrees_cv) {
    dal::detail::threader_for_int64(edges.size(), [&](std::int64_t u) {
        ++degrees_cv[edges[u].first];
    });
}
...
template <typename Graph>
struct fill_unfiltered_neighs<Graph, /* IsInput = */ false> {
...
template <typename Graph>
struct fill_unfiltered_neighs<Graph, /* IsInput = */ true> {
    template <typename Vertex, typename AtomicEdge>
    auto operator()(const edge_list<Vertex> &edges,
                    AtomicEdge *rows_vec_atomic,
                    Vertex *unfiltered_neighs) {
...
template <typename Vertex, typename AtomicEdge>
void fill_unfiltered_outward_neighs(const edge_list<Vertex> &edges,
                            AtomicEdge *rows_vec_atomic,
                            Vertex *unfiltered_neighs) {
    dal::detail::threader_for_int64(edges.size(), [&](std::int64_t u) {
        unfiltered_neighs[++rows_vec_atomic[edges[u].first] - 1] = edges[u].second;
...
void convert_to_csr_impl(const edge_list<typename graph_traits<undirected_adjacency_vector_graph<VertexValue, EdgeValue, GraphValue, IndexType, Allocator>>::vertex_type> &edges,
                         undirected_adjacency_vector_graph<VertexValue, EdgeValue, GraphValue, IndexType, Allocator> &g) {
    using Graph = undirected_adjacency_vector_graph<VertexValue, EdgeValue, GraphValue, IndexType, Allocator>;
    if (edges.size() == 0) {
        throw invalid_argument(dal::detail::error_messages::empty_edge_list());
    }
...
template <typename VertexValue = empty_value,
          ...
          typename IndexType = std::int32_t,
          typename Allocator = std::allocator<char>>
void convert_to_csr_impl(const edge_list<typename graph_traits<directed_adjacency_vector_graph<VertexValue, EdgeValue, GraphValue, IndexType, Allocator>>::vertex_type> &edges,
                         directed_adjacency_vector_graph<VertexValue, EdgeValue, GraphValue, IndexType, Allocator> &g) {
    using Graph = undirected_adjacency_vector_graph<VertexValue, EdgeValue, GraphValue, IndexType, Allocator>;
    if (edges.size() == 0) {
        throw invalid_argument(dal::detail::error_messages::empty_edge_list());
    }
    ...
    collect_outward_degrees_from_edge_list(edges, degrees_cv);
    ...
    fill_unfiltered_outward_neighs(edges, rows_vec_atomic, unfiltered_neighs);
