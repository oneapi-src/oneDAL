  ...
  while (getline(file, line)) {
    ...
    if (dest_endptr[0] != '\0') {
      throw invalid_argument(
          "Incorrect number of values per line or extra white-space: " + line);
    }
...
inline void read_edge_list(const std::string &name,
                           weighted_edge_list<Vertex, Weight> &elist) {
  std::ifstream file(name);
  if (!file.is_open()) {
    throw invalid_argument(dal::detail::error_messages::file_not_found());
  }
  ...
  while (getline(file, line)) {
    ...
    auto edge = std::tuple<Vertex, Vertex, Weight>(
        source_vertex, destination_vertex, edge_value);
...
struct get_edges_count<Graph, /* IsDirected = */ false> {
  template <typename Index> Index operator()(Index degrees_sum) {
...
template <>
ONEDAL_EXPORT std::int64_t
compute_prefix_sum<std::int64_t, std::int32_t>(const std::int32_t *degrees,
                                               std::int64_t degrees_count,
                                               std::int64_t *edge_offsets);

template <typename Index, typename AtomicIndex>
void fill_from_atomics(Index *arr, AtomicIndex *atomic_arr,
                       std::int64_t elements_count) {
  dal::detail::threader_for_int64(
      elements_count, [&](std::int64_t n) { arr[n] = atomic_arr[n].load(); });
}
...
struct fill_unfiltered_neighs<Graph, /* IsDirected = */ false> {
  ...
  auto operator()(const edge_list<Vertex> &edges, AtomicEdge *rows_vec_atomic,
                  Vertex *unfiltered_neighs) {
    dal::detail::threader_for_int64(edges.size(), [&](std::int64_t u) {
      unfiltered_neighs[++rows_vec_atomic[edges[u].first] - 1] =
          edges[u].second;
      unfiltered_neighs[++rows_vec_atomic[edges[u].second] - 1] =
          edges[u].first;
    });
  }
...
void fill_filtered_neighs(
    const Edge *unfiltered_offsets,
    const std::pair<Vertex, Weight> *unfiltered_neighs_vals,
    const Vertex *filtered_degrees, const Edge *filtered_offsets,
    Vertex *filtered_neighs, Weight *filtered_vals, std::int64_t vertex_count) {
  dal::detail::threader_for_int64(vertex_count, [&](std::int64_t u) {
    auto u_neighs = filtered_neighs + filtered_offsets[u];
...
ONEDAL_EXPORT void fill_filtered_neighs<std::int32_t, std::int64_t>(
    ...
    const std::int32_t *unfiltered_neighs, const std::int32_t *filtered_degrees,
    const std::int64_t *filtered_offsets, std::int32_t *filtered_neighs,
    std::int64_t vertex_count);
...
void filter_neighbors_and_fill_new_degrees(
    std::pair<Vertex, Weight> *unfiltered_neighs_vals,
    EdgeIndex *unfiltered_offsets, Vertex *new_degrees,
    std::int64_t vertex_count) {
  // removing self-loops,  multiple edges from graph, and make neighbors in CSR
  // sorted
  dal::detail::threader_for_int64(vertex_count, [&](std::int64_t u) {
    ...
    neighs_u_new_end = std::remove_if(
        start_p, neighs_u_new_end, [&u](std::pair<Vertex, Weight> &p) {
...
template <>
ONEDAL_EXPORT void
filter_neighbors_and_fill_new_degrees<std::int32_t, std::int64_t>(
    std::int32_t *unfiltered_neighs, std::int64_t *unfiltered_offsets,
    std::int32_t *new_degrees, std::int64_t vertex_count);

template <typename Graph>
void convert_to_csr_impl(
    const edge_list<typename graph_traits<Graph>::vertex_type> &edges,
    Graph &g) {
  if (edges.size() == 0) {
  ...
  edge_t total_sum_degrees = compute_prefix_sum_atomic<edge_t>(
      degrees_cv, vertex_count, rows_vec_atomic);
  ...
  auto [degrees_array, degrees_data] =
      ra.template allocate_array<vertex_set_t>(vertex_count);

  filter_neighbors_and_fill_new_degrees(unfiltered_neighs, unfiltered_offsets,
                                        degrees_data, vertex_count);

  auto edge_offsets_tup =
      ra.template allocate_array<edge_set_t>(vertex_count + 1);
  ...
  fill_filtered_neighs(unfiltered_offsets, unfiltered_neighs, degrees_data,
                       edge_offsets_data, vertex_neighbors, vertex_count);
  ...
  if (filtered_total_sum_degrees <
      oneapi::dal::detail::limits<std::int32_t>::max()) {
    using vertex_edge_t =
        typename graph_traits<Graph>::impl_type::vertex_edge_type;
    using vertex_edge_set_t =
        typename graph_traits<Graph>::impl_type::vertex_edge_set;

    oneapi::dal::preview::detail::rebinded_allocator ra_vertex_edge(
        graph_impl._vertex_edge_allocator);

    auto edge_offsets_tup =
        ra_vertex_edge.template allocate_array<vertex_edge_set_t>(vertex_count +
                                                                  1);
...
void convert_to_csr_impl(
    const weighted_edge_list<typename graph_traits<Graph>::vertex_type,
                             typename graph_traits<Graph>::edge_user_value_type>
        &edges,
    Graph &g) {
  if (edges.size() == 0) {
    throw invalid_argument(dal::detail::error_messages::empty_edge_list());
  ...
  auto [unfiltered_neighs_and_vals_array, unfiltered_neighs_and_vals] =
      ra.template allocate_array<dal::array<vertex_weight_pair>>(
          total_sum_degrees);
  unfiltered_neighs_and_vals =
      new (unfiltered_neighs_and_vals) vertex_weight_pair[total_sum_degrees]();

  fill_unfiltered_neighs<Graph>{}(edges, rows_vec_atomic,
                                  unfiltered_neighs_and_vals);
  ...
  filter_neighbors_and_fill_new_degrees(unfiltered_neighs_and_vals,
                                        unfiltered_offsets, degrees_data,
                                        vertex_count);
  ...
  auto [edge_values_array, edge_values_data] =
      ra.template allocate_array<edge_values<edge_value_type>>(
          filtered_total_sum_degrees);

  fill_filtered_neighs(unfiltered_offsets, unfiltered_neighs_and_vals,
                       degrees_data, edge_offsets_data, vertex_neighbors,
                       edge_values_data, vertex_count);
...
void read_from_edgelist(EdgeListType &elist, const DataSource &ds,
...
public:
  void operator()(const DataSource &ds, const Descriptor &desc,
                  typename Descriptor::object_t &graph) {
