...
void run(sycl::queue &queue) {
    const auto train_data_file_name =
        get_data_path("linear_regression_train_data.csv");
    const auto train_response_file_name =
        get_data_path("linear_regression_train_responses.csv");
    const auto test_data_file_name = get_data_path("linear_regression_test_data.csv");
    const auto test_response_file_name =
        get_data_path("linear_regression_test_responses.csv");
    ...
    const auto result_train = dal::preview::train(
        comm, lr_desc, x_train_vec.at(rank_id), y_train_vec.at(rank_id));

    const auto result_infer = dal::preview::infer(
        comm, lr_desc, x_test_vec.at(rank_id), result_train.get_model());
