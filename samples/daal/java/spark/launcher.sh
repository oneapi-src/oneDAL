...
export CLASSPATH=${SPARK_HOME}/jars/spark-core_2.11-2.0.0.jar:${SPARK_HOME}/jars/spark-sql_2.11-2.0.0.jar:${SPARK_HOME}/jars/spark-catalyst_2.11-2.0.0.jar:${DALROOT}/lib/onedal.jar:$CLASSPATH
...
if [ "$MKL_FPK_CPU_VERSION_LINE" == "2024.0.0" ]; then
    TBBLIBS=""
    if [ -d "${TBBROOT}/lib" ]; then
        TBBLIBS="${TBBROOT}/lib"
    fi
    if [ -z "${TBBLIBS}" ]; then
    ...
    if [ -d "${TBBROOT}/lib/intel64/gcc4.8" ]; then
        TBBLIBS="${TBBROOT}/lib/intel64/gcc4.8"
    fi
    if [ -z "${TBBLIBS}" ]; then
        echo "Can not find TBB runtimes"
        exit 1
