...
services::Status boundingBoxKernelImpl(DataType * posx, DataType * posy, const IdxType N, const IdxType nNodes, DataType & radius)
{
    ...
    std::cout << "**********************boundingBoxKernelImpl***************************" <<std::endl;
    ...
    std::cout << "sizeOfBlock = " << nThreads <<std::endl;
    ...
    std::cout << "box1 = " << box[0] << ",   " << box[1] << ",   " << box[2] << ",   " << box[3]   <<std::endl;
    ...
    std::cout << "box2 = " << box[0] << ",   " << box[1] << ",   " << box[2] << ",   " << box[3]   <<std::endl;
    //save results
...
template <typename IdxType, typename DataType, daal::CpuType cpu>
services::Status qTreeBuildingKernelImpl2(IdxType * child, const DataType * posx, const DataType * posy, const IdxType nNodes, const IdxType N,
                                         IdxType & maxDepth, IdxType & bottom, const DataType & radius, const IdxType & start_iter)
{
    // internal variables
    IdxType j      = 0;
    IdxType depth  = 0;
    ...
    if (start_iter == (IdxType)58) verbose = 0;
    if (verbose) for (int l = 0; l < nNodes+1; l++) std::cout << "posx[" << l <<"] = " << posx[l] <<std::endl;
    if (verbose) std::cout  << std::endl;
    if (verbose) for (int l = 0; l < nNodes+1; l++) std::cout << "posy[" << l <<"] = " << posy[l] <<std::endl;
    if (verbose) std::cout  << std::endl;
    //if (verbose) for (int l = 0; l < (nNodes+1)*4; l++) std::cout << "child[" << l <<"] = " << child[l] <<std::endl;
    ...
    while (i < N)
    {
        if (verbose) std::cout << "01" << std::endl;
        if (skip != 0)
        {
            if (verbose) std::cout << "02" << std::endl;
        ...
        {
            if (verbose) std::cout << "03" << std::endl;
            n = ch;
            depth++;
            r *= 0.5;
            ...
            //if (verbose) std::cout << "n * 4 + j = " << n * 4 + j << std::endl;
            //if (verbose) std::cout << "child[n * 4 + j] = " << child[n * 4 + j] << std::endl;
            //if (verbose) std::cout << "depth = " << depth << std::endl;
            //if (verbose) std::cout << "r = " << r << std::endl;
            //if (verbose) std::cout << "n = " << n << std::endl;
            //if (verbose) std::cout << "x = " << x << std::endl;
            //if (verbose) std::cout << "y = " << y << std::endl;
            // if (verbose) 
            // {
            //     char cha = getchar();
            //     if (cha == 'h') for (int l = 0; l < (nNodes+1)*4; l++) std::cout << "child[" << l <<"] = " << child[l] <<std::endl;
            // }
        }
        ...
        if (ch != -2)
        {
            if (verbose) std::cout << "04" << std::endl;
            ...
            {
                if (verbose) std::cout << "07" << std::endl;
                // Child node isn't empty, so we store the current value of the child, lock the leaf, and patch in a new cell
                if (posx[i] - posx[ch] <= 1e-15 && posy[i] - posy[ch] <= 1e-15)
                //if (posx[i] == posx[ch] && posy[i] == posy[ch])
                {
                    std::cout << "Identical points" << std::endl;
                    i += inc;
                    skip = 1;
                    continue;
                }
                if (child[locked] == ch)
                {
                    if (verbose) std::cout << "08" << std::endl;
                    //child[locked] = -2;
                    patch         = -1;

                    while (ch >= 0)
                    {
                        if (verbose) std::cout << "09" << std::endl;
                        depth++;
                        ...
                        {
                            if (verbose) std::cout << "11" << std::endl;
                            depth--;
                            continue;
                        }
                        ...
                        {
                            child[n * 4 + j] = cell;
                            if (verbose) std::cout << "12" << std::endl;
                        }

                        if (cell > patch)
                        {
                            patch = cell;
                            if (verbose) std::cout << "13" << std::endl;
                        }

                        // Insert migrated child node
                        if (verbose) std::cout << "14 = " << (int)(x < posx[ch]) << std::endl;
                        j = (x < posx[ch]) ? 1 : 0;
                        if (y < posy[ch])
                        {
                            if (verbose) std::cout << "15" << std::endl;
                            j |= 2;
                        }
                        ...
                        if (r <= 1e-10)
                        {
                            if (verbose) std::cout << "16" << std::endl;
                            break;
                        }
                    }
        ...
        if (skip == 2)
        {
            if (verbose) std::cout << "17" << std::endl;
            child[locked] = patch;
        }
    }
...
template <typename IdxType, typename DataType, daal::CpuType cpu>
services::Status summarizationKernelImpl2(IdxType * count, IdxType * child, DataType * mass, DataType * posx, DataType * posy, const IdxType nNodes,
...
services::Status qTreeBuildingKernelImpl(IdxType * child, const DataType * posx, const DataType * posy, const IdxType nNodes, const IdxType N,
                                         IdxType & maxDepth, IdxType & bottom, const DataType & radius, IdxType * duplicates)
{
    // internal variables
    IdxType j      = 0;
    IdxType depth  = 0;
    ...
    {
        std::cout << "pos[" << l << "] = " << posx[l] <<",    " << posy[l] << std::endl;
    }
...
services::Status summarizationKernelImpl(IdxType * count, IdxType * child, DataType * mass, DataType * posx, DataType * posy, const IdxType nNodes,
                                         const IdxType N, const IdxType & bottom, IdxType * duplicates)
{
    DAAL_CHECK_MALLOC(count);
    DAAL_CHECK_MALLOC(child);
    DAAL_CHECK_MALLOC(mass);
    ...
    const auto restart = k;
    std::cout  << "************** summarization kernel*******************"<<std::endl;
    // iterate over all cells assigned to thread
    while (k <= nNodes)
    {
        if (mass[k] < 0.)
    ...
    for (int l = 0; l < nNodes+1;l++)
    {
        std::cout << "mass,count[" << l << "] = " << mass[l] <<",     " << count[l] << std::endl;
    }

    std::cout << std::endl;
    for (int l = 0; l < nNodes+1;l++)
    {
        std::cout << "posx,posy, [" << l << "] = " << posx[l] <<",     " << posy[l] << ",      ";
        if (l < N) std::cout << duplicates[l] << std::endl;
        else std::cout << std::endl;
    }
    std::cout << std::endl;
    for (int l = 0; l < nNodes*4+4;l++)
    {
        std::cout << "child[" << l << "] = " << child[l] << std::endl;
    }
                                      ...
                                      const IdxType N, const IdxType nnz, const IdxType nElements, const DataType exaggeration)
{
    ...
    //Find_Normalization
    zNorm = (zNorm - DataType(N)) ? DataType(1) / (zNorm - DataType(N)) : (DataType(1) / 1e-6); // eps
                                         ...
                                         const NumericTablePtr paramTable, const NumericTablePtr resultTable)
{
    ...
    for (IdxType i = 0; i < explorationIter; ++i)
    {
        ...
        std::cout << i << ": divergence = " << divergence << std::endl;
    ...
    {
        std::cout << "iter of MAIN loop: " << i << std::endl;
        ...
        if (i == 608)
        {
            for (int l = 0; l < nNodes*4+4; l++)
            {
                std::cout << "MAIN child[" << l << "] = " << child[l] << std::endl;
            }
            std::cout << std::endl;
            for (int l = 0; l < nNodes+1; l++)
            {
                std::cout << "MAIN pos[" << l << "], mass, repx, repy = " << posx[l] <<",    " << posy[l] << ",    "<< mass[l] << ",    "<< repx[l]<<",    "<<repy[l] << std::endl;
            }
        }
        std::cout << "MAIN maxDepth, zNorm = [" << maxDepth <<"] ["<<zNorm <<"]" << std::endl;
        ...
        if (i == 608)
        {
            for (int l = 0; l < nNodes+1; l++)
            {
                std::cout << "MAIN pos[" << l << "] = " << posx[l] <<",    " << posy[l] << std::endl;
            }
            for (int l = 0; l < N; l++)
            {
                std::cout << "MAIN attr[" << l << "] = " << attrx[l] <<",    " << attry[l] << std::endl;
            }
        }
        std::cout << "MAIN zNorm, nElements, exaggeration = [" << zNorm <<"] ["<<nElements<<"] ["<<exaggeration<<"]"<< std::endl;
        std::cout << i << ": divergence = " << divergence << std::endl;
