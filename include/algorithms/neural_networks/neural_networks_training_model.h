...
public:
     ...
     */
    Parameter(const services::SharedPtr<optimization_solver::iterative_solver::Batch > &optimizationSolver_) :
...
public:
    ...
    /** \brief Constructor */
    Model() : _backwardLayers(new BackwardLayers()), _solverOptionalArgumentCollection() {}
     ...
     * \param[in] parameter  Parameters of the training. This parameter is not used in current version of the library.
     *
     ...
     */
    template<typename modelFPType>
    services::Status allocate(const services::Collection<size_t> &sampleSize, const daal::algorithms::Parameter *parameter = 0)
    {
        using namespace services;
        using namespace data_management;
        using namespace layers;
