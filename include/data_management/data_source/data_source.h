...
protected:
    ...
    void setNumericTableDictionary(NumericTablePtr nt)
    {
...
template<>
inline void DataSource::allocateNumericTableImpl(services::SharedPtr<AOSNumericTable> &nt)
{
    size_t nFeatures = _dict->getNumberOfFeatures();
    size_t structureSize = getStructureSize();
    nt = services::SharedPtr<AOSNumericTable>(new AOSNumericTable(structureSize, nFeatures, 0));
    setNumericTableDictionary(nt);
}

template<>
inline void DataSource::allocateNumericTableImpl(services::SharedPtr<SOANumericTable> &nt)
{
    nt = services::SharedPtr<SOANumericTable>();
}
...
public:
    ...
    services::Status resizeNumericTableImpl( size_t linesToLoad, NumericTable* nt)
    {
        if(!nt)
