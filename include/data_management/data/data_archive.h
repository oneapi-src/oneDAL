...
public:
    ...
    void setSingleObj(SerializationIface **ptr)
    {
        ...
        int serTag = segmentHeader();

        *ptr = Factory::instance().createObject(serTag);
        if( *ptr == 0 ) { this->_errors->add(services::ErrorObjectDoesNotSupportSerialization); return; }
    ...
    {
        SerializationIface *ptr = 0;

        int serTag = segmentHeader();
        ptr = Factory::instance().createObject(serTag);
        if( ptr == 0 ) { this->_errors->add(services::ErrorObjectDoesNotSupportSerialization); return services::SharedPtr<SerializationIface>(); }
        ptr->deserializeImpl( this );
        segmentFooter();

        return services::SharedPtr<SerializationIface>( ptr );
    }
