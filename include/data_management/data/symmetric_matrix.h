...
public:
    ...
    static services::SharedPtr<PackedSymmetricMatrix> create(const services::SharedPtr<DataType> &ptr, size_t nDim,
                                                             services::Status* stat = NULL)
    {
        services::SharedPtr<PackedSymmetricMatrix> ntPtr(new PackedSymmetricMatrix(nDim));
        if(ntPtr.get())
        {
            services::Status s = ntPtr->setArray(ptr);
            if(!s)
                ntPtr = services::SharedPtr<PackedSymmetricMatrix>();
            if(stat)
                *stat = s;
        }
        else
    ...
    static services::SharedPtr<PackedSymmetricMatrix> create(services::SharedPtr<DataType> &ptr, size_t nDim, const DataType &constValue,
                                                             services::Status* stat = NULL)
    {
        services::SharedPtr<PackedSymmetricMatrix> ntPtr(new PackedSymmetricMatrix(nDim));
        if(ntPtr.get())
