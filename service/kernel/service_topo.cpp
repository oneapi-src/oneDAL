...
static int __internal_daal_queryParseSubIDs(void)
{
    ...
    if(__internal_daal_allocateGenericAffinityMask(&glbl_obj.cpu_generic_processAffinity, lcl_OSProcessorCount))
        return -1;
    if(__internal_daal_allocateGenericAffinityMask(&glbl_obj.cpu_generic_systemAffinity, lcl_OSProcessorCount))
        return -1;
