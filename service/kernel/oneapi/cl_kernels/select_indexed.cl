...
DECLARE_SOURCE(
    ...
    void partition(__global algorithmFPType * pdist, __global int * plabels, int start, int end, int local_id, int local_size, algorithmFPType pivot,
                   int * split, int * total_big_num) {
    ...
    __kernel void quick_select_group(__global algorithmFPType * distances, __global int * labels, __global algorithmFPType * nbrdistances,
                                     __global int * nbrlabels, __global const algorithmFPType * rndseq, int RndPeriod, int N, int K,
                                     int BlockOffset) {
        const int global_id  = get_global_id(0);
        ...
        while (1)
        {
            ...
            if ((start + split) == K || (!split && !total_big_num)) break;
