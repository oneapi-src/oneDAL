...
String::String(size_t length, char placeholder) : _c_str(0)
{
    if (length)
    {
        ...
        for (size_t i = 0; i < length; i++) { _c_str[i] = placeholder; }
        _c_str[length] = '\0';
    }
}
...
{
    size_t strLength = 0;
    if (str) { strLength = strnlen(str, String::__DAAL_STR_MAX_SIZE); }
...
{
    if (_c_str) { return strnlen(_c_str, String::__DAAL_STR_MAX_SIZE); }
    return 0;
}
