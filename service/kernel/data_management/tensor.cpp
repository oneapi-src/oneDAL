...
DAAL_EXPORT SubtensorDescriptor<DataType>::SubtensorDescriptor()
    : _ptr(),
      _buffer(),
      _capacity(0),
      _tensorNDims(0),
    ...
    if ((block.getRWFlag() & (int)writeOnly) && !block.getInplaceFlag())
    {
        if (block.getLayout()->isDefaultLayout())
        {
            if (!IsSameType<T, DataType>::value)
            {
                ...
                internal::getVectorDownCast(features::internal::getIndexNumType<DataType>(), data_management::internal::getConversionDataType<T>())(
...
#define DAAL_IMPL_HOMOGENTENSORCONSTRUCTOR(T)                                                                                              \
    ...
    template <>                                                                                                                            \
    HomogenTensor<T>::HomogenTensor(const services::Collection<size_t> & dims, const services::SharedPtr<T> & data, services::Status & st) \
        : Tensor(&_layout), _layout(dims)                                                                                                  \
    {                                                                                                                                      \
        _ptr           = services::reinterpretPointerCast<byte, T>(data);                                                                  \
        _allocatedSize = 0;                                                                                                                \
...
#define DAAL_INSTANTIATE(T1, T2)                                                                                                       \
    template services::Status HomogenTensor<T1>::getTSubtensor(size_t, const size_t *, size_t, size_t, int, SubtensorDescriptor<T2> &, \
...
template <typename DataType>
template <typename T>
services::Status MklTensor<DataType>::getTSubtensor(size_t fixedDims, const size_t * fixedDimNums, size_t rangeDimIdx, size_t rangeDimNum, int rwFlag,
                                                    SubtensorDescriptor<T> & block, const TensorOffsetLayout & layout)
{
    ...
    if (pLayout->isRawLayout())
    {
        ...
        else
        {
            ...
            {
                data_management::internal::getVectorUpCast(features::internal::getIndexNumType<DataType>(),
                                                           data_management::internal::getConversionDataType<T>())(blockSize, _plainPtr + shift,
                                                                                                                  block.getPtr());
            }
        }
    }
        ...
        if (block.getLayout()->isDefaultLayout())
        {
            if (!IsSameType<T, DataType>::value)
            {
                ...
                data_management::internal::getVectorDownCast(features::internal::getIndexNumType<DataType>(),
                                                             data_management::internal::getConversionDataType<T>())(blockSize, block.getPtr(),
                                                                                                                    _plainPtr + shift);
            }
        }
        else
...
#define DAAL_IMPL_MKLTENSOR(T)                                                                                                                      \
    ...
    {                                                                                                                                               \
        ...
        {                                                                                                                                           \
            ...
            {                                                                                                                                       \
                ...
                if (!sameLayout)                                                                                                                    \
                {                                                                                                                                   \
                    err = allocateBufferCpu<T>((void **)&newPtr, (dnnLayout_t)newDnnLayout.get());                                                  \
                    ON_ERR(err);                                                                                                                    \
    ...
    {                                                                                                                                               \
        ...
        {                                                                                                                                           \
            syncDnnToPlain();                                                                                                                       \
            T * newPtr = NULL;                                                                                                                      \
            err        = allocateBufferCpu<T>((void **)&newPtr, (dnnLayout_t)_plainLayout.get());                                                   \
            ON_ERR(err);                                                                                                                            \
    ...
    MklTensor<T>::MklTensor(size_t nDim, const size_t * dimSizes)                                                                                   \
        : Tensor(&_layout),                                                                                                                         \
          _layout(services::Collection<size_t>(nDim, dimSizes)),                                                                                    \
          _dnnPtr(NULL),                                                                                                                            \
          _isDnnLayout(false),                                                                                                                      \
          _plainPtr(NULL),                                                                                                                          \
          _isPlainLayout(false)                                                                                                                     \
    ...
    MklTensor<T>::MklTensor(const services::Collection<size_t> & dims, AllocationFlag memoryAllocationFlag)                                         \
        : Tensor(&_layout), _layout(dims), _dnnPtr(NULL), _isDnnLayout(false), _plainPtr(NULL), _isPlainLayout(false)                               \
    {                                                                                                                                               \
        setPlainLayout();                                                                                                                           \
                                                                                                                                                    \
        if (memoryAllocationFlag == doAllocate)                                                                                                     \
...
#define DAAL_MKL_INSTANTIATE(T1, T2)                                                                                               \
    template services::Status MklTensor<T1>::getTSubtensor(size_t, const size_t *, size_t, size_t, int, SubtensorDescriptor<T2> &, \
                                                           const TensorOffsetLayout &);                                            \
