...
/**
 * <a name="DAAL-CLASS-ALGORITHMS__DBSCAN__InitDistributedStep2Master"></a>
...
public class InitDistributedStep2Master extends AnalysisDistributed {
    ...
    public InitDistributedStep2Master(DaalContext context, InitDistributedStep2Master other) {
        ...
        input     = new InitDistributedStep2MasterInput(getContext(), cGetInput     (cObject, precision.getValue(), method.getValue()));
     ...
     * @param minBinSize
     */
    public InitDistributedStep2Master(DaalContext context, Class<? extends Number> cls, InitMethod method, long maxBins, long minBinSize) {
        super(context);
        ...
        this.cObject = cInit(precision.getValue(), this.method.getValue(), maxBins, minBinSize);

        input     = new InitDistributedStep2MasterInput(getContext(), cGetInput     (cObject, precision.getValue(), method.getValue()));
        parameter = new InitParameter                 (getContext(), cInitParameter(cObject, precision.getValue(), method.getValue()));
        parameter.setMaxBins(maxBins);
        parameter.setMinBinSize(minBinSize);
    }
    ...
    public InitDistributedPartialResultStep2 compute() {
        super.compute();
        return new InitDistributedPartialResultStep2(getContext(), cGetPartialResult(cObject, precision.getValue(), method.getValue()));
    }
    ...
    private native long cInit(int precision, int method, long maxBins, long minBinSize);
